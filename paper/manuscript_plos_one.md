...
**Methods:** MetaSprint Autopilot is a single HTML file (27,605 lines) implementing 14 workflow tabs spanning topic discovery through manuscript drafting. The statistical engine provides DerSimonian-Laird and REML random-effects meta-analysis with HKSJ confidence intervals, publication bias diagnostics, GRADE assessment, subgroup and meta-regression analysis, and NNT computation. Specialized modules support diagnostic test accuracy (bivariate GLMM, HSROC), network meta-analysis (Bucher indirect comparisons, P-score ranking), and dose-response meta-analysis (restricted cubic splines, Greenland-Longnecker). Validation used a triple-blinded architecture against 291 Cochrane reviews, cross-validated against R metafor v4.8.0. Software quality was assessed through 670 automated tests across 17 test files.

**Results:** The engine achieved 100.0% accuracy across all 291 reviews (median difference: 1.65 x 10^-7). Cross-validation against R metafor confirmed exact agreement (CCC = 1.0000). Leave-one-out analysis showed 24.9% of reviews changed direction upon single-study removal, and 92.2% of prediction intervals crossed the null.
...
MetaSprint Autopilot was developed to address these gaps. It is a single HTML file (27,605 lines) that runs in any modern web browser (Chrome 90+, Firefox 90+, Edge 90+, Safari 15+) without installation, server infrastructure, internet connection (after initial download), or institutional license. It implements a 14-tab workflow covering all phases of systematic review — including specialized modules for diagnostic test accuracy, network meta-analysis, and dose-response meta-analysis — with a DerSimonian-Laird random-effects meta-analysis engine that has been validated against 291 Cochrane systematic reviews and cross-validated against the R metafor v4.8.0 reference implementation.

## Methods

### System design and architecture

MetaSprint Autopilot is implemented as a single HTML file containing HTML structure, CSS styling, and JavaScript application logic. This monolithic single-file architecture was chosen to maximize portability and eliminate deployment complexity: the application can be distributed via email attachment, USB drive, institutional intranet, or direct download from a repository. It runs in any modern browser without compilation, transpilation, package management, or server infrastructure. The complete application occupies approximately 300 KB uncompressed.
...
3. **Search**: Multi-database literature search across six sources — PubMed (via E-utilities API), ClinicalTrials.gov (via API v2), OpenAlex, Europe PMC, CrossRef, and AACT — with automatic cross-source deduplication via DOI matching and Levenshtein title similarity (threshold 0.85).
...
6. **Analyze**: DerSimonian-Laird random-effects meta-analysis with interactive SVG forest plots, funnel plots, leave-one-out sensitivity analysis, subgroup analysis with chi-squared test for interaction, publication bias diagnostics, automated GRADE certainty of evidence assessment (five domains with transparent scoring), Number Needed to Treat (NNT) computation with an interactive baseline risk slider, cumulative meta-analysis with SVG visualization, Duval-Tweedie trim-and-fill publication bias adjustment, and Fragility Index computation. Ratio measures (OR, RR, HR) are analyzed on the natural logarithm scale with back-transformation for presentation.
...
**Prediction intervals.** Prediction intervals, which estimate the range of true effects that might be observed in a future study, are computed using the t-distribution on k - 1 degrees of freedom, following the updated recommendation in Cochrane Handbook v6.5 (January 2025) [19] and the methods described by Higgins et al. [22] and Riley et al. [23].
...
**Fragility Index.** For binary outcomes (OR, RR, HR), the Fragility Index quantifies result robustness by counting how many individual study removals would change the pooled result from statistically significant (p < 0.05) to non-significant, or vice versa [28]. This study-level leave-one-out approach identifies the number and identity of studies whose removal would flip the statistical conclusion. The Fragility Quotient (FQ = FI/k) normalizes for the number of studies; FQ > 0.33 suggests the meta-analysis conclusion is sensitive to individual studies. The Fragility Index is computed for ratio measures only (OR, RR, HR) and is not applicable to continuous outcomes (MD, SMD).

**Subgroup analysis.** When studies are assigned subgroup labels in the data extraction table, the analysis automatically performs stratified meta-analysis by computing separate pooled estimates for each subgroup using the same DL+HKSJ engine. A test for subgroup differences is conducted using the chi-squared test for heterogeneity between subgroups (Cochrane Handbook v6.5, Section 10.11): Q_between = sum_j(w_j * (theta_j - theta_overall)^2) on J - 1 degrees of freedom, where J is the number of subgroups, w_j = 1/v_j is the inverse of each subgroup's pooled variance, and theta_overall is the fixed-effect weighted average of subgroup estimates. A p-value below 0.10 is flagged as evidence of subgroup interaction, consistent with the conventional threshold for exploratory subgroup analyses.
...
The automated test suite comprised 670 tests across 17 test files, executed via Playwright (end-to-end browser tests) and Selenium WebDriver (validation tests in headless Chrome), with no external services or API keys required:

- **Playwright end-to-end tests** (246 tests across 2 spec files): 233 benchmark tests covering the complete analysis pipeline (DL, HKSJ, REML, Egger, Peters, leave-one-out, prediction intervals, cumulative MA, trim-and-fill, fragility index, subgroup analysis, meta-regression, GRADE, NNT, forest/funnel plots, CSV export, manuscript generation, confidence level propagation, error conditions, edge cases, tab navigation, and UX behavior), plus 13 R cross-validation tests comparing JavaScript engine outputs against pre-computed R metafor v4.8.0, mada, and netmeta reference values.
...
The engine achieved 100.0% accuracy (291 of 291 reviews within pre-specified tolerances) across all three outcome data types: binary (202 of 202, 100.0%), continuous (63 of 63, 100.0%), and generic inverse-variance (26 of 26, 100.0%) (Table 2). Accuracy was maintained across all five study count strata, from small meta-analyses (k = 2-3, 76 of 76, 100.0%) to large meta-analyses (k > 20, 43 of 43, 100.0%) (Table 3).
...
DerSimonian-Laird and REML heterogeneity estimators showed near-perfect agreement for binary outcomes (CCC = 0.9994, MAD = 0.0104) and generic inverse-variance outcomes (CCC = 1.0000, MAD = 0.0092). Continuous outcomes showed greater estimator divergence (CCC = 0.7388, MAD = 1.0952) attributable to the heterogeneous effect scales across diverse clinical measurement units in the continuous outcome reviews (Table 6).
...
Statistical heterogeneity increased systematically with study count: 70% of reviews with k > 20 had I-squared > 50%, compared with 28% for reviews with k = 2-3 (S1 Table). Mean I-squared was 26.7% for small meta-analyses (k = 2-3) and 60.9% for large meta-analyses (k > 20). This pattern is consistent with the expectation that larger meta-analyses are more likely to include clinically diverse study populations, resulting in greater between-study variability.

### Leave-one-out sensitivity analysis

Leave-one-out analysis was performed on 245 reviews with k >= 3. Direction of the pooled effect changed upon removal of a single study in 61 reviews (24.9%), and statistical significance at alpha = 0.05 changed in 65 reviews (26.5%). The median range of leave-one-out pooled effects across all removed studies within each review was 0.3453 log units. These findings indicate that approximately one quarter of published meta-analyses contain at least one study whose inclusion or exclusion changes the qualitative conclusion, underscoring the importance of routine sensitivity analysis.

### Prediction intervals

Prediction intervals were computed for 245 reviews with k >= 3. Of these, 226 reviews (92.2%) had prediction intervals crossing the null, indicating that even when the pooled effect is nominally statistically notable, the predicted true effect in a new study setting frequently includes the possibility of no benefit or harm. The median prediction interval width was 2.96 log units. This finding supports the recommendation that prediction intervals should be routinely reported alongside confidence intervals in meta-analyses [22].
...
| Multi-database search | Yes (6) | No | No | No | No | No |
...
MetaSprint Autopilot is a free, open-source, zero-install platform for systematic review and meta-analysis that achieves 100.0% accuracy across 291 Cochrane systematic reviews, validated through a triple-blinded architecture with SHA-256 sealed references and cross-validated against R metafor v4.8.0. The median pooled effect difference of 1.65 x 10^-7 demonstrates near-exact numerical agreement. The single-file design maximizes accessibility for medical students and clinicians globally, particularly in settings where institutional software licenses, reliable internet, or local IT infrastructure may be unavailable.

## Ethics statement

This study involved computational validation using publicly available Cochrane systematic review datasets and publicly registered clinical trial data from ClinicalTrials.gov. No human participants, patient data, or animal subjects were involved. No ethics committee approval was required.
...
[19] Higgins JPT, Thomas J, Chandler J, Cumpston M, Li T, Page MJ, et al., editors. Cochrane Handbook for Systematic Reviews of Interventions version 6.5 (updated January 2025). Cochrane; 2025. Available from: https://training.cochrane.org/handbook
...
[28] Walsh M, Srinathan SK, McAuley DF, Mrkobrada M, Levine O, Ribic C, et al. The statistical significance of randomized controlled trial results is frequently fragile: a case for a Fragility Index. J Clin Epidemiol. 2014;67(6):622-628. https://doi.org/10.1016/j.jclinepi.2013.10.019
...
**S3 Fig.** Screenshot of MetaSprint Autopilot showing the 14-tab workflow interface with forest plot and funnel plot outputs.
