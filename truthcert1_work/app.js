...
function safeParseJSON(e, t = null) {
  if ("string" != typeof e || !e.trim()) return t;
  try {
    return JSON.parse(e)
  } catch (e) {
    return t
...
function win1252ByteFromCharCode(e) {
  return e <= 255 ? e : WINDOWS_1252_EXT_BYTES[e] ?? null
}

function decodeMojibakeText(e) {
  if ("string" != typeof e || !e || "undefined" == typeof TextDecoder || !MOJIBAKE_PATTERN.test(e)) return e;
  let t = e;
  for (let e = 0; e < 3; e++) {
    if (!MOJIBAKE_PATTERN.test(t)) break;
    const e = new Uint8Array(t.length);
    let n = !1;
    for (let a = 0; a < t.length; a++) {
      const s = win1252ByteFromCharCode(t.charCodeAt(a));
      if (null === s) {
        n = !0;
        break
      }
    ...
    if (n) break;
    const a = new TextDecoder("utf-8").decode(e);
    if (!a || a === t || a.includes("\uFFFD")) break;
    t = a
  }
  return t
}

function stripResidualMojibakeArtifacts(e) {
  return "string" != typeof e ? e : e.includes("\uFFFD") ? e.replace(/\uFFFD+/g, "").replace(/\s{2,}/g, " ") : e
}

function normalizeMojibakeInDOM(e = document && document.body ? document.body : null) {
  if (!e || "undefined" == typeof document) return;
  try {
    const t = "undefined" != typeof NodeFilter && document.createTreeWalker ? document.createTreeWalker(e, NodeFilter.SHOW_TEXT, null) : null,
      n = [];
    if (t) {
      let e;
      for (; e = t.nextNode();) n.push(e)
    } else e.querySelectorAll && e.querySelectorAll("*").forEach(e => {
      e.childNodes && e.childNodes.forEach && e.childNodes.forEach(e => {
        3 === e.nodeType && n.push(e)
      })
    });
    n.forEach(e => {
      const t = e.nodeValue,
        n = stripResidualMojibakeArtifacts(decodeMojibakeText(t));
      n && n !== t && (e.nodeValue = n)
    });
    e.querySelectorAll && e.querySelectorAll("[title],[aria-label],[placeholder]").forEach(e => {
      ["title", "aria-label", "placeholder"].forEach(t => {
        if (e.hasAttribute && e.hasAttribute(t)) {
          const n = e.getAttribute(t),
            a = stripResidualMojibakeArtifacts(decodeMojibakeText(n));
          a && a !== n && e.setAttribute(t, a)
        }
      })
    })
  } catch (e) {
    log.warn("normalizeMojibakeInDOM skipped:", e.message || e)
  }
}

let mojibakeObserverStarted = !1;

function startMojibakeObserver() {
  if (mojibakeObserverStarted || "undefined" == typeof MutationObserver || "undefined" == typeof document || !document.body) return;
  const e = new MutationObserver(e => {
    e.forEach(e => {
      if ("characterData" === e.type && e.target && "string" == typeof e.target.nodeValue) {
        const t = stripResidualMojibakeArtifacts(decodeMojibakeText(e.target.nodeValue));
        t && t !== e.target.nodeValue && (e.target.nodeValue = t);
        return
      }
      e.addedNodes && e.addedNodes.forEach(e => {
        3 === e.nodeType ? e.nodeValue = stripResidualMojibakeArtifacts(decodeMojibakeText(e.nodeValue || "")) : 1 === e.nodeType && normalizeMojibakeInDOM(e)
      }), "attributes" === e.type && e.target && e.attributeName && e.target.getAttribute && e.target.setAttribute && (() => {
        const t = e.target.getAttribute(e.attributeName),
          n = stripResidualMojibakeArtifacts(decodeMojibakeText(t));
        n && n !== t && e.target.setAttribute(e.attributeName, n)
      })()
    })
  });
  ...
  }), mojibakeObserverStarted = !0
}

function getElementByIds(e) {
  if ("undefined" == typeof document || !Array.isArray(e)) return null;
...
function canUseUndoManager() {
  return "undefined" != typeof UndoManager && UndoManager && "function" == typeof UndoManager.saveState
}
...
function initializeTabs() {
  ...
  e.forEach(n => {
    n.addEventListener("click", () => {
      ...
      }), "multioutcome" === a && populateOutcomeDictionary(), "verdict" === a && "function" == typeof renderVerdictPanel && renderVerdictPanel(), "hta" === a && "function" == typeof populateHTADropdowns && populateHTADropdowns(), "function" == typeof onTabActivate && onTabActivate(a), safeTimeout(() => normalizeMojibakeInDOM(document.body), 0)
    })
  })
}
...
function initializeStudyTable() {
  ...
  for (let e = 0; e < 5; e++) addStudyRow(null, !1);
  updateDataSummary()
}
...
const TAU2_METHOD_OPTIONS = [{
  value: "REML",
  label: "REML (Recommended)"
}, {
  value: "DL",
  ...
  value: "PMM",
  label: "Paule-Mandel Median-Unbiased"
}, {
  value: "ML",
  label: "Maximum Likelihood"
}, {
  value: "HS",
  label: "Hunter-Schmidt"
}, {
  value: "HSk",
  label: "Hunter-Schmidt (k-corrected)"
}, {
  value: "SJ",
  label: "Sidik-Jonkman"
}, {
  ...
  value: "BMM",
  label: "Binomial-Normal Marginal"
}, {
  value: "QG",
  label: "Q-Generalized Robust"
}];

function syncTau2MethodSelectors() {
  const e = document.getElementById("tau2MethodSelect"),
    t = document.getElementById("tau2MethodSelectPanel");
  if (!e && !t) return;
  const n = TAU2_METHOD_OPTIONS.map(e => `<option value="${e.value}">${e.label}</option>`).join(""),
    a = new Set(TAU2_METHOD_OPTIONS.map(e => e.value));
  let s = AppState.settings.tau2Method;
  a.has(s) || (s = "REML");
  e && (e.innerHTML = n, e.value = s);
  t && (t.innerHTML = n, t.value = s);
  AppState.settings.tau2Method = s
}
...
function rebuildTable() {
  ...
  for (let e = 0; e < n; e++) addStudyRow(null, !1);
  updateDataSummary()
}

function addStudyRow(e = null, t = !0) {
  t && canUseUndoManager() && UndoManager.saveState("Add study row");
  const n = document.getElementById("studyTableBody"),
    a = n.children.length + 1,
    s = AppState.settings.dataType,
    i = getTableColumns(s),
    r = document.createElement("tr");
  let o = `<td class="font-mono text-secondary">${a}</td>`;
  i.forEach(t => {
    const n = e && void 0 !== e[t.key] ? e[t.key] : "",
      a = "text" === t.type ? sanitizeHTML(String(n)) : n,
      i = t.step ? `step="${t.step}"` : "",
      r = "number" === t.type && "yi" !== t.key ? 'min="0"' : "";
    o += `<td><input type="${t.type}" class="input input--sm ${"number"===t.type?"input--mono":""}" \n          placeholder="${t.placeholder}" ${r} ${i} value="${a}" data-key="${t.key}"></td>`
  }), o += '<td><button class="btn btn--ghost btn--icon btn--sm remove-study-btn" title="Remove study" aria-label="Remove study">Ã—</button></td>', r.innerHTML = o, n.appendChild(r), r.querySelector(".remove-study-btn").addEventListener("click", () => {
    canUseUndoManager() && UndoManager.saveState("Remove study row"), r.remove(), renumberRows(), updateDataSummary()
  }), r.querySelectorAll("input").forEach(e => {
    const t = () => {
      validateStudyRow(r, s), updateDataSummary()
    };
    e.dataset.prevValue = e.value, e.addEventListener("focus", () => {
      e.dataset.prevValue = e.value
    }), e.addEventListener("change", () => {
      canUseUndoManager() && e.dataset.prevValue !== e.value && UndoManager.saveState("Edit study row"), e.dataset.prevValue = e.value, t()
...
function _updateDataSummary() {
  ...
  if (0 === e.length) a.className = "alert alert--info", a.innerHTML = '\n          <span class="alert__icon">Ã¢â€žÂ¹Ã¯Â¸Â</span>\n          <div class="alert__content">\n            <div class="alert__text">Enter study data or load a demo dataset to begin.</div>\n          </div>\n        ';
    ...
    a.innerHTML = `\n          <span class="alert__icon">Ã¢Å“â€œ</span>\n          <div class="alert__content">\n            <div class="alert__text">${e.length} valid studies${n>0?", "+n.toLocaleString()+" "+s:""}. Ready for analysis.</div>\n          </div>\n        `
  }
  AppState.studies = e
}
document.addEventListener("DOMContentLoaded", function() {
  window.showToast = showToast, initializeTabs(), syncTau2MethodSelectors(), updateEffectMeasures(), initializeStudyTable(), bindEventListeners(), initializeTheme(), startMojibakeObserver(), safeTimeout(() => normalizeMojibakeInDOM(document.body), 0), canUseUndoManager() && (UndoManager.clear(), UndoManager.saveState("Initial state")), document.getElementById("themeToggle").addEventListener("click", toggleTheme), document.getElementById("loadDemoBtn").addEventListener("click", () => {
...
function getStudyTableSnapshot() {
  const e = document.getElementById("studyTableBody");
  if (!e) return [];
  const t = getTableColumns(AppState.settings.dataType),
    n = [];
  return Array.from(e.children).forEach(e => {
    const a = {},
      s = e.querySelectorAll("input");
    t.forEach((e, t) => {
      a[e.key] = s[t] ? s[t].value : ""
    }), n.push(a)
  }), n
}
function bindEventListeners() {
  document.getElementById("addStudyBtn").addEventListener("click", () => {
    ...
    confirm("Clear all study data?") && (canUseUndoManager() && UndoManager.saveState("Clear all data"), initializeStudyTable())
    ...
    if (t) {
      try {
        ...
        n.success ? alert(`Ã¢Å“â€¦ ${n.message}`) : alert(`Ã¢ÂÅ’ Import failed: ${n.message}`)
      } catch (e) {
        log.error("KM import error:", e), alert(`Ã¢ÂÅ’ Error reading file: ${e.message}`)
      }
      e.target.value = ""
    }
...
function estimateTau2_REML(e, t, n = 100, a = 1e-8) {
  ...
  return p.converged ? {
    ...
    iterations: n,
    method: "REML",
    warning: "REML failed to converge - using DL estimate. DL may underestimate Ãâ€žÃ‚Â² by 20-40% in some cases. Consider using PM estimator for this dataset.",
    fallback: "DL"
  }
}
...
function estimateTau2_PMM(e, t, n = 100, a = 1e-8) {
  const s = e.length;
  if (s < 2) return {
    tau2: 0,
    converged: !0,
  ...
  let d = estimateTau2_PM(e, t).tau2;
  for (let n = 0; n < 20; n++) {
    const s = t.map(e => 1 / (e + d)),
      i = sum(s),
      r = sum(s.map(e => e * e)) / i,
      c = l(e, t, d),
      u = i - r,
      p = Math.max(0, d + (c - o) / u);
    if (Math.abs(c - o) < a || Math.abs(p - d) < a) return {
      tau2: p,
      converged: !0,
      iterations: n + 1,
      method: "PMM"
  ...
  let c = 0,
    u = Math.max(10 * d, 1),
    p = 0;
  for (; l(e, t, u) > o && u < 1e6 && p++ < 30;) u *= 2;
  for (let n = 0; n < 100; n++) {
    const s = (c + u) / 2,
      i = l(e, t, s);
    if (Math.abs(i - o) < a || u - c < a) return {
      tau2: s,
      converged: !0,
      iterations: n + 21,
      method: "PMM"
  ...
  return {
    ...
    iterations: 121,
    method: "PMM",
    warning: "Median-unbiased PM did not fully converge"
  }
}
...
function estimateTau2_HSk(e, t) {
  // Hunter-Schmidt with k correction (metafor method="HSk")
  // Intercept-only form from rma.uni source:
  // tau2 = ((k/(k-1))*Q - k) / sum(wi)
  const n = e.length;
  if (n < 2) return {
    tau2: 0,
    Q: 0,
...
function estimateTau2_SJ(e, t) {
  // Sidik-Jonkman estimator (approximates metafor method="SJ")
  // Uses modified formula based on sample variance adjustment
  const k = e.length;
  const w = t.map(vi => 1 / vi);
  const W = sum(w);
  const theta = sum(e.map((yi, i) => w[i] * yi)) / W;
  ...
  // SJ uses formula: tau2 = (Q - (k-1)) / C_adj
  // where C_adj is adjusted to give larger tau2 than DL
  // Approximation: C_adj = C * (k-1) / k - small_correction
  const C_adj = C * (k - 1) / k - k * 0.5;
  const tau2 = Math.max(0, (Q - (k - 1)) / Math.max(C_adj, C * 0.8));
...
function estimateTau2_EB(e, t, maxIter = 100, tol = 1e-8) {
  // Empirical Bayes estimator (matches metafor method="EB")
  // Uses Paule-Mandel style iteration: find tau2 where Q(tau2) = k - 1
  const k = e.length;
  const p = 1;
  ...
  const Q_tau = (tau2) => {
    ...
    const W = sum(w);
    const theta = sum(e.map((yi, i) => w[i] * yi)) / W;
    return sum(e.map((yi, i) => w[i] * Math.pow(yi - theta, 2)));
  };

  let tau2_lo = 0;
  let tau2_hi = 10;

  while (Q_tau(tau2_hi) > k - p && tau2_hi < 1000) {
    tau2_hi *= 2;
  }

  if (Q_tau(0) <= k - p) {
    return { tau2: 0, converged: true, iterations: 0, method: "EB" };
  }

  let tau2 = (tau2_lo + tau2_hi) / 2;
  for (let iter = 0; iter < maxIter; iter++) {
    const Q = Q_tau(tau2);
    const diff = Q - (k - p);

    if (Math.abs(diff) < tol) {
      return { tau2: tau2, converged: true, iterations: iter + 1, method: "EB" };
    }
...
function estimateTau2_GENQM(e, t) {
  const n = e.length;
  if (n < 2) return {
    tau2: 0,
    method: "GENQM"
  };
  let a = estimateTau2_GENQ(e, t).tau2;
  for (let s = 0; s < 50; s++) {
    const i = t.map(e => 1 / (e + a)),
      r = sum(i),
      o = sum(i.map(e => e * e)),
      l = sum(e.map((e, t) => i[t] * e)) / r,
      d = sum(e.map((e, t) => i[t] * Math.pow(e - l, 2))),
      c = r - o / r,
      u = (n - 1) / n,
      p = Math.max(0, (d - (n - 1) * u) / c);
    if (Math.abs(p - a) < 1e-8) return {
      tau2: p,
      converged: !0,
      iterations: s + 1,
      method: "GENQM"
...
function estimateTau2_DL2(e, t) {
  const n = e.length;
  if (n < 2) return {
    tau2: 0,
    method: "DL2"
  };
  const a = estimateTau2_DL(e, t).tau2,
    s = t.map(e => 1 / (e + a)),
    i = sum(s),
    r = sum(s.map(e => e * e)),
    o = sum(e.map((e, t) => s[t] * e)) / i,
    l = sum(e.map((e, t) => s[t] * Math.pow(e - o, 2))),
    d = i - r / i,
    c = Math.max(0, (l - (n - 1)) / d);
  return {
    tau2: c,
    tau2_step1: a,
    method: "DL2",
...
function estimateTau2_CA(e, t) {
  ...
  const a = sum(e) / n,
    s = sum(e.map(e => Math.pow(e - a, 2))),
    i = sum(t) / n,
    r = Math.max(0, s / (n - 1) - i);
  return {
    tau2: r,
    ybar: a,
    SSB: s,
...
function estimateTau2_BMM(e, t) {
  const n = e.length;
  if (n < 2) return {
    tau2: 0,
    method: "BMM"
  };
  let a = estimateTau2_DL(e, t).tau2;
  for (let s = 0; s < 100; s++) {
    const i = t.map(e => 1 / (e + a)),
      r = sum(i),
      o = sum(e.map((e, t) => i[t] * e)) / r,
      l = -0.5 * sum(e.map((e, n) => Math.log(t[n] + a) + Math.pow(e - o, 2) / (t[n] + a))),
      d = -0.5 * sum(e.map((e, n) => 1 / (t[n] + a) - Math.pow(e - o, 2) / Math.pow(t[n] + a, 2))),
      c = 0.5 * sum(t.map(e => 1 / Math.pow(e + a, 2))),
      u = Math.max(0, a + d / c);
    if (Math.abs(u - a) < 1e-8) return {
      tau2: u,
      converged: !0,
      iterations: s + 1,
      logLik: l,
...
function estimateTau2_QG(e, t) {
  ...
  const a = [...e].sort((e, t) => e - t),
    s = n % 2 == 0 ? (a[n / 2 - 1] + a[n / 2]) / 2 : a[Math.floor(n / 2)],
    i = e.map(e => Math.abs(e - s)),
    r = i.sort((e, t) => e - t)[Math.floor(n / 2)] * 1.4826,
    o = 1.345,
    l = e.map((e, n) => {
      const a = (e - s) / (r + 1e-4),
        i = Math.abs(a) <= o ? 1 : o / Math.abs(a);
      return i / t[n]
    }),
    d = sum(l),
    c = sum(l.map(e => e * e)),
    u = sum(e.map((e, t) => l[t] * e)) / d,
    p = sum(e.map((e, t) => l[t] * Math.pow(e - u, 2))),
    m = d - c / d,
    h = Math.max(0, (p - (n - 1)) / m);
  return {
    tau2: h,
    median: s,
    mad: r,
...
function estimateTau2(e, t, n = "REML") {
  const a = {
    ...
    PM: estimateTau2_PM,
    PMM: estimateTau2_PMM,
    ML: estimateTau2_ML,
    HS: estimateTau2_HS,
    HSk: estimateTau2_HSk,
    SJ: estimateTau2_SJ,
    HE: estimateTau2_HE,
    EB: estimateTau2_EB,
    GENQ: estimateTau2_GENQ,
    GENQM: estimateTau2_GENQM,
    PL: estimateTau2_ProfileLikelihood,
    DL2: estimateTau2_DL2,
    CA: estimateTau2_CA,
    BMM: estimateTau2_BMM,
    QG: estimateTau2_QG
  };
...
function estimateTau2_All(e, t) {
  const n = ["DL", "REML", "PM", "PMM", "ML", "HS", "HSk", "SJ", "HE", "EB", "GENQ", "GENQM", "PL", "DL2", "CA", "BMM", "QG"],
    a = {};
...
function randomEffectsLogLikelihood(e, t, n, a = null) {
  if (!Array.isArray(e) || !Array.isArray(t) || e.length !== t.length || 0 === e.length) return NaN;
  if (!isFinite(n) || n < 0) return NaN;
  const s = t.map(e => 1 / (e + n));
  if (s.some(e => !isFinite(e) || e <= 0)) return NaN;
  const i = sum(s);
  if (!(i > 0)) return NaN;
  const r = null == a ? sum(e.map((e, t) => s[t] * e)) / i : a;
  let o = 0;
  for (let a = 0; a < e.length; a++) {
    const s = t[a] + n;
    if (!(s > 0) || !isFinite(s)) return NaN;
    const i = e[a] - r;
    o += -.5 * (Math.log(2 * Math.PI * s) + i * i / s)
  }
  return o
}

function computePairwiseEstimatorConsensus(e, t, n = "REML") {
  const a = estimateTau2_All(e, t),
    s = e.length,
    i = {},
    r = [];
  for (const [o, l] of Object.entries(a)) {
    const a = l || {},
      ...
      c = !1 !== a.converged;
    if (isFinite(d) && d >= 0) {
      const l = calculatePooledEstimate(e, t, d),
        u = calculateHeterogeneity(e, t, d),
        p = randomEffectsLogLikelihood(e, t, d, l.theta),
        m = 2,
        h = isFinite(p) ? -2 * p + 2 * m : NaN,
        v = isFinite(h) && s > m + 1 ? h + 2 * m * (m + 1) / (s - m - 1) : h,
        g = isFinite(p) ? -2 * p + m * Math.log(Math.max(s, 2)) : NaN,
        f = {
          ...a,
          method: o,
          tau2: d,
          tau: Math.sqrt(Math.max(0, d)),
          converged: c,
          theta: l.theta,
          se: l.se,
          ci_lower: l.ci_lower,
      ...
      tau: NaN,
      converged: !1,
      error: a.error || "Invalid estimate",
      weight: 0
    }
  }
  const o = r.filter(e => isFinite(e.theta) && isFinite(e.se) && isFinite(e.aicc));
  let l = [];
  if (o.length > 0) {
    const e = Math.min(...o.map(e => e.aicc)),
      t = o.map(t => {
        let n = Math.exp(-.5 * (t.aicc - e));
        return !1 === t.converged && (n *= .5), n
      }),
      n = sum(t),
      a = n > 0 ? n : o.length;
    l = o.map((e, n) => ({
  ...
  const d = i[n] || l[0] || null;
  let c = null;
  if (l.length > 0) {
    const n = l.reduce((e, t) => e + t.weight * t.theta, 0),
      a = l.reduce((e, t) => e + t.weight * t.tau2, 0),
      i = l.reduce((e, t) => e + t.weight * t.se * t.se, 0),
      r = l.reduce((e, t) => e + t.weight * Math.pow(t.theta - n, 2), 0),
      o = Math.sqrt(Math.max(0, i + r)),
      d = o > 0 ? n / o : NaN,
      u = isFinite(d) ? 2 * (1 - pnorm(Math.abs(d))) : NaN;
    c = {
      method: "AICc-weighted estimator ensemble",
      theta: n,
      se: o,
      ci_lower: n - 1.959964 * o,
      ci_upper: n + 1.959964 * o,
      p_value: u,
      tau2: a,
      tau: Math.sqrt(Math.max(0, a)),
      I2: calculateI2Metafor(a, t, s),
      top_method: l[0].method,
      top_weight: l[0].weight
    }
  }
  const u = l.map(e => e.theta),
    p = l.map(e => e.tau2),
    m = l.map(e => e.I2).filter(e => isFinite(e)),
    h = u.map(e => Math.sign(e)).filter(e => 0 !== e),
    v = h.length ? Math.max(h.filter(e => e > 0).length, h.filter(e => e < 0).length) / h.length : 1,
    g = l.filter(e => isFinite(e.p_value) && e.p_value < .05).length,
    f = l.length || 1,
    _ = Math.max(g / f, 1 - g / f),
    y = {
      methods_valid: l.length,
      theta_min: u.length ? Math.min(...u) : NaN,
      theta_max: u.length ? Math.max(...u) : NaN,
      theta_range: u.length ? Math.max(...u) - Math.min(...u) : NaN,
      tau2_min: p.length ? Math.min(...p) : NaN,
      tau2_max: p.length ? Math.max(...p) : NaN,
      tau2_range: p.length ? Math.max(...p) - Math.min(...p) : NaN,
      I2_min: m.length ? Math.min(...m) : NaN,
      I2_max: m.length ? Math.max(...m) : NaN,
      I2_range: m.length ? Math.max(...m) - Math.min(...m) : NaN,
      direction_agreement: v,
      significance_agreement: _,
      significant_methods: g
    },
    b = {
      direction_stable: v >= .875,
      significance_stable: _ >= .75,
      tau2_stable: isFinite(y.tau2_range) && y.tau2_range <= .15,
      effect_stable: isFinite(y.theta_range) && y.theta_range <= .2
    };
  return b.overall = b.direction_stable && b.significance_stable && (b.tau2_stable || b.effect_stable) ? "stable" : "sensitive", {
    available: l.length > 0,
    selected_method: n,
    selected: d,
    consensus: c,
    sensitivity: y,
...
function buildBinary2x2Tables(e) {
  return (Array.isArray(e) ? e : []).map(e => {
    const t = Number(e.events_t),
      n = Number(e.n_t),
      a = Number(e.events_c),
      s = Number(e.n_c);
    if (!isFinite(t) || !isFinite(n) || !isFinite(a) || !isFinite(s) || n <= 0 || s <= 0 || t < 0 || a < 0 || t > n || a > s) return null;
    const i = n - t,
      r = s - a;
    return i < 0 || r < 0 ? null : {
      a: t,
...
function computeEstimatorEnsemblePI(e, t, n, a = .05) {
  if (!e || !e.available || !e.consensus || !isFinite(n) || n < 2) return {
    skipped: !0,
    reason: "Estimator consensus unavailable"
  };
  const s = e.consensus,
    i = isFinite(s.tau2) && s.tau2 >= 0 ? s.tau2 : 0,
    r = isFinite(s.se) && s.se > 0 ? s.se : t && isFinite(t.se) ? t.se : null;
  if (!isFinite(s.theta) || !isFinite(r)) return {
    skipped: !0,
    reason: "Invalid consensus uncertainty"
  };
  const o = Math.max(1e-12, r * r - i),
    l = predictionInterval_Standard(s.theta, Math.sqrt(o), i, n, a, "k-2");
  return {
    ...l,
    method: "ensemble",
    source: "AICc-weighted estimator ensemble",
    skipped: !1
  }
}

function analyzeRareEventEngine(e, t, n, a, s) {
  const i = buildBinary2x2Tables(e);
  if (0 === i.length) return {
    available: !1,
    skipped: !0,
    reason: "No binary 2x2 data"
  };
  const r = i.length,
    o = sum(i.map(e => e.n1)),
    l = sum(i.map(e => e.n2)),
    ...
    u = o + l,
    p = d + c,
    m = r > 0 ? i.filter(e => 0 === e.a && 0 === e.c).length : 0,
    h = r > 0 ? i.filter(e => 0 === e.a || 0 === e.b || 0 === e.c || 0 === e.d).length : 0,
    v = Math.max(0, h - m),
    g = u > 0 ? p / u : NaN,
    f = o > 0 ? d / o : NaN,
    _ = l > 0 ? c / l : NaN,
    y = i.reduce((e, t) => {
      const n = t.n2 > 0 ? t.n1 / t.n2 : Infinity,
        a = t.n1 > 0 ? t.n2 / t.n1 : Infinity;
      return Math.max(e, n, a)
    }, 1),
    b = isFinite(g) && g < .01,
    x = isFinite(g) && g < .05,
    w = isFinite(n) ? Math.exp(n) : NaN,
    M = isFinite(w) && (w < .67 || w > 1.5);
  let S = "RE-IV",
    E = "moderate";
  const A = [];
  "RD" === t ? (S = "MH-RD", E = "high", A.push("Risk-difference analyses are best stabilized with Mantel-Haenszel weighting.")) : b && !M && y <= 3 && s && !0 === s.available ? (S = "PETO", E = "high", A.push("Ultra-rare event setting with near-null effects favors Peto OR.")) : (x || h > 0) && a && !0 === a.available ? (S = "MH", E = "high", A.push("Sparse events and zero cells favor Mantel-Haenszel over inverse-variance RE.")) : (S = "RE-IV", E = "moderate", A.push("Event rates and sparsity do not force rare-event corrections."));
  return m > 0 && A.push("Double-zero studies detected; report sensitivity with and without continuity correction."), y > 4 && A.push("Large arm-size imbalance can bias Peto; prefer MH/RE sensitivity checks."), M && A.push("Large effect magnitude detected; Peto can be biased away from null."), {
    available: !0,
    skipped: !1,
    k_binary: r,
    total_n: u,
    total_events: p,
    event_rate_overall: g,
    event_rate_treatment: f,
    event_rate_control: _,
    zero_cells_any: h,
    zero_cells_single: v,
    zero_cells_double: m,
    zero_cell_fraction: r > 0 ? h / r : 0,
    max_arm_ratio: y,
    rare: x,
    ultra_rare: b,
    large_effect: M,
...
function computeRobustnessVerdict(e) {
  if (!e || !e.pooled) return {
    available: !1,
    skipped: !0,
    reason: "Missing pooled analysis"
  };
  const t = e.k || (Array.isArray(e.yi) ? e.yi.length : 0),
    n = e.het && isFinite(e.het.I2) ? e.het.I2 : NaN,
    a = e.pairwiseConsensus && e.pairwiseConsensus.robustness ? e.pairwiseConsensus.robustness : null,
    s = e.pi && e.pi.ensemble && !e.pi.ensemble.skipped ? e.pi.ensemble : e.pi && e.pi.noma ? e.pi.noma : e.pi ? e.pi.standard : null,
    i = !!(s && isFinite(s.lower) && isFinite(s.upper) && s.lower <= 0 && s.upper >= 0),
    r = [];
  let o = 0;
  t < 5 ? (o += 2, r.push("Very small evidence base (k < 5).")) : t < 10 && (o += 1, r.push("Small evidence base (k < 10).")), isFinite(n) && (n >= 75 ? (o += 2, r.push("High heterogeneity (I2 >= 75%).")) : n >= 50 && (o += 1, r.push("Moderate heterogeneity (I2 >= 50%)."))), a && (!a.direction_stable && (o += 2, r.push("Effect direction is not stable across tau2 estimators.")), a.direction_stable && !a.significance_stable && (o += 1, r.push("Significance changes across estimators.")), !a.tau2_stable && (o += 1, r.push("tau2 is unstable across estimators."))), i && (o += 1, r.push("Prediction interval crosses the null effect.")), e.rareEventEngine && e.rareEventEngine.available && e.rareEventEngine.rare && (o += 1, r.push("Rare-event regime detected; method choice sensitivity is likely."));
  let l = "High robustness",
    d = "high",
    c = "low";
  return o >= 5 ? (l = "Fragile", d = "low", c = "high") : o >= 3 && (l = "Moderate robustness", d = "moderate", c = "moderate"), r.length || r.push("No major instability signals detected."), {
    available: !0,
    skipped: !1,
    verdict: l,
    confidence: d,
    ...
    I2: n,
    crosses_null: i,
    pi_method: s && s.method ? s.method : "none",
    reasons: r
  }
}
...
function calculateHKSJ(e, t, n, a, s = .05) {
  ...
  return {
    ...
    q_raw: l,
    q_bounded: d,
    warning: d ? "HKSJ q < 1 (bounded to 1): suggests possible model misspecification or Ãâ€žÃ‚Â² overestimation" : null
  }
}
...
function computeMCMCDiagnostics(e) {
  ...
  const i = a("mu"),
    ...
    c = s(l);
  let u = null;
  return i > 1.1 || r > 1.1 ? u = "Chains may not have converged (RÃŒâ€š > 1.1). Consider increasing iterations or burn-in." : (d < 100 || c < 100) && (u = "Effective sample size is low. Consider increasing iterations."), {
    Rhat: {
      mu: i,
      tau2: r
    },
...
function validateCI(e, t, n) {
  return null === e || null === n || isNaN(e) || isNaN(n) ? {
    ...
    valid: !1,
    reason: "Lower CI Ã¢â€°Â¤ 0 but point estimate > 0"
  } : {
    valid: !0
  }
}
...
function mapOutcomeToStandard(e) {
  const t = e.toLowerCase().trim().replace(/[Ã¢â‚¬ÂÃ¢â‚¬â€œÃ¢â‚¬â€]/g, "-").replace(/\s+/g, " ");
...
const FOLLOW_UP_PATTERNS = [{
  regex: /(\d+(?:\.\d+)?)\s*[-Ã¢â‚¬â€œ]?\s*months?\s*(?:follow[- ]?up)?/i,
  unit: 1
}, {
  regex: /(\d+(?:\.\d+)?)\s*[-Ã¢â‚¬â€œ]?\s*years?\s*(?:follow[- ]?up)?/i,
  unit: 12
}, {
  ...
  regex: /at\s*(\d+(?:\.\d+)?)\s*[-Ã¢â‚¬â€œ]?\s*months?/i,
  unit: 1
}, {
  regex: /at\s*(\d+(?:\.\d+)?)\s*[-Ã¢â‚¬â€œ]?\s*years?/i,
  unit: 12
}, {
  regex: /(\d+(?:\.\d+)?)\s*[-Ã¢â‚¬â€œ]?\s*year/i,
  unit: 12
}, {
  regex: /^(\d+(?:\.\d+)?)$/,
  unit: "auto"
...
const BASELINE_COVARIATE_MAP = {
  ...
  age_gte_65: ["age Ã¢â€°Â¥65", "age >=65", "age 65+", "elderly"],
  age_gte_75: ["age Ã¢â€°Â¥75", "age >=75", "age 75+"],
  ...
  lvef_gte_40: ["lvef Ã¢â€°Â¥40", "lvef >= 40", "hfpef", "hfmref", "preserved ef"],
  ...
  bb_pct: ["beta blocker", "bb", "ÃŽÂ²-blocker"],
...
function loadMultiOutcomeData() {
  ...
  if (e && e.value.trim()) try {
    ...
    let o = `<span class="text-success">Ã¢Å“â€œ Loaded ${s.length} trials, ${r.size} unique outcomes</span>`;
    if (i.length > 0) {
      const e = i.map(e => sanitizeHTML(e)).join(", ");
      o += `<br><span class="text-warning text-xs">${e}</span>`
    }
    t.innerHTML = o
  } catch (e) {
    t.innerHTML = `<span class="text-error">JSON parse error: ${sanitizeHTML(e.message)}</span>`
...
function populateMultiOutcomeDropdowns(e) {
  ...
  const s = document.getElementById("covOutcomeSelect"),
    ...
    r = [...n].sort().map(e => `<option value="${e}">${"ratio"===getOutcomeMetadata(e).type?"Ã°Å¸â€œÅ ":"Ã°Å¸â€œË†"} ${e}</option>`).join("");
...
function populateOutcomeDictionary() {
  ...
  for (const [e, n] of Object.entries(OUTCOME_DICTIONARY)) {
    ...
    t += `<li style="margin-bottom: 4px;"><strong>${"ratio"===a.type?"Ã°Å¸â€œÅ ":"Ã°Å¸â€œË†"} ${e}</strong> ${"lower"===a.direction?"Ã¢â€ â€œ":"Ã¢â€ â€˜"}: ${n.slice(0,3).join(", ")}${n.length>3?"...":""}</li>`
  }
  t += "</ul>", e.innerHTML = t
}
...
function renderMultiOutcomeResults(e) {
  ...
  if (!t) return;
  let n = `\n        <div class="card">\n          <div class="card__header">\n            <h3 class="card__title">Ã°Å¸Å½Â¯ Joint DDMA Results (${e.n_outcomes} Outcomes, ${e.n_trials} Trials)</h3>\n          </div>\n          <div class="card__body" style="padding: 0;">\n            <table class="data-table">\n              <thead>\n                <tr>\n                  <th>Outcome</th>\n                  <th>Type</th>\n                  <th>k</th>\n                  <th>Effect</th>\n                  <th>95% CI</th>\n                  <th>P(Benefit)</th>\n                  <th>IÃ‚Â²</th>\n                </tr>\n              </thead>\n              <tbody>\n      `;
  for (const [t, a] of Object.entries(e.outcomes)) {
    ...
    let i, r;
    "ratio" === a.outcomeType ? (i = a.pooled && void 0 !== a.pooled.effect_exp ? a.pooled.effect_exp.toFixed(3) : "N/A", r = a.pooled && void 0 !== a.pooled.ci_lower && void 0 !== a.pooled.ci_upper ? `${a.pooled.ci_lower.toFixed(2)} Ã¢â‚¬â€œ ${a.pooled.ci_upper.toFixed(2)}` : "N/A") : (i = a.pooled && void 0 !== a.pooled.effect ? a.pooled.effect.toFixed(2) : "N/A", r = a.pooled && void 0 !== a.pooled.ci_lower && void 0 !== a.pooled.ci_upper ? `${a.pooled.ci_lower.toFixed(2)} Ã¢â‚¬â€œ ${a.pooled.ci_upper.toFixed(2)}` : "N/A");
    const o = "ratio" === a.outcomeType ? "HR/OR" : "MD",
      l = "lower" === a.direction ? "Ã¢â€ â€œ" : "Ã¢â€ â€˜";
...
function calculateI2Metafor(e, t, n = null) {
  const a = null == n ? t.length : n;
  if (!isFinite(e) || e <= 0 || a < 2 || !Array.isArray(t) || 0 === t.length) return 0;
  const s = t.map(e => 1 / e).filter(e => isFinite(e) && e > 0);
...
function calculateHeterogeneity(e, t, n) {
  ...
  return {
    ...
    tau2: n,
    tau: h,
    interpretation: interpretI2(u),
    I2_method: "tau2_vtilde"
  }
}

function interpretI2(e) {
  return e < 25 ? "Low (but see Ãâ€ž for clinical relevance)" : e < 50 ? "Low-Moderate" : e < 75 ? "Moderate-High" : "High (investigate sources)"
}
...
function calculateICC_MA(e, t, n, a = null, s = "OR", i = null) {
  ...
  if (i && ("OR" === s || "RR" === s) && i.every(e => void 0 !== e.events_t && e.n_t && e.n_c)) {
    o = harmonicMean(i.map(e => 1 / (e.events_t + .5) + 1 / (e.n_t - e.events_t + .5) + 1 / (e.events_c + .5) + 1 / (e.n_c - e.events_c + .5))), r = a ? harmonicMean(a) : 4 / o, d = "exact", l = "Exact calculation from 2Ãƒâ€”2 data (Adapted from ICC methodology)"
  } else if (a && a.length === c && a.every(e => e > 0)) {
    switch (r = harmonicMean(a), s) {
      case "OR":
      case "RR":
        o = 4 / r, l = "Approximation: ÃÆ’ÃŒÆ’Ã‚Â² = 4/ÃƒÂ± (assumes balanced groups, ~50% event rate)";
        break;
      case "SMD":
...
function predictionInterval_Standard(e, t, n, a, s = .05, i = "k-2") {
  ...
  switch (i) {
    case "k-1":
      r = Math.max(1, a - 1), o = "df = k-1 (only ÃŽÂ¸ estimated)";
      break;
    case "conservative":
...
function predictionInterval_Conformal(e, t, n, a = .05, s = "REML") {
  ...
  if (i < CONFIG.MIN_STUDIES_FOR_CONFORMAL) return {
    ...
    warning: `Requires k Ã¢â€°Â¥ ${CONFIG.MIN_STUDIES_FOR_CONFORMAL}`,
    valid: !1
  };
  const r = [];
  for (let n = 0; n < i; n++) {
  ...
  return {
    ...
    quantile: l,
    method: "Conformal (Jackknife+)",
    note: "Distribution-free PI. Jackknife+ guarantees Ã¢â€°Â¥" + (100 * (1 - 2 * a)).toFixed(0) + "% coverage (Lei et al., 2018)",
...
function calculateDDMA(e, t, n, a, s = {}) {
  ...
  return "lower" === r ? (p = pnorm((0 - e) / l), m = pnorm((0 - e) / d), h = pnorm((-i - e) / l), v = pnorm((-i - e) / d), g = pnorm((-.25 - e) / l), f = pnorm((-.25 - e) / d), _ = pnorm((c - e) / l) - pnorm((-.05 - e) / l), y = pnorm((c - e) / d) - pnorm((-.05 - e) / d)) : (p = 1 - pnorm((0 - e) / l), m = 1 - pnorm((0 - e) / d), h = 1 - pnorm((i - e) / l), v = 1 - pnorm((i - e) / d), g = 1 - pnorm((u - e) / l), f = 1 - pnorm((u - e) / d), _ = pnorm((c - e) / l) - pnorm((-.05 - e) / l), y = pnorm((c - e) / d) - pnorm((-.05 - e) / d)), {
    confidence: {
      ...
      P_large_benefit: g,
      P_trivial: _,
      interpretation: "Based on estimation uncertainty only (SE of ÃŽÂ¸ÃŒâ€š)"
    },
    predictive: {
      P_benefit: m,
      P_harm: 1 - m,
      ...
      P_large_benefit: f,
      P_trivial: y,
      interpretation: "Includes heterogeneity (Ãâ€žÃ‚Â²) for new-setting prediction"
    },
    effect: e,
    se_confidence: l,
    se_predictive: d,
    ...
    interpretation_note: {
      ...
      bayesian_flat_prior: "Equivalent to P(ÃŽÂ¸ Ã¢Ë†Ë† region | data) under improper uniform prior",
...
function calculateLaEV(e, t, n, a = {}) {
  ...
  return {
    ...
    lambda: s,
    lambda_source: o,
    lambda_note: `ÃŽÂ»=${s} from ${o}. Medical decisions may warrant ÃŽÂ»=3-5 for irreversible harms.`,
...
function eggersTest(e, t, n = null, a = "binary") {
  const s = e.length,
    i = [];
  s < 10 ? i.push("Very low power with k=" + s + " (recommend kÃ¢â€°Â¥10 for reliable results)") : s < 20 && i.push("Limited power with k=" + s + " (optimal kÃ¢â€°Â¥20)"), "binary" === a && i.push("For binary outcomes, consider Peters' test which is less affected by effect-precision correlation");
  const r = t.map(e => 1 / e),
    o = e.map((e, n) => e / t[n]),
    l = r.map(e => e * e),
    d = sum(l),
...
function metaRegression(e, t, n, a = "Covariate", s = "REML") {
  ...
  return {
    ...
    R2: B,
    R2_truncated: H,
    R2_note: H ? "RÃ‚Â² was negative (covariate did not reduce heterogeneity), truncated to 0" : null,
...
function calculateNNT_withCI(e, t, n, a = "OR") {
  ...
  if (t.lower < 1 && t.upper > 1) return {
    ...
    ci_interpretation: "NNT range: " + Math.ceil(i.NNT_exact) + " (NNT) to Ã¢Ë†Å¾ to " + Math.ceil(r.NNT_exact) + " (NNH)"
  };
  let o, l;
...
function predictionInterval_Noma(e, t, n, a, s, i = .05) {
  ...
  return {
    ...
    df: r,
    method: "Noma2023",
    note: "Uses conservative df and accounts for Ãâ€žÃ‚Â² uncertainty"
  }
}
...
function assessMetaoverfit(e, t = 2, n, a, s) {
  const i = e / t;
  let r, o, l;
  i < CONFIG.METAOVERFIT_SEVERE ? (r = "SEVERE", o = `Only ${e} studies for ${t} parameters (ratio ${i.toFixed(1)}). Ãâ€žÃ‚Â² estimation highly unstable; standard errors likely too narrow.`, l = "Consider fixed-effect model, use HKSJ adjustment, or report as exploratory only.") : i < CONFIG.METAOVERFIT_HIGH ? (r = "HIGH", o = `${e} studies for ${t} parameters (ratio ${i.toFixed(1)}). Coverage probability may be below nominal; Ãâ€žÃ‚Â² may be biased.`, l = "Use HKSJ adjustment; report sensitivity to Ãâ€žÃ‚Â² estimator choice; interpret with caution.") : i < CONFIG.METAOVERFIT_MODERATE ? (r = "MODERATE", o = `${e} studies for ${t} parameters (ratio ${i.toFixed(1)}). Adequate but not ideal; some instability possible with different Ãâ€žÃ‚Â² estimators.`, l = "Standard methods appropriate; consider reporting Q-profile CI for Ãâ€žÃ‚Â².") : (r = "LOW", o = `${e} studies for ${t} parameters (ratio ${i.toFixed(1)}). Sufficient for stable estimation.`, l = "Standard interpretation applies.");
...
function calculateCV_I2(e, t, n = "REML") {
  ...
  return {
    ...
    loo_results: s,
    interpretation: `IÃ‚Â² = ${i.I2.toFixed(1)}% (CV: ${r.toFixed(1)}%, range: ${l.toFixed(1)}pp) - ${c}`
  }
}
...
function petPeese(e, t) {
  ...
  return {
    ...
    caveats: ["Assumes linear relationship between effect and SE (PET) or SEÃ‚Â² (PEESE)", "May overcorrect when true heterogeneity exists", "Works best with >10 studies"],
...
function linChuSkewnessTest(e, t, n = 5e3) {
  ...
  return {
    ...
    n_resample: n,
    k: a,
    interpretation: w < .1 ? `Significant skewness detected (ÃŽÂ³=${b.toFixed(3)}, p=${w.toFixed(4)}). Suggests publication bias.` : `No significant skewness (ÃŽÂ³=${b.toFixed(3)}, p=${w.toFixed(4)}).`,
...
function calculatePeto_OR(e) {
  ...
  return (r < .5 || r > 2) && (l = "Peto's method may be biased for large effects (OR far from 1). Consider MH or inverse-variance."), {
    ...
    sum_V: a,
    warning: l,
    note: "Best for rare events with OR Ã¢â€°Ë† 1. Biased for large effects.",
...
function interpretHeterogeneity(e) {
  ...
  if (n && n.upper - n.lower > 50 && d.push({
      type: "uncertainty",
      text: `IÃ‚Â² = ${t.toFixed(0)}% but 95% CI is very wide (${n.lower.toFixed(0)}Ã¢â‚¬â€œ${n.upper.toFixed(0)}%). The true proportion of variance due to heterogeneity is highly uncertain.`
    }), void 0 !== a) {
  ...
  if (o) {
    ...
    "OR" !== l && "RR" !== l && "HR" !== l || (e < 0 && t > 0 ? c.push({
      type: "direction",
      text: "Ã¢Å¡Â Ã¯Â¸Â Prediction interval crosses null: a new study in a similar setting could plausibly show either benefit OR harm. Effect direction is uncertain."
    }) : d.push({
      type: "direction",
  ...
  u > 3 && c.push({
    ...
    type: "sample",
    text: `With only ${r+1} studies, IÃ‚Â² has limited precision. Focus on Ãâ€ž (absolute heterogeneity) rather than IÃ‚Â² (relative).`
  });
  return {
    I2: t,
    I2_ci: n,
    ...
    messages: d,
    warnings: c,
    threshold_note: "Note: IÃ‚Â² thresholds (25/50/75%) are arbitrary. Clinical importance depends on Ãâ€ž magnitude and decision context.",
    ...
    references: ["Higgins JPT, Thompson SG. Stat Med 2002;21:1539-1558", "RÃƒÂ¼cker G, et al. BMC Med Res Methodol 2008;8:79", "IntHout J, et al. BMJ Open 2016;6:e010247"]
  }
}
  ...
  switch (t) {
    ...
    case "HR":
      const t = Math.exp(e),
        n = Math.exp(-1.96 * e),
        a = Math.exp(1.96 * e);
      return `Ãâ€ž = ${e.toFixed(3)} on log scale. True effects vary by a factor of ~${t.toFixed(2)}Ãƒâ€” between studies. 95% of true effects fall between ${n.toFixed(2)}Ãƒâ€” and ${a.toFixed(2)}Ãƒâ€” the average.`;
    case "SMD":
      return `Ãâ€ž = ${e.toFixed(3)} standardized units. True standardized effects vary by ~${(2*e).toFixed(2)} SD between studies (Ã‚Â±1 SD from mean).`;
    case "MD":
      return `Ãâ€ž = ${e.toFixed(3)} in original units. True effects vary by ~${(2*e).toFixed(3)} units between studies (Ã‚Â±1 SD from mean).`;
    case "RD":
      return `Ãâ€ž = ${(100*e).toFixed(1)} percentage points. True risk differences vary by ~${(200*e).toFixed(1)} pp between studies.`;
    default:
      return `Ãâ€ž = ${e.toFixed(4)} (SD of true effects across studies)`
  }
}
...
const BENCHMARK_DATASETS = {
    ...
    HOMOGENEOUS_MA: {
      name: "Homogeneous MA (Ãâ€žÃ‚Â²=0)",
...
function runValidationBenchmarks(e = !1) {
  const t = [];
...
function runValidation(e = !1) {
  return "interactive" === e || "ui" === e ? runValidationInteractive() : runValidationBenchmarks(e)
}
...
function calculateLnCVR(e) {
  ...
  if (t.length < 2) return {
    available: !1,
    reason: "Requires continuous outcomes with mean, SD, n for both arms AND positive means (CV undefined for mean Ã¢â€°Â¤ 0)",
...
function lowI2Diagnostic(e, t, n) {
  ...
  let m, h;
  return n < 25 ? o < .5 ? (m = "UNDERPOWERED", h = "Low - cannot reliably detect moderate heterogeneity") : c > 10 ? (m = "MISLEADING", h = "Low IÃ‚Â² but high pairwise tension suggests hidden heterogeneity") : (m = "LIKELY HOMOGENEOUS", h = "Adequate power and low tension support genuine homogeneity") : (m = "HETEROGENEITY DETECTED", h = "IÃ‚Â² indicates meaningful between-study variation"), {
    available: !0,
    I2_observed: n,
    Q_statistic: p.Q,
    Q_pvalue: p.p_Q,
...
function treatmentResponseDistribution(e, t, n = .15, a = "lower") {
  ...
  return {
    ...
    population_distribution: {
      ...
      sd_theta: i,
      variance_source: "Ãâ€žÃ‚Â² only (within-patient variability not estimable from aggregate data)"
    },
    response_rates: {
      P_benefit: r,
      P_mcid: l,
    ...
    clinical_summary: {
      ...
      nnt_standard: isFinite(m) ? m.toFixed(1) : "Ã¢Ë†Å¾",
      nnt_responder_adjusted: isFinite(h) ? h.toFixed(1) : "Ã¢Ë†Å¾"
    },
    ...
    limitations: ["Cannot estimate within-patient variability from aggregate data", "Ãâ€žÃ‚Â² reflects between-study, not between-patient heterogeneity", "True patient-level response distribution requires IPD meta-analysis"],
...
function heteroscedasticityTest(e, t, n) {
  ...
  return f > 3 && b < .1 ? (v = !0, g = "Strong evidence of heteroscedasticity - Ãâ€žÃ‚Â² varies substantially across subgroups") : f > 2 || f > 1.5 && b < .1 ? (v = "possible", g = "Moderate variation in Ãâ€žÃ‚Â² across subgroups - interpret pooled Ãâ€žÃ‚Â² cautiously") : (v = !1, g = "Ãâ€žÃ‚Â² appears relatively constant across subgroups"), {
    available: !0,
    n_groups: a.length,
    group_stats: s,
    tau2_ratio: f,
    ...
    heteroscedastic: v,
    interpretation: g,
    recommendation: !0 === v ? "Consider separate analyses by subgroup or location-scale modeling (rma() with scale= in metafor)" : "possible" === v ? "Sensitivity analysis with subgroup-specific Ãâ€žÃ‚Â² recommended" : "Pooled analysis with single Ãâ€žÃ‚Â² appears appropriate",
    reference: "Viechtbauer W, LÃƒÂ³pez-LÃƒÂ³pez JA. Res Synth Methods 2022;13:314-332"
  }
}
...
function copasSelectionSensitivityBounds(e, t) {
  const n = e.length,
    a = t.map(e => Math.sqrt(e)),
    s = estimateTau2_REML(e, t),
    i = pooledEstimate_RE(e, t, s.tau2),
  ...
  if (0 === r.length) return {
    ...
    robustness: "Insufficient variation in selectable studies for Copas sensitivity contour",
    interpretation: "Unable to estimate Copas sensitivity bounds with current data"
  };
  ...
  return {
    ...
    interpretation: `Under various selection scenarios, adjusted ÃŽÂ¸ ranges from ${c.toFixed(3)} to ${u.toFixed(3)}`
  }
}
const Matrix = {
  identity: e => {
...
const DEMO_DATASETS = {
  ...
  PSYCH_INTERVENTION: {
    ...
    teaching_point: "RR Ã¢â€°Ë† 1.0 - DDMA correctly shows ~50% P(benefit), ~50% P(harm)",
    studies: [{
      name: "Trial A",
      events_t: 45,
      n_t: 500,
  ...
  BCG: {
    ...
    category: "benchmark",
    teaching_point: "Classic benchmark dataset with high heterogeneity (IÃ‚Â² > 90%)",
    studies: [{
      name: "Aronson 1948",
      events_t: 4,
      n_t: 123,
  ...
  // Note: DTA (Diagnostic Test Accuracy) is handled by DTA Pro app
  // This app focuses on pairwise treatment effect meta-analysis
};
...
function runFullAnalysis() {
  ...
  t && (t.disabled = !0, t.innerHTML = '<span class="btn-spinner">Ã¢Å¸Â³</span> Computing...'), setTimeout(() => {
    try {
      const t = e.yi,
        n = e.vi,
      ...
      t && (t.disabled = !1, t.innerHTML = "Ã°Å¸â€Â¬ Run Full Analysis")
    }
  }, 50)
}
...
function onTabActivate(e) {
  ...
  if ("advanced" === e) try {
    const e = document.getElementById("advancedMethodsAlert");
    if (e) {
      const n = e.querySelector(".alert__text");
      n && (n.textContent = "Advanced methods ready. Click any method card to run analyses.")
    }
    "function" == typeof ensureAdvancedGapCards && ensureAdvancedGapCards(),
    "function" == typeof autoRenderPowerAnalysis && autoRenderPowerAnalysis()
  } catch (e) {
    console.error("Deferred advanced failed:", e)
  }
  if ("ddma" === e) try {
    safeRender(renderDDMAPanel, "panel-ddma", t)
  } catch (e) {
    console.error("Deferred DDMA failed:", e)
  }
  if ("clinical" === e) try {
    (!Array.isArray(t.nnt_range) || 0 === t.nnt_range.length || t.nnt_range?.skipped) && (t.nnt_range = computeNntRangeFromPooled(t.pooled, t.measure, t.direction || AppState.settings.direction)), safeRender(renderClinicalPanel, "panel-clinical", t)
  } catch (e) {
    console.error("Deferred clinical failed:", e)
  }
  if ("crossdisciplinary" === e) try {
    (!t.crossDisc || 0 === Object.keys(t.crossDisc).length || t.crossDisc.error) && (t.crossDisc = runCrossDisciplinaryAnalyses(t, t.studies || [])), safeRender(renderCrossDisciplinaryPanel, "panel-crossdisciplinary", t)
  } catch (e) {
    console.error("Deferred cross-disciplinary failed:", e)
  }
  if ("code" === e) try {
    safeRender(renderCodePanel, "panel-code", t)
  } catch (e) {
    console.error("Deferred code export failed:", e)
  }
  if ("report" === e) try {
    safeRender(renderReportPanel, "panel-report", t)
  } catch (e) {
    console.error("Deferred report failed:", e)
  }
}
...
function runAnalysis() {
  const e = document.getElementById("runAnalysisBtn"),
    t = e.innerHTML;
  e.classList.add("btn--loading"), e.innerHTML = '<span class="btn-spinner">Ã¢Å¸Â³</span> Running...', setTimeout(() => {
    try {
      ...
      const h = calculatePooledEstimate(r, o, p),
        feModel = calculatePooledEstimate(r, o, 0),
        ...
        x = predictionInterval_Noma(h.theta, h.se, p, m, c),
        w = {
          lower: b.lower,
          upper: b.upper,
          skipped: !0
      ...
      const F = computeNntRangeFromPooled(h, s, AppState.settings.direction);
      let L = estimateTau2_All(r, o),
        eeEstimatorConsensus = {
          available: !1,
          skipped: !0,
          tau2_all: L
        };
      try {
        eeEstimatorConsensus = computePairwiseEstimatorConsensus(r, o, AppState.settings.tau2Method), eeEstimatorConsensus && eeEstimatorConsensus.tau2_all && (L = eeEstimatorConsensus.tau2_all)
      } catch (e) {
        eeEstimatorConsensus = {
          available: !1,
          skipped: !0,
      ...
      const teEstimatorSpread = eeEstimatorConsensus && eeEstimatorConsensus.sensitivity && isFinite(eeEstimatorConsensus.sensitivity.tau2_range) ? eeEstimatorConsensus.sensitivity.tau2_range : null,
        neTau2Stable = eeEstimatorConsensus && eeEstimatorConsensus.robustness ? !!eeEstimatorConsensus.robustness.tau2_stable : null;
      let aePiEnsemble = {
        skipped: !0
      };
      try {
        aePiEnsemble = computeEstimatorEnsemblePI(eeEstimatorConsensus, h, c)
      } catch (e) {
        aePiEnsemble = {
          skipped: !0,
          error: e.message
      ...
      let O = {
        ...
        Y = null;
      const ceBinaryTables = buildBinary2x2Tables(i);
      let ceRareEventEngine = {
        available: !1,
        skipped: !0
      };
      if ("binary" === AppState.settings.dataType && ceBinaryTables.length > 0) {
        try {
          const e = "RR" === s ? "RR" : "RD" === s ? "RD" : "OR",
            t = mantelHaenszel(ceBinaryTables, e);
          H = t && !t.error && isFinite(t.estimate) && isFinite(t.se) ? {
            ...t,
            available: !0,
            skipped: !1
          } : {
            available: !1,
            skipped: !0,
            reason: t && t.error ? t.error : "Mantel-Haenszel unavailable"
          }
        } catch (e) {
          H = {
            available: !1,
        ...
        try {
          const e = petoMethod(ceBinaryTables),
            t = e && isFinite(e.estimate) && isFinite(e.se) && isFinite(e.ci_lower) && isFinite(e.ci_upper);
          q = t ? {
            ...e,
            available: !0,
            skipped: !1,
            note: "OR" === s ? e.note : "Computed on OR scale for rare-event sensitivity."
          } : {
            available: !1,
            skipped: !0,
            reason: e && e.error ? e.error : "Peto unavailable"
          }
        } catch (e) {
          q = {
            available: !1,
      ...
      try {
        ceRareEventEngine = analyzeRareEventEngine(i, s, h.theta, H, q)
      } catch (e) {
        ceRareEventEngine = {
          available: !1,
          skipped: !0,
      ...
      let K = {
        ...
        Z = {};
      try {
        Z = runCrossDisciplinaryAnalyses({
          yi: r,
          vi: o,
          het: g,
          pooled: h,
      ...
      AppState.results = {
        ...
        mh_result: H,
        peto_result: q,
        rareEventEngine: ceRareEventEngine,
        permutation: j,
        bcaBootstrap: U,
        fragility: G,
        linChuSkewness: W,
        ...
        nnt_range: F,
        tau2_all: L,
        pairwiseConsensus: eeEstimatorConsensus,
        estimatorSpread: teEstimatorSpread,
        tau2Stable: neTau2Stable,
        bayesian: Y,
        validation: K,
        crossDisc: Z,
        cumulative: ee,
      ...
      try {
        AppState.results.robustnessVerdict = computeRobustnessVerdict(AppState.results)
      } catch (e) {
...
function formatEffect(e, t = "OR") {
  if (null == e || !isFinite(e) || isNaN(e)) return "Ã¢â‚¬â€";
  switch (t) {
    case "RD":
...
function isRatioBasedMeasure(e) {
  const t = String(e || "").toUpperCase();
  return !["RD", "MD", "SMD", "SMDH", "SMCC", "SMCR", "SMCRH", "PLO", "PAS", "PFT", "PR", "GEN"].includes(t)
}

function computeNntRangeFromPooled(e, t, n = "lower") {
  if (!e || !isFinite(e.theta)) return [];
  const a = String(t || "").toUpperCase();
  if (["OR", "RR", "HR"].includes(a)) {
    const t = Math.exp(e.theta),
      n = isFinite(e.ci_lower) && isFinite(e.ci_upper) ? {
        lower: Math.exp(e.ci_lower),
        upper: Math.exp(e.ci_upper)
      } : null,
      s = "OR" === a ? "OR" : "RR";
    return calculateNNT_range(t, s, n)
  }
  if ("RD" === a) {
    const t = [.01, .05, .1, .2, .3, .5];
    return t.map(t => {
      const a = "lower" === n ? -e.theta : e.theta,
        s = Math.max(0, Math.min(1, t - a)),
        i = t - s,
        r = Math.abs(i) > 1e-10 ? Math.ceil(1 / Math.abs(i)) : 1 / 0;
      return {
        baseline_risk: t,
        baseline_risk_pct: (100 * t).toFixed(0),
        treatment_risk: s,
        ARR: i,
        ARR_pct: 100 * i,
        NNT: r,
        NNT_exact: Math.abs(i) > 1e-10 ? Math.abs(1 / i) : 1 / 0,
        type: i > 0 ? "NNT" : i < 0 ? "NNH" : "None"
      }
    })
  }
  return []
...
function renderPairwisePoolingSummaryCard(e, t) {
  try {
    if (!e || !t || !t.pooled) return;
    const n = e.querySelector("#pairwisePoolingSummaryCard");
    n && n.remove();
    const a = String(t.measure || "OR").toUpperCase(),
      s = e => isFinite(e) ? e < .001 ? "<0.001" : e.toFixed(4) : "N/A",
      i = (e, t = a) => "RD" === t ? isFinite(e) ? `${(100*e).toFixed(2)}%` : "N/A" : isFinite(e) ? e.toFixed(3) : "N/A",
      r = (e, t = a) => `${formatEffect(e.lower,t)} - ${formatEffect(e.upper,t)}`,
      o = t.fixed && isFinite(t.fixed.theta) ? t.fixed : Array.isArray(t.yi) && Array.isArray(t.vi) ? calculatePooledEstimate(t.yi, t.vi, 0) : null,
      l = [];
    l.push({
      model: `Random-effects (${t.tau2Result?.method||"RE"})`,
      estimate: formatEffect(t.pooled.theta, a),
      ci: r({
        lower: t.pooled.ci_lower,
        upper: t.pooled.ci_upper
      }, a),
      tau2: isFinite(t.tau2) ? t.tau2.toFixed(4) : "N/A",
      pval: s(t.pooled.p_value),
      note: "Primary"
    }), o && l.push({
      model: "Fixed-effect (tau2=0)",
      estimate: formatEffect(o.theta, a),
      ci: r({
        lower: o.ci_lower,
        upper: o.ci_upper
      }, a),
      ...
      pval: s(o.p_value),
      note: "Sensitivity"
    }), t.hksj && isFinite(t.hksj.ci_lower) && isFinite(t.hksj.ci_upper) && l.push({
      model: "Random-effects (HKSJ CI)",
      estimate: formatEffect(t.pooled.theta, a),
      ci: r({
        lower: t.hksj.ci_lower,
        upper: t.hksj.ci_upper
      }, a),
      tau2: isFinite(t.tau2) ? t.tau2.toFixed(4) : "N/A",
      pval: s(t.pooled.p_value),
      note: "Small-sample adjusted"
    }), t.pairwiseConsensus && t.pairwiseConsensus.consensus && isFinite(t.pairwiseConsensus.consensus.theta) && l.push({
      model: "AICc-weighted estimator ensemble",
      estimate: formatEffect(t.pairwiseConsensus.consensus.theta, a),
      ci: r({
        lower: t.pairwiseConsensus.consensus.ci_lower,
        upper: t.pairwiseConsensus.consensus.ci_upper
      }, a),
      tau2: isFinite(t.pairwiseConsensus.consensus.tau2) ? t.pairwiseConsensus.consensus.tau2.toFixed(4) : "N/A",
      pval: s(t.pairwiseConsensus.consensus.p_value),
      note: "Model-averaged"
    }), t.mh_result && !1 !== t.mh_result.available && isFinite(t.mh_result.estimate) && l.push({
      model: `Mantel-Haenszel (${t.mh_result.measure||a})`,
      estimate: i(t.mh_result.estimate, t.mh_result.measure || a),
      ci: `${i(t.mh_result.ci_lower,t.mh_result.measure||a)} - ${i(t.mh_result.ci_upper,t.mh_result.measure||a)}`,
      tau2: "N/A",
      pval: s(t.mh_result.pvalue),
      note: "Rare-event check"
    }), t.peto_result && !1 !== t.peto_result.available && isFinite(t.peto_result.estimate) && l.push({
      model: "Peto OR",
      estimate: i(t.peto_result.estimate, "OR"),
      ci: `${i(t.peto_result.ci_lower,"OR")} - ${i(t.peto_result.ci_upper,"OR")}`,
      tau2: "N/A",
      pval: s(t.peto_result.pvalue),
      note: "Ultra-rare sensitivity"
    });
    if (t.tau2_all && Array.isArray(t.yi) && Array.isArray(t.vi)) {
      Object.entries(t.tau2_all).forEach(([e, n]) => {
        if (!n || !isFinite(n.tau2)) return;
        try {
          if (t.tau2Result && t.tau2Result.method === e) return;
          const o = calculatePooledEstimate(t.yi, t.vi, n.tau2),
            d = "Tau2 estimator";
          l.push({
            model: `RE (${e})`,
            estimate: formatEffect(o.theta, a),
    ...
    const d = Array.isArray(t.nnt_range) ? t.nnt_range.find(e => Number(e.baseline_risk_pct) === 10) || t.nnt_range.find(e => Math.abs((e.baseline_risk || 0) - .1) < 1e-6) : null,
      c = isRatioBasedMeasure(a) && isFinite(t.pooled.theta) ? (Math.exp(t.pooled.theta) - 1) * 100 : null,
      u = t.pairwiseConsensus?.sensitivity?.direction_agreement,
      p = t.pairwiseConsensus?.sensitivity?.tau2_range,
      m = document.createElement("div");
    m.id = "pairwisePoolingSummaryCard", m.className = "card", m.style.marginTop = "var(--space-6)", m.innerHTML = `\n      <div class="card__header">\n        <h2 class="card__title">Pairwise Pooling and Summary Measures</h2>\n      </div>\n      <div class="card__body" style="padding: 0;">\n        <table class="data-table data-table--mono">\n          <thead>\n            <tr>\n              <th>Model</th>\n              <th>Estimate</th>\n              <th>95% CI</th>\n              <th>tau2</th>\n              <th>p</th>\n              <th>Note</th>\n            </tr>\n          </thead>\n          <tbody>\n            ${l.map(e=>`\n              <tr>\n                <td class="font-sans">${sanitizeHTML(e.model)}</td>\n                <td>${e.estimate}</td>\n                <td>${e.ci}</td>\n                <td>${e.tau2}</td>\n                <td>${e.pval}</td>\n                <td class="text-secondary">${sanitizeHTML(e.note)}</td>\n              </tr>\n            `).join("")}\n          </tbody>\n        </table>\n        <div style="padding: var(--space-4); border-top: 1px solid var(--border-subtle);">\n          <div class="stat-grid" style="grid-template-columns: repeat(4, 1fr);">\n            <div class="stat-card">\n              <div class="stat-card__value">${null!==c?`${c>=0?"+":""}${c.toFixed(1)}%`:"N/A"}</div>\n              <div class="stat-card__label">Relative change</div>\n            </div>\n            <div class="stat-card">\n              <div class="stat-card__value">${d&&isFinite(d.ARR_pct)?`${d.ARR_pct.toFixed(2)}%`:"N/A"}</div>\n              <div class="stat-card__label">Absolute change @10% risk</div>\n            </div>\n            <div class="stat-card">\n              <div class="stat-card__value">${d&&isFinite(d.NNT)?`${d.type||"NNT"} ${d.NNT}`:"N/A"}</div>\n              <div class="stat-card__label">NNT/NNH @10% risk</div>\n            </div>\n            <div class="stat-card">\n              <div class="stat-card__value">${isFinite(u)?`${(100*u).toFixed(0)}%`:"N/A"}</div>\n              <div class="stat-card__label">Direction agreement</div>\n            </div>\n          </div>\n          <p class="text-xs text-secondary" style="margin-top: var(--space-2);">\n            Estimator tau2 spread: ${isFinite(p)?p.toFixed(4):"N/A"}\n          </p>\n        </div>\n      </div>\n    `;
    const h = Array.from(e.querySelectorAll(".card__title")).find(e => /Prediction Intervals/i.test(e.textContent || "")),
      v = h ? h.closest(".card") : null;
    v ? v.insertAdjacentElement("beforebegin", m) : e.prepend(m)
  } catch (e) {
    log.error("renderPairwisePoolingSummaryCard failed:", e)
  }
}
...
function renderResults() {
  const e = AppState.results;
  e && (safeRender(renderAnalysisPanel, "panel-analysis", e), safeRender(renderHeterogeneityPanel, "panel-heterogeneity", e), safeRender(renderBiasPanel, "panel-bias", e), updateFloatingNav(), safeTimeout(() => normalizeMojibakeInDOM(document.body), 0))
}
  ...
  try {
    ...
    n.querySelectorAll("tbody tr").forEach(t => {
      ...
      const a = n.textContent.trim(),
        s = e.tau2_all[a],
        i = s && void 0 !== s.weight && isFinite(s.weight) ? `${(100*s.weight).toFixed(1)}%` : "â€”";
      a === (e.tau2Result && e.tau2Result.method) ? t.style.background = "var(--surface-highlight)" : t.style.background = "";
      let r = t.querySelector('td[data-col="weight"]');
      if (!r) {
        if (r = document.createElement("td"), r.dataset.col = "weight", r.style.whiteSpace = "nowrap", 0 === t.children.length) return void t.appendChild(r);
    ...
    log.error("enhanceTau2EstimatorTable failed:", e)
  }
}

function renderPairwiseConsensusCard(e) {
  try {
    const t = document.getElementById("panel-heterogeneity");
    if (!t) return;
    const n = t.querySelector("#pairwiseConsensusCard");
    n && n.remove();
    const a = e && e.pairwiseConsensus;
    if (!a || !a.available || !a.consensus) return;
    const s = t.querySelector(".grid");
    if (!s) return;
    const i = a.consensus,
      r = a.sensitivity || {},
      o = a.robustness || {},
      l = null != i.top_weight && isFinite(i.top_weight) ? (100 * i.top_weight).toFixed(1) : "N/A",
      d = null != i.tau2 && isFinite(i.tau2) ? i.tau2.toFixed(4) : "N/A",
      c = null != i.I2 && isFinite(i.I2) ? i.I2.toFixed(1) : "N/A",
      u = null != r.theta_range && isFinite(r.theta_range) ? r.theta_range.toFixed(3) : "N/A",
      p = null != r.significance_agreement && isFinite(r.significance_agreement) ? (100 * r.significance_agreement).toFixed(0) : "N/A",
      m = document.createElement("div");
    m.id = "pairwiseConsensusCard", m.className = "card", m.innerHTML = `
      <div class="card__header">
        <h2 class="card__title">Estimator Ensemble Consensus</h2>
      </div>
      <div class="card__body">
        <div class="stat-grid" style="grid-template-columns: repeat(2, 1fr);">
          <div class="stat-card">
            <div class="stat-card__value stat-card__value--accent">${formatEffect(i.theta,e.measure)}</div>
            <div class="stat-card__label">Model-averaged effect</div>
          </div>
          <div class="stat-card">
            <div class="stat-card__value">${d}</div>
            <div class="stat-card__label">Model-averaged tau2</div>
          </div>
          <div class="stat-card">
            <div class="stat-card__value">${c}%</div>
            <div class="stat-card__label">Model-averaged I2</div>
          </div>
          <div class="stat-card">
            <div class="stat-card__value">${u}</div>
            <div class="stat-card__label">Effect range across estimators</div>
          </div>
        </div>
        <p class="text-sm text-secondary" style="margin-top: var(--space-3);">
          Top estimator: <strong>${sanitizeHTML(i.top_method || "N/A")}</strong> (${l}% model weight)
        </p>
        <p class="font-mono text-sm" style="margin-top: var(--space-1);">
          95% CI: ${formatEffect(i.ci_lower,e.measure)} - ${formatEffect(i.ci_upper,e.measure)}
        </p>
        <p class="text-xs text-secondary" style="margin-top: var(--space-2);">
          Stability: ${sanitizeHTML(o.overall || "N/A")}; significance agreement ${p}%
        </p>
      </div>
    `;
    const h = s.querySelector(".card");
    h && h.nextSibling ? s.insertBefore(m, h.nextSibling) : s.appendChild(m)
  } catch (e) {
    log.error("renderPairwiseConsensusCard failed:", e)
  }
}

function enhancePredictionIntervalMethods(e) {
  try {
    if (!e || !e.pi || !e.pi.ensemble || e.pi.ensemble.skipped) return;
    const t = document.getElementById("panel-heterogeneity");
    if (!t) return;
    const n = Array.from(t.querySelectorAll("table.data-table")).find(e => {
      const t = e.querySelector("thead tr");
      if (!t) return !1;
      const n = Array.from(t.querySelectorAll("th")).map(e => (e.textContent || "").trim().toLowerCase());
      return n.length >= 3 && "method" === n[0] && "lower" === n[1] && "upper" === n[2]
    });
    if (!n) return;
    const a = n.querySelector("tbody");
    if (!a || a.querySelector('tr[data-method="ensemble"]')) return;
    const s = e.pi.ensemble,
      i = "RD" === e.measure || "MD" === e.measure || "SMD" === e.measure ? (s.upper - s.lower).toFixed(3) : (Math.exp(s.upper) - Math.exp(s.lower)).toFixed(3),
      r = document.createElement("tr");
    r.dataset.method = "ensemble", r.innerHTML = `
      <td class="font-sans">Estimator ensemble</td>
      <td>${formatEffect(s.lower,e.measure)}</td>
      <td>${formatEffect(s.upper,e.measure)}</td>
      <td>${i}</td>
      <td class="text-secondary">Model-averaged PI</td>
    `, a.appendChild(r)
  } catch (e) {
    log.error("enhancePredictionIntervalMethods failed:", e)
  }
}

function renderRareEventEngineCard(e) {
  try {
    const t = document.getElementById("panel-analysis");
    if (!t) return;
    const n = t.querySelector("#rareEventEngineCard");
    n && n.remove();
    const a = e && e.rareEventEngine;
    if (!a || !a.available) return;
    const s = a.recommendation || {},
      i = isFinite(a.event_rate_overall) ? (100 * a.event_rate_overall).toFixed(2) + "%" : "N/A",
      r = isFinite(a.event_rate_treatment) ? (100 * a.event_rate_treatment).toFixed(2) + "%" : "N/A",
      o = isFinite(a.event_rate_control) ? (100 * a.event_rate_control).toFixed(2) + "%" : "N/A",
      l = isFinite(a.max_arm_ratio) ? a.max_arm_ratio.toFixed(2) : "N/A",
      d = a.methods && a.methods.mh && isFinite(a.methods.mh.estimate) ? a.methods.mh.estimate.toFixed("RD" === (a.methods.mh.measure || e.measure) ? 4 : 3) : "N/A",
      c = a.methods && a.methods.peto && isFinite(a.methods.peto.estimate) ? a.methods.peto.estimate.toFixed(3) : "N/A",
      u = Array.isArray(s.rationale) ? s.rationale.slice(0, 3).map(e => `<li>${sanitizeHTML(String(e))}</li>`).join("") : "",
      p = document.createElement("div");
    p.id = "rareEventEngineCard", p.className = "card", p.style.marginTop = "var(--space-6)", p.innerHTML = `
      <div class="card__header">
        <h2 class="card__title">Rare-Event Engine</h2>
      </div>
      <div class="card__body">
        <div class="stat-grid" style="grid-template-columns: repeat(4, 1fr);">
          <div class="stat-card">
            <div class="stat-card__value">${i}</div>
            <div class="stat-card__label">Overall event rate</div>
          </div>
          <div class="stat-card">
            <div class="stat-card__value">${a.zero_cells_any || 0}</div>
            <div class="stat-card__label">Studies with any zero cell</div>
          </div>
          <div class="stat-card">
            <div class="stat-card__value">${a.zero_cells_double || 0}</div>
            <div class="stat-card__label">Double-zero studies</div>
          </div>
          <div class="stat-card">
            <div class="stat-card__value">${l}</div>
            <div class="stat-card__label">Max arm-size ratio</div>
          </div>
        </div>
        <div class="alert alert--info" style="margin-top: var(--space-4);">
          <span class="alert__icon">i</span>
          <div class="alert__content">
            <div class="alert__title">Recommended engine: ${sanitizeHTML(s.engine || "N/A")}</div>
            <div class="alert__text">Confidence: ${sanitizeHTML(s.confidence || "N/A")} | Treatment event rate ${r}, control event rate ${o}</div>
          </div>
        </div>
        <p class="text-sm text-secondary" style="margin-top: var(--space-3);">
          MH estimate: <strong>${d}</strong> | Peto OR: <strong>${c}</strong>
        </p>
        ${u ? `<ul class="text-xs text-secondary" style="margin-top: var(--space-2);">${u}</ul>` : ""}
      </div>
    `;
    const m = t.querySelector("div.grid");
    m && m.parentNode === t ? m.insertAdjacentElement("afterend", p) : t.prepend(p)
  } catch (e) {
    log.error("renderRareEventEngineCard failed:", e)
  }
}

function renderRobustnessVerdictCard(e) {
  try {
    const t = document.getElementById("panel-analysis");
    if (!t) return;
    const n = t.querySelector("#robustnessVerdictCard");
    n && n.remove();
    const a = e && e.robustnessVerdict;
    if (!a || !a.available) return;
    const s = "High robustness" === a.verdict ? "text-success" : "Moderate robustness" === a.verdict ? "text-warning" : "text-danger",
      i = e && e.pi && e.pi.ensemble && !e.pi.ensemble.skipped ? e.pi.ensemble : e && e.pi && e.pi.noma ? e.pi.noma : e && e.pi ? e.pi.standard : null,
      r = i && isFinite(i.lower) && isFinite(i.upper) ? `${formatEffect(i.lower,e.measure)} - ${formatEffect(i.upper,e.measure)}` : "N/A",
      o = i && i.method ? i.method : "none",
      l = Array.isArray(a.reasons) ? a.reasons.slice(0, 4).map(e => `<li>${sanitizeHTML(String(e))}</li>`).join("") : "",
      d = document.createElement("div");
    d.id = "robustnessVerdictCard", d.className = "card", d.style.marginTop = "var(--space-6)", d.innerHTML = `
      <div class="card__header">
        <h2 class="card__title">Robustness Verdict</h2>
      </div>
      <div class="card__body">
        <div class="stat-grid" style="grid-template-columns: repeat(4, 1fr);">
          <div class="stat-card">
            <div class="stat-card__value ${s}">${sanitizeHTML(a.verdict || "N/A")}</div>
            <div class="stat-card__label">Verdict</div>
          </div>
          <div class="stat-card">
            <div class="stat-card__value">${void 0 !== a.score ? a.score : "N/A"}</div>
            <div class="stat-card__label">Instability score</div>
          </div>
          <div class="stat-card">
            <div class="stat-card__value">${isFinite(a.I2) ? a.I2.toFixed(1) + "%" : "N/A"}</div>
            <div class="stat-card__label">I2</div>
          </div>
          <div class="stat-card">
            <div class="stat-card__value">${a.crosses_null ? "Yes" : "No"}</div>
            <div class="stat-card__label">PI crosses null</div>
          </div>
        </div>
        <p class="font-mono text-sm" style="margin-top: var(--space-3);">
          Ensemble PI (${sanitizeHTML(o)}): ${r}
        </p>
        ${l ? `<ul class="text-xs text-secondary" style="margin-top: var(--space-2);">${l}</ul>` : ""}
      </div>
    `;
    const c = t.querySelector("#rareEventEngineCard") || t.querySelector("div.grid");
    c && c.parentNode === t ? c.insertAdjacentElement("afterend", d) : t.prepend(d)
  } catch (e) {
    log.error("renderRobustnessVerdictCard failed:", e)
  }
}
...
function generateMethodsSection(e, t) {
  ...
  return "brief" === n ? (l += "<p>We conducted a random-effects meta-analysis of " + r + " studies. ", l += "Effect sizes were expressed as " + o + ". ", l += "Between-study variance (tau-squared) was estimated using the " + s + " method. ", l += "Heterogeneity was assessed using I-squared and Cochran Q test. ", l += "Publication bias was evaluated using funnel plots and Egger test.</p>") : (l += "<p>We performed a pairwise meta-analysis following established methodological guidelines. ", l += "Effect sizes were expressed as " + o, "RD" !== a && "MD" !== a && (l += " on the natural logarithm scale for analysis, then back-transformed for presentation"), l += ". ", l += "A random-effects model was used to account for expected clinical and methodological heterogeneity across studies. ", l += "Between-study variance (tau-squared) was estimated using the " + ({
    ...
    PM: "Paule-Mandel",
    PMM: "Paule-Mandel median-unbiased (PMM)",
    ML: "maximum likelihood",
    HS: "Hunter-Schmidt",
    HSk: "Hunter-Schmidt (k-corrected)",
    EB: "empirical Bayes",
    SJ: "Sidik-Jonkman",
    HE: "Hedges",
    GENQ: "generalized Q",
    GENQM: "generalized Q modified",
    PL: "profile likelihood",
    DL2: "two-step DerSimonian-Laird",
    CA: "Cochran ANOVA",
    BMM: "binomial-normal marginal",
    QG: "Q-generalized robust"
...
function generateResultsSection(e, t) {
  ...
  var n = t && t.length || "standard",
    ...
    s = e.measure || "OR",
    i = "RD" !== s && "MD" !== s,
    o = getJournalStyleConfig(t && t.style || "general"),
    l = o.ciFormat || "to",
    d = Number.isFinite(o.decimalPlaces) ? o.decimalPlaces : 2;

  function r(e, t) {
    return null == e || isNaN(e) ? "N/A" : Number(e).toFixed(void 0 === t ? d : t)
  }
  var c = i ? Math.exp(e.pooled.theta) : e.pooled.theta,
    u = i ? Math.exp(e.pooled.ci_lower) : e.pooled.ci_lower,
    p = i ? Math.exp(e.pooled.ci_upper) : e.pooled.ci_upper,
    m = null,
    h = null;
  e.pi && e.pi.standard && (m = i ? Math.exp(e.pi.standard.lower) : e.pi.standard.lower, h = i ? Math.exp(e.pi.standard.upper) : e.pi.standard.upper);
  var v = {
      OR: "OR",
      RR: "RR",
      HR: "HR",
  ...
  if (e.studies && e.studies.length)
    for (var y = 0; y < e.studies.length; y++) {
      var b = e.studies[y];
      f += (b.n_t || 0) + (b.n_c || 0), _ += (b.events_t || 0) + (b.events_c || 0)
    }
  var x = "N/A";
  void 0 !== e.pooled.p_value && null !== e.pooled.p_value && (x = e.pooled.p_value < .001 ? "<0.001" : r(e.pooled.p_value, 4));
  var w = e.het && void 0 !== e.het.I2 ? r(e.het.I2, 1) : "N/A",
    M = void 0 !== e.tau2 ? e.tau2 : e.het && void 0 !== e.het.tau2 ? e.het.tau2 : null,
    S = null !== M ? r(M, 4) : "N/A",
    E = null !== M ? r(Math.sqrt(Math.max(0, M)), 3) : "N/A",
    A = e.het && void 0 !== e.het.Q ? r(e.het.Q, 2) : "N/A",
    R = e.het && void 0 !== e.het.df ? e.het.df : e.k - 1,
    I = e.het && void 0 !== e.het.p_Q ? e.het.p_Q < .001 ? "<0.001" : r(e.het.p_Q, 4) : "N/A",
    T = "low";
  e.het && void 0 !== e.het.I2 && (e.het.I2 > 75 ? T = "high" : e.het.I2 > 50 ? T = "substantial" : e.het.I2 > 25 && (T = "moderate"));
  var C = "";
  if (!1 !== a.results) {
    if (C += "<h3>Main Results</h3>", "brief" === n) return C += "<p>" + e.k + " studies (N=" + f.toLocaleString() + ") were included. ", C += "Pooled " + v + " = " + r(c) + " (95% CI: " + r(u) + " " + l + " " + r(p) + "; p=" + x + "). ", C += "I-squared = " + w + "%. ", e.egger && (C += "Egger test p=" + (void 0 !== e.egger.p_value ? e.egger.p_value < .001 ? "<0.001" : r(e.egger.p_value, 3) : "N/A") + ".</p>"), C;
    if (C += "<p>A total of <strong>" + e.k + " studies</strong> met inclusion criteria, comprising " + f.toLocaleString() + " participants", _ > 0 && (C += " with " + _.toLocaleString() + " events"), C += ". ", "brief" !== n && "standard" !== n) {
      var P = [];
      if (e.studies)
        for (var k = 0; k < e.studies.length; k++) e.studies[k].year && P.push(e.studies[k].year);
      P.length > 0 && (C += "Studies were published between " + Math.min.apply(null, P) + " and " + Math.max.apply(null, P) + ". ")
    }
    if (C += "</p>", C += "<p>The pooled " + g + " was <strong>" + v + " = " + r(c) + "</strong> ", C += "(95% CI: " + r(u) + " " + l + " " + r(p) + "; p=" + x + ")", void 0 !== e.pooled.p_value && e.pooled.p_value < .05 ? (C += ", indicating a statistically significant ", C += i ? c < 1 ? "reduction" : "increase" : "difference") : C += ", which did not reach statistical significance", C += ". ", i && 1 !== c) C += "This corresponds to a <strong>" + r(Math.abs(100 * (1 - c)), 0) + "% relative " + (c < 1 ? "reduction" : "increase") + "</strong> in risk. ";
    if (C += "</p>", null !== m && null !== h) C += "<p>The 95% prediction interval ranged from " + r(m) + " " + l + " " + r(h), C += i && m < 1 && h > 1 || !i && m < 0 && h > 0 ? ", crossing the null value, indicating that effects in future similar settings may vary from beneficial to harmful" : ", suggesting consistent " + (i ? h < 1 ? "benefit" : "harm" : h < 0 ? "benefit" : "harm") + " across settings", C += ".</p>"
  }
  if (!1 !== a.heterogeneity) {
    if (C += "<h3>Heterogeneity</h3>", C += "<p>Statistical heterogeneity was <strong>" + T + "</strong>. ", C += "The I-squared statistic was " + w + "%", e.het && e.het.I2_ci && (C += " (95% CI: " + r(e.het.I2_ci.lower, 1) + "% " + l + " " + r(e.het.I2_ci.upper, 1) + "%)"), C += ", indicating that ", e.het && void 0 !== e.het.I2 && (e.het.I2 < 25 ? C += "most of the observed variance was due to sampling error rather than true differences between studies" : e.het.I2 < 50 ? C += "a moderate proportion of observed variance reflected true differences between studies" : C += "the majority of observed variance reflected true differences between studies"), C += ". ", C += "The between-study variance was tau-squared = " + S, i && null !== M && M > 0) C += " (tau = " + E + ", corresponding to approximately " + Math.round(100 * (Math.exp(Math.sqrt(M)) - 1)) + "% typical variation in effects on the ratio scale)";
    if (C += ". ", C += "Cochran Q test: Q = " + A + ", df = " + R + ", p=" + I, e.het && void 0 !== e.het.p_Q && (C += e.het.p_Q < .1 ? " (significant)" : " (not significant)"), C += ".</p>", ("detailed" === n || "extended" === n) && e.het && void 0 !== e.het.Q && R > 0) {
      var D = e.het.Q / R;
      C += "<p>The H-squared statistic was " + r(D, 2) + ", suggesting that observed variance was " + r(D, 1) + " times larger than expected under homogeneity.</p>"
    }
  }
  if (!1 !== a.bias && e.egger) {
    C += "<h3>Publication Bias</h3>";
    var $ = void 0 !== e.egger.p_value ? e.egger.p_value < .001 ? "<0.001" : r(e.egger.p_value, 4) : "N/A",
      N = void 0 !== e.egger.intercept ? r(e.egger.intercept, 3) : "N/A",
      F = void 0 !== e.egger.se ? r(e.egger.se, 3) : "N/A",
      L = void 0 !== e.egger.p_value && e.egger.p_value < .1;
    if (C += "<p>Visual inspection of the funnel plot " + (L ? "suggested asymmetry" : "did not reveal obvious asymmetry") + ". ", C += "Egger regression test " + (L ? "was significant" : "was not significant") + " ", C += "(intercept = " + N + ", SE = " + F + ", p=" + $ + "), ", C += (L ? "suggesting possible publication bias or small-study effects" : "providing no strong evidence of publication bias") + ". ", "brief" !== n && e.peters) C += "Peters test (less affected by heterogeneity): p=" + (void 0 !== e.peters.p_value ? e.peters.p_value < .001 ? "<0.001" : r(e.peters.p_value, 4) : "N/A") + ". ";
    if (e.begg && ("detailed" === n || "extended" === n)) C += "Begg rank correlation: tau = " + (void 0 !== e.begg.tau ? r(e.begg.tau, 3) : "N/A") + ", p=" + (void 0 !== e.begg.p_value ? e.begg.p_value < .001 ? "<0.001" : r(e.begg.p_value, 4) : "N/A") + ". ";
    if (C += "</p>", e.trimfill) {
      if (C += "<p>Trim-and-fill analysis imputed " + (e.trimfill.k0_imputed || 0) + " potentially missing studies", e.trimfill.k0_imputed > 0 && e.trimfill.adjusted) {
        var O = i ? Math.exp(e.trimfill.adjusted.theta) : e.trimfill.adjusted.theta,
          B = i ? Math.exp(e.trimfill.adjusted.ci_lower) : e.trimfill.adjusted.ci_lower,
          H = i ? Math.exp(e.trimfill.adjusted.ci_upper) : e.trimfill.adjusted.ci_upper;
        C += ", yielding an adjusted estimate of " + v + " = " + r(O) + " (95% CI: " + r(B) + " " + l + " " + r(H) + ")"
      }
      C += ".</p>"
    }
  }
  if (!1 !== a.sensitivity && ("detailed" === n || "extended" === n)) {
    if (C += "<h3>Sensitivity Analyses</h3>", C += "<p>Leave-one-out analysis was performed to assess the influence of individual studies on the pooled estimate. ", e.loo && e.loo.studies) {
      for (var q = 0, U = "", V = 0; V < e.loo.studies.length; V++) {
        var W = e.loo.studies[V];
        void 0 !== W.change_pct && Math.abs(W.change_pct) > Math.abs(q) && (q = W.change_pct, U = W.name || "Study " + (V + 1))
      }
      U && (C += "The most influential study was " + U + ", whose removal changed the estimate by " + r(Math.abs(q), 1) + "%. ")
    }
    C += "Results were robust to the removal of any single study.</p>"
  }!1 === a.ddma || "detailed" !== n && "extended" !== n || !e.ddma || (C += "<h3>Decision-Driven Analysis</h3>", C += "<p>Using the confidence distribution, the probability of any beneficial effect (" + v + " ", C += (i ? "<1" : "<0") + ") was ", C += (void 0 !== e.ddma.P_benefit_conf ? r(100 * e.ddma.P_benefit_conf, 1) : "N/A") + "%. ", void 0 !== e.ddma.P_mcid_conf && (C += "The probability of exceeding the minimum clinically important difference was " + r(100 * e.ddma.P_mcid_conf, 1) + "%. "), void 0 !== e.ddma.P_benefit_pred ? C += "Using the predictive distribution (accounting for heterogeneity), the probability that a future similar setting would show benefit was " + r(100 * e.ddma.P_benefit_pred, 1) + "%.</p>" : C += "</p>");
  if (!1 !== a.crossDisc && e.crossDisc) {
    var G = e.crossDisc,
      j = [];
    if (G.pdgScaleFactor && G.pdgScaleFactor.available) {
      var z = void 0 !== G.pdgScaleFactor.S_applied ? r(G.pdgScaleFactor.S_applied, 2) : "N/A",
        X = void 0 !== G.pdgScaleFactor.inflation_percent ? r(G.pdgScaleFactor.inflation_percent, 1) : "0.0",
        Y = "The PDG scale-factor analysis gave S = " + z + ". ";
      Y += G.pdgScaleFactor.S_applied > 1 ? "Confidence intervals were inflation-adjusted by " + X + "% for excess dispersion." : "No confidence interval inflation was required.", j.push(Y)
    }
    if (G.lowI2Diagnostic && G.lowI2Diagnostic.available) {
      var K = void 0 !== G.lowI2Diagnostic.power_to_detect_I2_25 ? r(100 * G.lowI2Diagnostic.power_to_detect_I2_25, 0) : "N/A",
        Z = G.lowI2Diagnostic.tension && void 0 !== G.lowI2Diagnostic.tension.percent ? r(G.lowI2Diagnostic.tension.percent, 1) : "N/A";
      j.push("The low-I2 diagnostic was classified as " + (G.lowI2Diagnostic.diagnosis || "N/A") + " (power to detect I2=25%: " + K + "%; disagreeing study pairs: " + Z + "%).")
    }
    if (G.winnersCurse && G.winnersCurse.available) {
      var J = "Winner's-curse diagnostics suggested " + (G.winnersCurse.interpretation || "uncertain inflation") + ". ";
      J += G.winnersCurse.consensus_estimate && void 0 !== G.winnersCurse.consensus_estimate.theta_exp ? "Bias-adjusted consensus estimate: " + v + " = " + r(G.winnersCurse.consensus_estimate.theta_exp) + "." : "Estimated inflation was " + (G.winnersCurse.bootstrap_correction && void 0 !== G.winnersCurse.bootstrap_correction.inflation_pct ? r(Math.abs(G.winnersCurse.bootstrap_correction.inflation_pct), 1) : "N/A") + "%.", j.push(J)
    }
    if (G.responseDistribution && G.responseDistribution.available) {
      var Q = G.responseDistribution.response_rates,
        ee = Q && void 0 !== Q.P_benefit ? r(100 * Q.P_benefit, 1) : "N/A",
        te = Q && void 0 !== Q.P_mcid ? r(100 * Q.P_mcid, 1) : "N/A",
        ne = Q && void 0 !== Q.P_harm ? r(100 * Q.P_harm, 1) : "N/A";
      j.push("Response-distribution modeling estimated P(benefit)=" + ee + "%, P(MCID)=" + te + "%, and P(harm/no-benefit)=" + ne + "% in future settings.")
    }
    if (j.length > 0) {
      C += "<h3>Cross-Disciplinary Robustness Checks</h3>";
      for (var ae = 0; ae < j.length; ae++) C += "<p>" + j[ae] + "</p>"
    }
  }
  return C
}
...
function getMethodRationale(e) {
  return {
    ...
    PMM: "median-unbiased variant for small-sample calibration",
    ML: "asymptotic efficiency",
    HS: "robustness to outliers",
    HSk: "small-sample correction to Hunter-Schmidt weighting",
    ...
    GENQ: "generalized method-of-moments weighting",
    GENQM: "bias-corrected generalized Q approach",
    PL: "likelihood-based calibration of between-study variance",
    DL2: "iterative refinement of DerSimonian-Laird",
    CA: "ANOVA-style variance decomposition",
    BMM: "marginal likelihood for sparse binary outcomes",
    QG: "robust generalized Q weighting"
...
function collectRenderedReportSections() {
  const e = [
      ["methodsContent", "Methods"],
      ["resultsContent", "Results"],
      ["verdictContent", "TruthCert Verdict"],
      ["htaContent", "Health Technology Assessment"],
      ["prismaChecklist", "PRISMA Checklist"],
      ["gradeProfile", "GRADE Evidence Profile"]
    ],
    t = [];
  for (let n = 0; n < e.length; n++) {
    const a = e[n],
      s = document.getElementById(a[0]);
    if (!s) continue;
    const i = (s.innerText || s.textContent || "").trim();
    i && t.push({
      title: a[1],
      text: i
    })
...
window.runFullAnalysis = runFullAnalysis, window.onTabActivate = onTabActivate, window.computeGRADE = computeGRADE, window.showGRADEDetails = showGRADEDetails, registerAdvancedMethodGlobals(), window.addEventListener("load", function() {
  registerAdvancedMethodGlobals(), startMojibakeObserver(), safeTimeout(() => normalizeMojibakeInDOM(document.body), 0)
...
function logTierDAnalysis(e) {
  ...
  const t = safeParseJSON(localStorage.getItem("tierDAuditLog"), []);
  Array.isArray(t) ? (t.push(e), localStorage.setItem("tierDAuditLog", JSON.stringify(t))) : localStorage.setItem("tierDAuditLog", JSON.stringify([e])), console.warn("[AUDIT] Tier D HTA Analysis:", e)
}

function getTierDAuditLog() {
  const e = safeParseJSON(localStorage.getItem("tierDAuditLog"), []);
...
function profileLikelihoodTau2CI(e, t = null, n = null, a = .05) {
  if (Array.isArray(e) && (null === t || void 0 === t) && e.length > 0 && "object" == typeof e[0]) {
    if (e.length < 3) return {
      ok: !1,
    ...
    const t = e.map(e => e.yi),
      ...
      s = estimateTau2_REML(t, n).tau2,
      i = profileLikelihoodTau2CIDetailed(t, n, s, a);
    return {
      ok: !0,
      tau2: s,
      ci_lo: i.ci_profile.lower,
      ci_hi: i.ci_profile.upper,
      details: i
    }
  }
  ...
  const s = "number" == typeof n && isFinite(n) ? n : estimateTau2_REML(e, t).tau2;
  return profileLikelihoodTau2CIDetailed(e, t, s, a)
}
...
function robSensitivity(e) {
  ...
  return logDecision(c ? "trigger" : "check", `RoB sensitivity: ÃŽâ€ = ${d.toFixed(4)}`), {
    ok: !0,
    triggered: c,
    effectAll: i.theta,
...
function buildThreatLedger(e, t) {
  ...
  n.push({
    ...
    detail: `Spread: ${r.toFixed(4)} (ÃŽÂ´: ${a.delta})`,
  ...
  n.push({
    ...
    detail: `IÃ‚Â² = ${o.toFixed(1)}%`,
    weight: o > l.high ? 1 : 0
  });
  const d = !1 !== e.tau2Stable;
  n.push({
    name: "Ãâ€žÃ‚Â² Stability",
  ...
  n.push({
    ...
    detail: x.triggered ? `ÃŽâ€ = ${(x.difference||0).toFixed(4)}` : "Stable",
...
function determineSeverity(e, t) {
  ...
  o("Extreme heterogeneity", a.extremeHeterogeneity || 1, (e.I2 || 0) > l), o("Ãâ€žÃ‚Â² unstable", a.tau2Unstable || 1, !1 === e.tau2Stable), o("Publication bias", a.publicationBias || 1, e.publicationBias?.detected);
...
function assessGRADE(e, t, n = {}) {
  ...
  const r = e.I2 || 0,
    ...
    d = [];
  if (r > 75 ? d.push(`IÃ‚Â² = ${r.toFixed(0)}% (high)`) : r > 50 && d.push(`IÃ‚Â² = ${r.toFixed(0)}% (moderate)`), l && o && d.push("PI crosses null despite significant CI"), d.length >= 2 || r > 75 ? a.inconsistency = {
      rating: "Serious",
      reason: d.join("; "),
      downgrade: 1
  ...
  if (e.ois?.metOIS) {
    if (void 0 !== e.ci_upper && void 0 !== e.ci_lower) {
      ...
      s || t > 6 * c ? a.imprecision = {
        rating: "Serious",
        reason: s ? "CI crosses both benefit and harm thresholds" : "CI width > 6Ãƒâ€” MCID (" + t.toFixed(2) + " vs " + (6 * c).toFixed(2) + ")",
        downgrade: 1
      } : (!n && Math.abs(e.theta || 0) >= c || t > 3 * c) && (a.imprecision = {
        rating: "Moderate",
        reason: t > 3 * c ? "CI width > 3Ãƒâ€” MCID" : "Important effect but CI includes null",
        downgrade: 1
      })
    }
  } else a.imprecision = {
...
function calculateICER(e, t, n = {}) {
  if (Array.isArray(e) && Array.isArray(t)) return calculateICERInterventions(e, t, n);
  const a = e.baselineRisk * t * e.qalyLoss,
    s = e.intervention.cost - e.comparator.cost;
  if (a <= 0 && s <= 0) return {
    value: null,
    incrementalCost: s,
    incrementalQALY: a,
  ...
  return {
    ...
    inferior: !1,
    dominant: !1,
    interpretation: `${formatCurrency(i)} per QALY gained`
  }
}
...
function getTierPermissions(e) {
  return {
    A: "Ã¢Å“â€¦ Full analysis Ã¢Å“â€¦ ICER Ã¢Å“â€¦ NMB Ã¢Å“â€¦ PSA Ã¢Å“â€¦ CEAC Ã¢Å“â€¦ EVPI",
    B: "Ã¢Å“â€¦ ICER Ã¢Å“â€¦ NMB Ã¢Å“â€¦ CEAC Ã¢Å¡Â Ã¯Â¸Â PSA limited",
    C: "Ã¢Å¡Â Ã¯Â¸Â Envelope only Ã¢ÂÅ’ Full PSA Ã¢Å¡Â Ã¯Â¸Â Conditional",
    D: "Ã¢ÂÅ’ Analysis refused Ã¢ÂÅ’ Conservative bounds only"
  } [e] || ""
}
...
function renderCEPlanePlot(e) {
  ...
  const t = e.incrementalQALY,
    ...
    s = {
      ...
      name: "ICER Estimate",
      hovertemplate: "ÃŽâ€E: %{x:.4f} QALYs<br>ÃŽâ€C: $%{y:,.0f}<extra></extra>"
    },
...
function renderGOSHPlot(e, t = "goshPlot") {
  ...
  n.forEach((e, t) => {
    ...
    r[n] || (r[n] = {
      ...
      I2: [],
      text: []
    }), r[n].theta.push(e.theta), r[n].I2.push(e.I2), r[n].text.push(`k=${e.k}, ÃŽÂ¸=${e.theta.toFixed(3)}, IÃ‚Â²=${e.I2.toFixed(1)}%`)
  ...
  const o = {
    ...
    xaxis: {
      title: "Pooled Effect (ÃŽÂ¸)"
    },
    yaxis: {
      title: "Heterogeneity (IÃ‚Â²%)",
      range: [0, 100]
    },
    paper_bgcolor: "rgba(0,0,0,0)",
    plot_bgcolor: "rgba(0,0,0,0)",
...
function pCurveAnalysis(e) {
  ...
  if (t.length < 3) return {
    ok: !1,
    message: "Insufficient significant studies (need Ã¢â€°Â¥ 3)",
    n: t.length
  };
  const n = [{
    range: [0, .01],
...
function zCurveAnalysis(e) {
  ...
  if (t.length < 5) return {
    ok: !1,
    message: "Insufficient significant studies (need Ã¢â€°Â¥ 5)",
    n: t.length
  };
  const n = t.map(e => e.z),
...
function runGOSHAnalysis() {
  ...
  const t = event?.target;
  t && (t.disabled = !0, t.innerHTML = '<span class="btn-spinner">Ã¢Å¸Â³</span> Running...'), showToast("Running GOSH analysis (this may take a moment)...", "info");
...
// =============================================================================
// FULL MCMC BAYESIAN META-ANALYSIS
// Comprehensive implementation with 4 chains, 20,000 iterations, proper R-hat/ESS
// =============================================================================

function fullMCMCBayesianMA(yi, vi, options = {}) {
  const {
    prior_mu = 0,
    prior_sd = 10,
    tau2_prior = 'half_cauchy',
  ...
  const dlResult = estimateTau2_DL(yi, vi);
  let tau2_init = Math.max(0.01, dlResult.tau2);
  let mu_init = calculatePooledEstimate(yi, vi, tau2_init).theta;

  // Storage for all chains
  const chainResults = [];
  const totalSamples = Math.floor((iterations - burnin) / thin);

  for (let c = 0; c < chains; c++) {
    // Disperse starting values across chains
    let tau2_current = tau2_init * (1 + 0.5 * (c - (chains - 1) / 2));
    let mu_current = mu_init + 0.3 * (c - (chains - 1) / 2);
    ...
    const chainSamples = {
      ...
      tau: [],
      theta_i: [],
      lp__: [] // Log posterior for diagnostics
    };

    for (let iter = 0; iter < iterations; iter++) {
      // Progress callback
      if (onProgress && iter % 1000 === 0) {
        const progress = ((c * iterations + iter) / (chains * iterations)) * 100;
        onProgress(progress, c + 1, chains, iter, iterations);
      }

      // Gibbs step for mu (conjugate update)
      const weights = vi.map(v => 1 / (v + tau2_current));
      const sum_weights = sum(weights);
      const prior_prec = 1 / (prior_sd * prior_sd);
      const post_prec = sum_weights + prior_prec;
      const post_var = 1 / post_prec;
      const weighted_mean = sum(yi.map((y, i) => weights[i] * y)) / sum_weights;
      const post_mean = (sum_weights * weighted_mean + prior_prec * prior_mu) / post_prec;
      mu_current = rnorm(post_mean, Math.sqrt(post_var), rng);

      // Slice sampling for tau2 (non-conjugate)
      tau2_current = sliceSampleTau2Full(yi, vi, mu_current, tau2_current, tau2_prior, tau2_scale, rng);

      // Sample study-specific effects (for shrinkage estimates)
      const theta_i = yi.map((y, i) => {
        const shrink = vi[i] / (vi[i] + tau2_current);
        const theta_var = shrink * tau2_current;
        return rnorm(shrink * mu_current + (1 - shrink) * y, Math.sqrt(theta_var), rng);
      });

      // Calculate log posterior for diagnostics
      const lp = calcLogPosterior(yi, vi, mu_current, tau2_current, prior_mu, prior_sd, tau2_prior, tau2_scale);

      // Store samples after burnin with thinning
      if (iter >= burnin && (iter - burnin) % thin === 0) {
        chainSamples.mu.push(mu_current);
        chainSamples.tau2.push(tau2_current);
        chainSamples.tau.push(Math.sqrt(tau2_current));
        chainSamples.theta_i.push(theta_i);
        chainSamples.lp__.push(lp);
      }
    }
  ...
  // Compute diagnostics with split-chain R-hat
  const diagnostics = computeFullMCMCDiagnostics(chainResults);

  // Combine all chains
  const combined = combineChains(chainResults);
  combined.lp__ = [].concat(...chainResults.map(c => c.lp__));

  // Compute summary statistics
  const summary = computeFullBayesianSummary(combined, yi, vi);
  ...
  return {
    ...
    convergence: {
      converged: diagnostics.rhat.mu < 1.1 && diagnostics.rhat.tau2 < 1.1,
      warning: diagnostics.rhat.mu > 1.05 || diagnostics.rhat.tau2 > 1.05 ?
        'Some R-hat values > 1.05 suggest chains may not have fully converged' : null
    },
    traceData: {
      mu: chainResults.map(c => c.mu),
      tau2: chainResults.map(c => c.tau2)
    }
  };
}

function sliceSampleTau2Full(yi, vi, mu, tau2_current, prior_type, scale, rng) {
  // Log posterior for tau2
  function logPost(tau2) {
    if (tau2 <= 0) return -Infinity;

    let ll = 0;
    for (let i = 0; i < yi.length; i++) {
      const v_total = vi[i] + tau2;
      ll += -0.5 * Math.log(v_total) - 0.5 * Math.pow(yi[i] - mu, 2) / v_total;
    }
    ...
    if (prior_type === 'half_cauchy') {
      ...
      lp = -Math.log(1 + Math.pow(tau / scale, 2)) - 0.5 * Math.log(tau2);
    } else if (prior_type === 'inv_gamma') {
      lp = -1.001 * Math.log(tau2) - 0.001 / tau2;
    } else {
      lp = -0.5 * Math.log(tau2); // Jeffreys
    }
  ...
  // Slice sampling
  const w = Math.max(0.1, Math.min(2, tau2_current));
  const y = logPost(tau2_current) + Math.log(rng());

  // Stepping out
  let L = Math.max(1e-10, tau2_current - w * rng());
  let R = tau2_current + w * rng();
  let J = Math.floor(100 * rng());
  let K = 99 - J;

  while (J > 0 && logPost(L) > y) {
    L = Math.max(1e-10, L - w);
    J--;
  }
  while (K > 0 && logPost(R) > y) {
    R = R + w;
    K--;
  }

  // Shrinking
  let tau2_new = tau2_current;
  for (let i = 0; i < 100; i++) {
    tau2_new = L + (R - L) * rng();
    if (logPost(tau2_new) > y) break;
    if (tau2_new < tau2_current) L = tau2_new;
    else R = tau2_new;
  }
...
function calcLogPosterior(yi, vi, mu, tau2, prior_mu, prior_sd, tau2_prior, tau2_scale) {
  let ll = 0;
  for (let i = 0; i < yi.length; i++) {
    const v_total = vi[i] + tau2;
    ll += -0.5 * Math.log(2 * Math.PI * v_total) - 0.5 * Math.pow(yi[i] - mu, 2) / v_total;
  }

  // Prior on mu
  const lp_mu = -0.5 * Math.log(2 * Math.PI * prior_sd * prior_sd) -
                0.5 * Math.pow(mu - prior_mu, 2) / (prior_sd * prior_sd);

  // Prior on tau2
  let lp_tau2;
  if (tau2_prior === 'half_cauchy') {
    const tau = Math.sqrt(tau2);
    lp_tau2 = -Math.log(Math.PI * tau2_scale * (1 + Math.pow(tau / tau2_scale, 2)));
  } else if (tau2_prior === 'inv_gamma') {
    lp_tau2 = -2 * Math.log(tau2) - 1 / tau2;
  } else {
    lp_tau2 = -0.5 * Math.log(tau2);
  }

  return ll + lp_mu + lp_tau2;
}

function computeFullMCMCDiagnostics(chains) {
  const nChains = chains.length;
  const nSamples = chains[0].mu.length;

  // Split-chain R-hat (Gelman et al. 2013)
  function splitRhat(param) {
    const splitChains = [];
    for (const chain of chains) {
      const samples = chain[param];
      const mid = Math.floor(samples.length / 2);
      splitChains.push(samples.slice(0, mid));
      splitChains.push(samples.slice(mid));
    }
    ...
    const chainMeans = splitChains.map(c => sum(c) / c.length);
    const W = sum(splitChains.map((c, i) => {
      const mean = chainMeans[i];
      return sum(c.map(x => Math.pow(x - mean, 2))) / (n - 1);
    })) / m;

    // Between-chain variance
    const grandMean = sum(chainMeans) / m;
    const B = (n / (m - 1)) * sum(chainMeans.map(m => Math.pow(m - grandMean, 2)));
  ...
  // Effective Sample Size with autocorrelation
  function computeESS(param) {
    const allSamples = [].concat(...chains.map(c => c[param]));
    const n = allSamples.length;
    const mean = sum(allSamples) / n;
    const variance = sum(allSamples.map(x => Math.pow(x - mean, 2))) / (n - 1);

    if (variance === 0) return n;

    // Compute autocorrelation using Geyer's initial monotone sequence
    let rho = [];
    for (let lag = 0; lag < Math.min(n - 1, 1000); lag++) {
      let acf = 0;
      for (let i = 0; i < n - lag; i++) {
        acf += (allSamples[i] - mean) * (allSamples[i + lag] - mean);
      }
      rho.push(acf / ((n - lag) * variance));
    }

    // Geyer's initial positive sequence estimator
    let sumRho = 0;
    for (let t = 1; t < rho.length - 1; t += 2) {
      const rhoSum = rho[t] + rho[t + 1];
      if (rhoSum < 0) break;
      sumRho += rhoSum;
    }
  ...
  function computeMCSE(param) {
    const allSamples = [].concat(...chains.map(c => c[param]));
    const ess = computeESS(param);
    const sd = Math.sqrt(sum(allSamples.map(x => {
      const mean = sum(allSamples) / allSamples.length;
  ...
  return {
    ...
    ess: {
      mu: computeESS('mu'),
      tau2: computeESS('tau2'),
      lp__: computeESS('lp__')
    },
    mcse: {
      mu: computeMCSE('mu'),
      tau2: computeMCSE('tau2')
    },
    n_chains: nChains,
    n_samples_per_chain: nSamples,
    total_samples: nChains * nSamples
  };
}

function computeFullBayesianSummary(combined, yi, vi) {
  const k = yi.length;
  const n = combined.mu.length;

  // Sort for quantiles
  const muSorted = [...combined.mu].sort((a, b) => a - b);
  const tau2Sorted = [...combined.tau2].sort((a, b) => a - b);
  const tauSorted = [...combined.tau].sort((a, b) => a - b);
  ...
  // Effect size summary
  const mu_mean = sum(combined.mu) / n;
  const mu_median = quantile(muSorted, 0.5);
  const mu_sd = Math.sqrt(sum(combined.mu.map(x => Math.pow(x - mu_mean, 2))) / (n - 1));
  const mu_ci_lower = quantile(muSorted, 0.025);
  const mu_ci_upper = quantile(muSorted, 0.975);
  const mu_hdi_lower = quantile(muSorted, 0.025);  // Simplified HDI = equal-tailed CI
  const mu_hdi_upper = quantile(muSorted, 0.975);

  // Tau2 summary
  const tau2_mean = sum(combined.tau2) / n;
  const tau2_median = quantile(tau2Sorted, 0.5);
  const tau2_ci_lower = quantile(tau2Sorted, 0.025);
  const tau2_ci_upper = quantile(tau2Sorted, 0.975);

  // Tau summary
  const tau_mean = sum(combined.tau) / n;
  const tau_median = quantile(tauSorted, 0.5);

  // I2 posterior
  const typical_vi = sum(vi) / k;
  const I2_samples = combined.tau2.map(t2 => 100 * t2 / (t2 + typical_vi));
  const I2_sorted = [...I2_samples].sort((a, b) => a - b);
  const I2_mean = sum(I2_samples) / n;
  const I2_ci_lower = quantile(I2_sorted, 0.025);
  const I2_ci_upper = quantile(I2_sorted, 0.975);

  // Posterior probabilities (DDMA)
  const P_benefit = combined.mu.filter(x => x < 0).length / n;
  const P_harm = 1 - P_benefit;
  const P_mcid = combined.mu.filter(x => x < -0.15).length / n;
  const P_large = combined.mu.filter(x => x < -0.25).length / n;

  // Prediction interval (95%)
  const pred_samples = combined.mu.map((m, i) => {
    const t2 = combined.tau2[i];
    return rnorm(m, Math.sqrt(t2));
  });
  const predSorted = [...pred_samples].sort((a, b) => a - b);
  const pred_lower = quantile(predSorted, 0.025);
  const pred_upper = quantile(predSorted, 0.975);
  const P_benefit_pred = pred_samples.filter(x => x < 0).length / n;

  // Shrinkage estimates for each study
  const shrinkage_estimates = yi.map((y, i) => {
    const theta_i_samples = combined.theta_i.map(t => t[i]);
    const mean = sum(theta_i_samples) / theta_i_samples.length;
    const sorted = [...theta_i_samples].sort((a, b) => a - b);
    return {
      original: y,
      shrunk_mean: mean,
      shrunk_median: quantile(sorted, 0.5),
      ci_lower: quantile(sorted, 0.025),
      ci_upper: quantile(sorted, 0.975),
      shrinkage: 1 - mean / y
    };
  });
  ...
  return {
    effect: {
      ...
      median: mu_median,
      sd: mu_sd,
      ci_lower: mu_ci_lower,
      ci_upper: mu_ci_upper,
      hdi_lower: mu_hdi_lower,
      hdi_upper: mu_hdi_upper
    },
    tau2: {
      mean: tau2_mean,
      median: tau2_median,
      ci_lower: tau2_ci_lower,
      ci_upper: tau2_ci_upper
    },
    tau: {
      mean: tau_mean,
      median: tau_median
    ...
    I2: {
      mean: I2_mean,
      ci_lower: I2_ci_lower,
      ci_upper: I2_ci_upper
    },
    prediction_interval: {
      lower: pred_lower,
      upper: pred_upper
    ...
    ddma: {
      ...
      P_large,
      P_benefit_pred,
      interpretation: 'Full Bayesian posterior probabilities with proper MCMC convergence'
    },
    shrinkage_estimates,
    n_samples: n
  };
...
function runFullMCMCAnalysis() {
  console.log("[Full MCMC] Starting full Bayesian MCMC analysis...");
  ...
  if (btn) {
    btn.disabled = true;
    btn.innerHTML = '<span class="btn-spinner">Ã¢Å¸Â³</span> Running MCMC (0%)...';
  }

  const progressDiv = document.getElementById('fullMCMCProgress');
  const statusDiv = document.getElementById('fullMCMCStatus');
  if (progressDiv) progressDiv.style.display = 'block';

  const yi = results.studies.map(s => s.yi);
  const vi = results.studies.map(s => s.vi);

  // Run in chunks to allow UI updates
  setTimeout(() => {
    try {
      const mcmcResult = fullMCMCBayesianMA(yi, vi, {
        chains: 4,
        iterations: 20000,
        burnin: 10000,
        thin: 2,
        tau2_prior: 'half_cauchy',
        tau2_scale: 1,
        onProgress: (progress, chain, totalChains, iter, totalIter) => {
          if (btn) {
            btn.innerHTML = `<span class="btn-spinner">Ã¢Å¸Â³</span> Chain ${chain}/${totalChains} (${progress.toFixed(0)}%)...`;
          }
          if (statusDiv) {
            statusDiv.textContent = `Chain ${chain}/${totalChains}, iteration ${iter}/${totalIter}`;
          }
        }
      });

      // Store in AppState
      AppState.fullMCMCResults = mcmcResult;

      // Display results
      displayFullMCMCResults(mcmcResult);

      // Render trace plots
      renderMCMCTracePlots(mcmcResult.traceData, 'fullMCMCTracePlot');

      if (progressDiv) progressDiv.style.display = 'none';
      showToast("Full MCMC analysis complete!", "success");

    } catch (e) {
      console.error("[Full MCMC] Error:", e);
      showToast("Full MCMC failed: " + e.message, "error");
    } finally {
      if (btn) {
        btn.disabled = false;
        btn.innerHTML = 'Ã°Å¸Å½Â² Run Full MCMC';
      }
    }
  }, 50);
}

function displayFullMCMCResults(result) {
  const container = document.getElementById('fullMCMCResults');
  if (!container) return;

  const { summary, diagnostics, convergence } = result;

  const rhatColor = (val) => val < 1.01 ? 'var(--color-success-500)' :
                            val < 1.05 ? 'var(--color-warning-500)' :
                            'var(--color-danger-500)';

  const essColor = (val) => val > 400 ? 'var(--color-success-500)' :
                           val > 100 ? 'var(--color-warning-500)' :
                           'var(--color-danger-500)';

  container.innerHTML = `
    <div style="margin-bottom: var(--space-3);">
      <h4 style="margin-bottom: var(--space-2);">Convergence Diagnostics</h4>
      <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: var(--space-2); font-size: var(--text-sm);">
        <div style="padding: var(--space-2); background: var(--surface-hover); border-radius: var(--radius-sm);">
          <strong>R-hat (ÃŽÂ¼):</strong>
          <span style="color: ${rhatColor(diagnostics.rhat.mu)}">${diagnostics.rhat.mu.toFixed(4)}</span>
        </div>
        <div style="padding: var(--space-2); background: var(--surface-hover); border-radius: var(--radius-sm);">
          <strong>R-hat (Ãâ€žÃ‚Â²):</strong>
          <span style="color: ${rhatColor(diagnostics.rhat.tau2)}">${diagnostics.rhat.tau2.toFixed(4)}</span>
        </div>
        <div style="padding: var(--space-2); background: var(--surface-hover); border-radius: var(--radius-sm);">
          <strong>R-hat (lp__):</strong>
          <span style="color: ${rhatColor(diagnostics.rhat.lp__)}">${diagnostics.rhat.lp__.toFixed(4)}</span>
        </div>
        <div style="padding: var(--space-2); background: var(--surface-hover); border-radius: var(--radius-sm);">
          <strong>ESS (ÃŽÂ¼):</strong>
          <span style="color: ${essColor(diagnostics.ess.mu)}">${Math.round(diagnostics.ess.mu)}</span>
        </div>
        <div style="padding: var(--space-2); background: var(--surface-hover); border-radius: var(--radius-sm);">
          <strong>ESS (Ãâ€žÃ‚Â²):</strong>
          <span style="color: ${essColor(diagnostics.ess.tau2)}">${Math.round(diagnostics.ess.tau2)}</span>
        </div>
        <div style="padding: var(--space-2); background: var(--surface-hover); border-radius: var(--radius-sm);">
          <strong>Total samples:</strong> ${diagnostics.total_samples.toLocaleString()}
        </div>
      </div>
      ${convergence.warning ? `<p style="color: var(--color-warning-500); margin-top: var(--space-2); font-size: var(--text-xs);">Ã¢Å¡Â Ã¯Â¸Â ${convergence.warning}</p>` :
        `<p style="color: var(--color-success-500); margin-top: var(--space-2); font-size: var(--text-xs);">Ã¢Å“â€œ Chains converged (R-hat < 1.1)</p>`}
    </div>

    <div style="margin-bottom: var(--space-3);">
      <h4 style="margin-bottom: var(--space-2);">Posterior Summary</h4>
      <table style="width: 100%; font-size: var(--text-sm); border-collapse: collapse;">
        <tr style="border-bottom: 1px solid var(--border-default);">
          <th style="text-align: left; padding: var(--space-1);">Parameter</th>
          <th style="text-align: right; padding: var(--space-1);">Mean</th>
          <th style="text-align: right; padding: var(--space-1);">Median</th>
          <th style="text-align: right; padding: var(--space-1);">95% CrI</th>
        </tr>
        <tr style="border-bottom: 1px solid var(--border-subtle);">
          <td style="padding: var(--space-1);">Effect (ÃŽÂ¼)</td>
          <td style="text-align: right; padding: var(--space-1);">${summary.effect.mean.toFixed(4)}</td>
          <td style="text-align: right; padding: var(--space-1);">${summary.effect.median.toFixed(4)}</td>
          <td style="text-align: right; padding: var(--space-1);">[${summary.effect.ci_lower.toFixed(4)}, ${summary.effect.ci_upper.toFixed(4)}]</td>
        </tr>
        <tr style="border-bottom: 1px solid var(--border-subtle);">
          <td style="padding: var(--space-1);">Ãâ€žÃ‚Â² (heterogeneity)</td>
          <td style="text-align: right; padding: var(--space-1);">${summary.tau2.mean.toFixed(4)}</td>
          <td style="text-align: right; padding: var(--space-1);">${summary.tau2.median.toFixed(4)}</td>
          <td style="text-align: right; padding: var(--space-1);">[${summary.tau2.ci_lower.toFixed(4)}, ${summary.tau2.ci_upper.toFixed(4)}]</td>
        </tr>
        <tr style="border-bottom: 1px solid var(--border-subtle);">
          <td style="padding: var(--space-1);">Ãâ€ž (SD)</td>
          <td style="text-align: right; padding: var(--space-1);">${summary.tau.mean.toFixed(4)}</td>
          <td style="text-align: right; padding: var(--space-1);">${summary.tau.median.toFixed(4)}</td>
          <td style="text-align: right; padding: var(--space-1);">Ã¢â‚¬â€</td>
        </tr>
        <tr style="border-bottom: 1px solid var(--border-subtle);">
          <td style="padding: var(--space-1);">IÃ‚Â² (%)</td>
          <td style="text-align: right; padding: var(--space-1);">${summary.I2.mean.toFixed(1)}%</td>
          <td style="text-align: right; padding: var(--space-1);">Ã¢â‚¬â€</td>
          <td style="text-align: right; padding: var(--space-1);">[${summary.I2.ci_lower.toFixed(1)}%, ${summary.I2.ci_upper.toFixed(1)}%]</td>
        </tr>
      </table>
    </div>

    <div style="margin-bottom: var(--space-3);">
      <h4 style="margin-bottom: var(--space-2);">95% Prediction Interval</h4>
      <p style="font-size: var(--text-sm);">
        [${summary.prediction_interval.lower.toFixed(4)}, ${summary.prediction_interval.upper.toFixed(4)}]
      </p>
      <p style="font-size: var(--text-xs); color: var(--text-secondary);">
        Expected range for a new study's true effect
      </p>
    </div>

    <div>
      <h4 style="margin-bottom: var(--space-2);">DDMA Posterior Probabilities</h4>
      <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: var(--space-2); font-size: var(--text-sm);">
        <div style="padding: var(--space-2); background: var(--surface-hover); border-radius: var(--radius-sm);">
          <strong>P(benefit):</strong> ${(summary.ddma.P_benefit * 100).toFixed(1)}%
        </div>
        <div style="padding: var(--space-2); background: var(--surface-hover); border-radius: var(--radius-sm);">
          <strong>P(harm):</strong> ${(summary.ddma.P_harm * 100).toFixed(1)}%
        </div>
        <div style="padding: var(--space-2); background: var(--surface-hover); border-radius: var(--radius-sm);">
          <strong>P(clinically meaningful):</strong> ${(summary.ddma.P_mcid * 100).toFixed(1)}%
        </div>
        <div style="padding: var(--space-2); background: var(--surface-hover); border-radius: var(--radius-sm);">
          <strong>P(benefit in new study):</strong> ${(summary.ddma.P_benefit_pred * 100).toFixed(1)}%
        </div>
      </div>
    </div>
  `;
...
function renderMCMCTracePlots(traceData, containerId) {
  const container = document.getElementById(containerId);
  if (!container || typeof Plotly === 'undefined') return;

  const colors = ['#2E86AB', '#E94F37', '#4CAF50', '#FF9800'];

  // Mu trace plot
  const muTraces = traceData.mu.map((chain, i) => ({
    y: chain,
    x: Array.from({length: chain.length}, (_, j) => j),
    type: 'scatter',
    mode: 'lines',
    name: `Chain ${i + 1}`,
  ...
  // Tau2 trace plot
  const tau2Traces = traceData.tau2.map((chain, i) => ({
    y: chain,
    x: Array.from({length: chain.length}, (_, j) => j),
    type: 'scatter',
    mode: 'lines',
  ...
  const layout = {
    ...
    xaxis: { title: 'Iteration', domain: [0, 1] },
    yaxis: { title: 'ÃŽÂ¼ (Effect)', domain: [0.55, 1] },
    xaxis2: { title: 'Iteration', domain: [0, 1], anchor: 'y2' },
    yaxis2: { title: 'Ãâ€žÃ‚Â²', domain: [0, 0.45], anchor: 'x2' },
    paper_bgcolor: 'rgba(0,0,0,0)',
    plot_bgcolor: 'rgba(0,0,0,0)',
    font: { family: 'system-ui', size: 11 }
  };

  Plotly.newPlot(container, [...muTraces, ...tau2Traces], layout, { responsive: true });
}
...
function runPowerAnalysis() {
  const e = AppState.results;
  if (!e || !e.studies || !e.pooled) return void showToast("Run analysis first", "error");
  try {
    const t = e.studies.reduce((e, t) => e + (t.n_t && t.n_c ? t.n_t + t.n_c : Math.round(4 / t.vi)), 0) / Math.max(1, e.studies.length);
    const n = document.getElementById("powerResults") ? "powerResults" : "powerAnalysisResults",
      a = getElementByIds(["powerPlot", "powerCurvePlot"]);
    renderPowerAnalysis({
      k: e.k,
      avgN: t,
      tau2: e.tau2 || 0
    }, e.pooled.theta, n);
    const s = powerCurve(e.k, t, e.tau2 || 0);
    a && Plotly.newPlot(a, [{
      type: "scatter",
      mode: "lines",
      x: s.effectSizes,
      y: s.powers.map(e => 100 * e),
      line: {
        color: "#4a7ab8",
        width: 2
      },
      ...
      responsive: !0
    }), showToast("Power analysis complete", "success")
  } catch (e) {
    showToast("Power analysis failed: " + e.message, "error"), console.error(e)
  }
}
...
function ensureAdvancedGapCards() {
  const e = document.getElementById("panel-advanced");
  if (!e) return;
  const t = e.querySelector(".grid");
  if (!t) return;
  if (!document.getElementById("glmmAdvancedCard")) {
    const e = document.createElement("div");
    e.id = "glmmAdvancedCard", e.className = "card", e.style.background = "var(--surface-overlay)", e.innerHTML = `
      <div class="card__header" style="padding: var(--space-3);">
        <h3 class="card__title" style="font-size: var(--text-base);">GLMM Rare-Event</h3>
        <p class="card__subtitle">Exact binomial likelihood (rma.glmm-style) plus beta-binomial sensitivity</p>
      </div>
      <div class="card__body" style="padding: var(--space-3);">
        <button class="btn btn--primary btn--sm" onclick="runGLMMAdvanced()">Run GLMM</button>
        <div id="glmmAdvancedResults" style="margin-top: var(--space-3); font-size: var(--text-sm);"></div>
        <div id="glmmAdvancedPlot" style="width:100%; height:280px; margin-top: var(--space-3);"></div>
      </div>
    `, t.appendChild(e)
  }
  if (!document.getElementById("profileLikelihoodAdvancedCard")) {
    const e = document.createElement("div");
    e.id = "profileLikelihoodAdvancedCard", e.className = "card", e.style.background = "var(--surface-overlay)", e.innerHTML = `
      <div class="card__header" style="padding: var(--space-3);">
        <h3 class="card__title" style="font-size: var(--text-base);">Profile Likelihood</h3>
        <p class="card__subtitle">Likelihood profiling for tau2/mu with LR heterogeneity test</p>
      </div>
      <div class="card__body" style="padding: var(--space-3);">
        <button class="btn btn--primary btn--sm" onclick="runProfileLikelihoodAdvanced()">Run Profile</button>
        <div id="profileLikelihoodAdvancedResults" style="margin-top: var(--space-3); font-size: var(--text-sm);"></div>
        <div id="profileLikelihoodAdvancedPlot" style="width:100%; height:280px; margin-top: var(--space-3);"></div>
      </div>
    `, t.appendChild(e)
  }
}

function runGLMMAdvanced() {
  const e = AppState.results;
  if (!e || !Array.isArray(e.studies)) return void showToast("Run analysis first", "error");
  const t = e.studies.filter(e => isFinite(e.events_t) && isFinite(e.n_t) && isFinite(e.events_c) && isFinite(e.n_c) && e.n_t > 0 && e.n_c > 0 && e.events_t >= 0 && e.events_t <= e.n_t && e.events_c >= 0 && e.events_c <= e.n_c);
  if (t.length < 2) return void showToast("GLMM requires binary 2x2 study data", "error");
  const n = t.map(e => Number(e.events_t)),
    a = t.map(e => Number(e.n_t) - Number(e.events_t)),
    s = t.map(e => Number(e.events_c)),
    i = t.map(e => Number(e.n_c) - Number(e.events_c)),
    r = t.filter(e => 0 === e.events_t || 0 === e.events_c || e.events_t === e.n_t || e.events_c === e.n_c).length,
    o = glmmMetaAnalysis(n, a, s, i, {
      measure: "OR",
      method: "ML",
      nQuad: 13,
      continuity: .5
    }),
    l = betaBinomialMA(n, a, s, i),
    d = document.getElementById("glmmAdvancedResults");
  if (!d) return;
  if (o.error) return d.innerHTML = `<p class="text-danger">GLMM failed: ${sanitizeHTML(o.error)}</p>`, void showToast("GLMM failed", "error");
  d.innerHTML = `
    <div class="stat-grid" style="grid-template-columns: repeat(2, 1fr); gap: var(--space-3);">
      <div class="stat-card">
        <div class="stat-card__label">GLMM OR</div>
        <div class="stat-card__value">${isFinite(o.estimate.exp)?o.estimate.exp.toFixed(3):"N/A"}</div>
        <div style="font-size:var(--text-xs);">95% CI: [${isFinite(o.ci.lower)?o.ci.lower.toFixed(3):"N/A"}, ${isFinite(o.ci.upper)?o.ci.upper.toFixed(3):"N/A"}]</div>
      </div>
      <div class="stat-card">
        <div class="stat-card__label">Beta-Binomial OR</div>
        <div class="stat-card__value">${isFinite(l.estimate.OR)?l.estimate.OR.toFixed(3):"N/A"}</div>
        <div style="font-size:var(--text-xs);">95% CI: [${isFinite(l.ci.lower)?l.ci.lower.toFixed(3):"N/A"}, ${isFinite(l.ci.upper)?l.ci.upper.toFixed(3):"N/A"}]</div>
      </div>
    </div>
    <p style="margin-top: var(--space-2);">
      <strong>Studies:</strong> ${t.length}, <strong>Zero-cell studies:</strong> ${r}, <strong>tau2 (GLMM):</strong> ${isFinite(o.tau2)?o.tau2.toFixed(4):"N/A"}
    </p>
    <p style="font-size: var(--text-xs); color: var(--text-secondary);">${o.interpretation}</p>
  `;
  const c = document.getElementById("glmmAdvancedPlot");
  if ("undefined" != typeof Plotly && c) {
    const e = ["GLMM", "Beta-Binomial"],
      t = [o.estimate.exp, l.estimate.OR],
      n = [Math.max(0, o.ci.upper - o.estimate.exp), Math.max(0, l.ci.upper - l.estimate.OR)],
      a = [Math.max(0, o.estimate.exp - o.ci.lower), Math.max(0, l.estimate.OR - l.ci.lower)];
    Plotly.newPlot(c, [{
      type: "scatter",
      mode: "markers",
      x: e,
      ...
      hovertemplate: "%{x}<br>OR=%{y:.3f}<extra></extra>"
    }], {
      title: "Rare-event pooled OR comparison",
      yaxis: {
  ...
  showToast("GLMM analysis complete", "success")
}

function runProfileLikelihoodAdvanced() {
  const e = AppState.results;
  if (!e || !Array.isArray(e.yi) || !Array.isArray(e.vi) || e.yi.length < 2) return void showToast("Run analysis first", "error");
  const t = profileLikelihood(e.yi, e.vi, "tau2", null, 80),
    n = profileLikelihood(e.yi, e.vi, "mu", null, 80),
    a = likelihoodRatioTest(e.yi, e.vi),
    s = document.getElementById("profileLikelihoodAdvancedResults");
  if (!s) return;
  s.innerHTML = `
    <div class="stat-grid" style="grid-template-columns: repeat(2, 1fr); gap: var(--space-3);">
      <div class="stat-card">
        <div class="stat-card__label">tau2 estimate</div>
        <div class="stat-card__value">${isFinite(t.estimate)?t.estimate.toFixed(4):"N/A"}</div>
        <div style="font-size:var(--text-xs);">95% CI: [${isFinite(t.ci.lower)?t.ci.lower.toFixed(4):"N/A"}, ${isFinite(t.ci.upper)?t.ci.upper.toFixed(4):"N/A"}]</div>
      </div>
      <div class="stat-card">
        <div class="stat-card__label">mu estimate</div>
        <div class="stat-card__value">${isFinite(n.estimate)?n.estimate.toFixed(4):"N/A"}</div>
        <div style="font-size:var(--text-xs);">95% CI: [${isFinite(n.ci.lower)?n.ci.lower.toFixed(4):"N/A"}, ${isFinite(n.ci.upper)?n.ci.upper.toFixed(4):"N/A"}]</div>
      </div>
    </div>
    <p style="margin-top: var(--space-2);">
      <strong>LR heterogeneity p-value:</strong> ${isFinite(a.pValue)?a.pValue < .001 ? "&lt;0.001" : a.pValue.toFixed(4):"N/A"} (${a.significant?"heterogeneity detected":"no significant heterogeneity"})
    </p>
  `;
  const i = document.getElementById("profileLikelihoodAdvancedPlot");
  if ("undefined" != typeof Plotly && i) {
    const e = t.plotData.x,
      n = t.plotData.y,
      a = t.plotData.threshold;
    Plotly.newPlot(i, [{
      type: "scatter",
      mode: "lines",
      x: e,
      ...
      title: "Profile likelihood for tau2",
      xaxis: {
        title: "tau2"
      },
      yaxis: {
  ...
  showToast("Profile likelihood complete", "success")
}
...
function autoRenderPowerAnalysis() {
  ...
  const t = e.studies.reduce((e, t) => e + (t.n_t && t.n_c ? t.n_t + t.n_c : Math.round(4 / t.vi)), 0) / e.studies.length,
    n = document.getElementById("powerAnalysisResults") ? "powerAnalysisResults" : "powerResults",
    a = getElementByIds(["powerCurvePlot", "powerPlot"]);
  renderPowerAnalysis({
    k: e.k,
    avgN: t,
    tau2: e.tau2
  }, e.pooled.theta, n);
  const s = powerCurve(e.k, t, e.tau2);
  a && Plotly.newPlot(a, [{
    type: "scatter",
    mode: "lines",
    x: s.effectSizes,
...
function updateRoBMAProgress(e) {
  const t = getElementByIds(["robmaProgressBar", "modelavgProgressBar"]),
    n = getElementByIds(["robmaStatus", "modelavgStatus"]),
    a = getElementByIds(["robmaProgress", "modelavgProgress"]);
...
function runRoBMA() {
  const e = AppState.results;
  if (!e || !e.studies) return void showToast("Run analysis first", "error");
  const t = getElementByIds(["robmaBtn", "modelavgBtn"]),
    n = getElementByIds(["robmaProgress", "modelavgProgress"]),
    a = getElementByIds(["robmaProgressBar", "modelavgProgressBar"]),
    s = getElementByIds(["robmaStatus", "modelavgStatus"]),
    u = t && "modelavgBtn" === t.id ? "Run ModelAvg" : "Run RoBMA";
  ...
  function c() {
    d.terminate(), URL.revokeObjectURL(l), t && (t.disabled = !1, t.innerHTML = u), n && (n.style.display = "none")
  }
...
function renderRoBMAResults(e) {
  const t = getElementByIds(["robmaResults", "modelavgResults"]);
  if (!t) return;
...
function copasSelectionModelAdjusted(e, t, n = [-2, 0], a = [0, 2], s = 20) {
  e.length;
  ...
  return {
    ...
    bias_estimate: m,
    bias_pct: 0 === p ? null : 100 * Math.abs(m / p),
    gamma0: r.gamma0,
    gamma1: r.gamma1,
...
function copasSelectionModel(e, t, n = [-2, 0], a = [0, 2], s = 20) {
  const i = copasSelectionSensitivityBounds(e, t),
    r = copasSelectionModelAdjusted(e, t, n, a, s),
    o = {
      ...r,
      unadjusted: i.unadjusted,
      sensitivityContour: i.sensitivityContour,
      bounds: i.bounds,
      robustness: i.robustness,
      sensitivityInterpretation: i.interpretation
    };
  return (void 0 === o.theta_unadj || !isFinite(o.theta_unadj)) && i.unadjusted && isFinite(i.unadjusted.theta) && (o.theta_unadj = i.unadjusted.theta), o
}
...
function zejnullahiCI(e, t, n, a = .05) {
  ...
  return {
    ...
    interpretation: s < 5 ? "With < 5 studies, Zejnullahi CI provides substantially better coverage than standard CI." : s < 10 ? "With < 10 studies, Zejnullahi CI provides improved coverage." : "With Ã¢â€°Â¥ 10 studies, improvement over standard CI is modest."
  }
}

function profileLikelihoodTau2CIDetailed(e, t, n, a = .05) {
  const s = e.length;
  ...
  return {
    ...
    interpretation: 0 === l ? "Lower bound of Ãâ€žÃ‚Â² CI includes 0, suggesting heterogeneity may be negligible." : "Profile likelihood CI suggests genuine heterogeneity present."
  }
}

function qchisqWilsonApprox(e, t) {
  if (1 === t) {
    const t = qnorm(.5 + e / 2);
    return t * t
  }
...
function validateAgainstMetafor(e) {
  const t = e.studies.map(t => "binary" === e.type ? convertBinaryToEffectSize(t, e.measure) : convertContinuousToEffectSize(t, e.measure)),
    n = t.map(e => e.yi),
    a = t.map(e => e.vi),
    s = estimateTau2(n, a, "REML"),
    i = "number" == typeof s ? s : s && "number" == typeof s.tau2 ? s.tau2 : 0,
    r = calculatePooledEstimate(n, a, i),
    o = calculateHeterogeneity(n, a, i),
    l = e.expected_metafor,
    d = Math.abs(r.theta - l.theta),
    c = Math.abs(r.se - l.se),
    u = Math.abs(i - l.tau2),
    p = Math.abs(o.I2 - l.I2),
    m = "number" == typeof l.Q ? Math.abs(o.Q - l.Q) : null,
    h = d < .02,
    v = c < .02,
    g = u < .02,
    f = p < 2,
    _ = null === m || m < Math.max(.5, .02 * Math.abs(l.Q));
  const y = {
    theta: h,
    se: v,
    tau2: g,
...
function convertContinuousToEffectSize(e, t) {
  if ("SMD" === t || "HEDGES_G" === t || "Hedges g" === t) {
    const t = e.n_t + e.n_c - 2,
      n = ((e.n_t - 1) * e.sd_t * e.sd_t + (e.n_c - 1) * e.sd_c * e.sd_c) / Math.max(1, t),
      a = Math.sqrt(Math.max(n, 1e-12)),
      s = (e.mean_t - e.mean_c) / a,
      i = 1 - 3 / Math.max(1, 4 * (e.n_t + e.n_c) - 9),
      r = s * i,
      o = (e.n_t + e.n_c) / (e.n_t * e.n_c) + r * r / Math.max(2, 2 * (e.n_t + e.n_c - 2));
    return {
      ...e,
      yi: r,
      vi: o,
...
function checkSmallSampleWarnings(e, t, n) {
  ...
  return e < 3 ? a.push({
    ...
    recommendation: "Use Hartung-Knapp-Sidik-Jonkman adjustment for CIs. Interpret Ãâ€žÃ‚Â² and IÃ‚Â² with caution."
  }) : e < 10 && a.push({
    severity: "info",
    ...
    severity: "warning",
    message: "High heterogeneity (IÃ‚Â²=" + n.toFixed(1) + "%) with few studies.",
...
function assessGRADEDomains(e, t = {}) {
  ...
  return {
    ...
    final_score: i,
    certainty: r[i],
    symbol: "Ã¢Å â€¢".repeat(i) + "Ã¢Å â€“".repeat(4 - i),
    summary: summarizeGRADEDomains(n, r[i])
  }
}
...
function assessInconsistencyDomain(e) {
  ...
  let s = 0,
    i = [];
  return t > 75 || n < .001 ? (s = 2, i.push("Very high heterogeneity (IÃ‚Â²=" + t.toFixed(1) + "%)")) : (t > 50 || n < .05) && (s = 1, i.push("Substantial heterogeneity (IÃ‚Â²=" + t.toFixed(1) + "%)")), a && a.lower * a.upper < 0 && (s < 1 && (s = 1), i.push("Prediction interval crosses null")), {
...
function summarizeGRADEDomains(e, t) {
  const n = [];
...
function runValidationInteractive() {
  showToast("Running validation suite...", "info"), setTimeout(() => {
    try {
      ...
      t += '<table style="width: 100%; border-collapse: collapse; font-size: var(--text-sm);">', t += '<thead><tr style="border-bottom: 2px solid var(--border-color);">', t += '<th style="text-align: left; padding: 8px;">Dataset</th>', t += "<th>ÃŽÂ¸ Match</th><th>SE Match</th><th>Ãâ€žÃ‚Â² Match</th><th>IÃ‚Â² Match</th><th>Status</th>", t += "</tr></thead><tbody>";
      for (const n of e.results) {
        const e = n.overall_pass ? "Ã¢Å“â€¦" : "Ã¢Å¡Â Ã¯Â¸Â";
        t += '<tr style="border-bottom: 1px solid var(--border-color);">', t += '<td style="padding: 8px;">' + n.dataset + "</td>", t += '<td style="text-align: center;">' + (n.passed.theta ? "Ã¢Å“â€œ" : "Ã¢Å“â€”") + "</td>", t += '<td style="text-align: center;">' + (n.passed.se ? "Ã¢Å“â€œ" : "Ã¢Å“â€”") + "</td>", t += '<td style="text-align: center;">' + (n.passed.tau2 ? "Ã¢Å“â€œ" : "Ã¢Å“â€”") + "</td>", t += '<td style="text-align: center;">' + (n.passed.I2 ? "Ã¢Å“â€œ" : "Ã¢Å“â€”") + "</td>", t += '<td style="text-align: center;">' + e + "</td>", t += "</tr>"
      }
...
function displaySmallSampleWarnings(e, t, n) {
  ...
  let i = "";
  for (const e of a) {
    i += '<div class="alert ' + ("critical" === e.severity ? "alert--danger" : "warning" === e.severity ? "alert--warning" : "alert--info") + '" style="margin-bottom: var(--space-2);">', i += '<span class="alert__icon">' + ("critical" === e.severity ? "Ã¢Å¡Â Ã¯Â¸Â" : "warning" === e.severity ? "Ã¢Å¡Â¡" : "Ã¢â€žÂ¹Ã¯Â¸Â") + "</span>", i += '<div class="alert__content">', i += "<strong>" + e.message + "</strong>", i += '<p style="margin-top: var(--space-1); font-size: var(--text-sm);">' + e.recommendation + "</p>", i += "</div></div>"
  }
  s.innerHTML = i
}
const VALIDATION_DATASETS_EXTENDED = {
  antidepressants: {
    ...
    description: "12 trials with known high heterogeneity (IÃ‚Â² > 90%)",
    type: "continuous",
    measure: "SMD",
    studies: [{
      name: "Study A",
...
const VALIDATION_STATUS = {
  ...
  methods: {
    ...
    'estimateTau2_HSk': { status: 'implemented', tolerance: 0.001, reference: 'Hunter-Schmidt (k-corrected)' },
    'estimateTau2_ProfileLikelihood': { status: 'implemented', tolerance: 0.001, reference: 'Profile likelihood tau2' },
...
const BCG_REFERENCE = {
  ...
  tau2: { DL: 0.308760, REML: 0.313243, ML: 0.280028, HE: 0.328564, HSk: 0.249172 },
...
function runAutomatedTests() {
  ...
  // Test 4b: HSk tau2
  try {
    const r = estimateTau2_HSk(yi, vi);
    const pass = Math.abs(r.tau2 - BCG_REFERENCE.tau2.HSk) < 0.001;
    results.tests.push({ name: 'HSk tau2', passed: pass, value: r.tau2, expected: BCG_REFERENCE.tau2.HSk });
    if (pass) results.passed++; else results.failed++;
  } catch(e) { results.failed++; results.tests.push({ name: 'HSk tau2', passed: false, error: e.message }); }
  ...
  try {
    const r = estimateTau2_DL(yi, vi);
    const k = yi.length;
    const I2 = calculateI2Metafor(r.tau2, vi, k);
  ...
  // Test 15: Pairwise estimator consensus
  try {
    const consensus = computePairwiseEstimatorConsensus(yi, vi, 'REML');
    const pass = !!(consensus && consensus.available && consensus.consensus && isFinite(consensus.consensus.theta));
    results.tests.push({
      name: 'Estimator consensus',
      passed: pass,
      value: consensus && consensus.consensus ? consensus.consensus.theta : NaN,
      expected: 'finite'
    });
    if (pass) results.passed++; else results.failed++;
  } catch(e) { results.failed++; results.tests.push({ name: 'Estimator consensus', passed: false, error: e.message }); }

  // Test 16: Profile likelihood tau2 is finite
  try {
    const prof = estimateTau2_ProfileLikelihood(yi, vi);
    const pass = !!(prof && isFinite(prof.tau2) && prof.tau2 >= 0);
    results.tests.push({
      name: 'Profile-likelihood tau2',
      passed: pass,
      value: prof && isFinite(prof.tau2) ? prof.tau2 : NaN,
      expected: 'finite >= 0'
    });
    if (pass) results.passed++; else results.failed++;
  } catch(e) { results.failed++; results.tests.push({ name: 'Profile-likelihood tau2', passed: false, error: e.message }); }
...
function runExtendedValidationUI() {
  ...
  setTimeout(() => {
    try {
      ...
      html += results.failed === 0 ? 'Ã¢Å“â€œ All Tests Passed' : 'Ã¢Å¡Â  Some Tests Failed';
      html += '</div>';
      ...
      results.tests.forEach(function(test) {
        ...
        const statusIcon = test.passed ? '<span style="color: green;">Ã¢Å“â€œ</span>' : '<span style="color: red;">Ã¢Å“â€”</span>';
      ...
      html += 'Ã¢â‚¬Â¢ Reference: metafor R package v4.6-0<br>';
      html += 'Ã¢â‚¬Â¢ Test Dataset: BCG Vaccine (dat.bcg)<br>';
      html += 'Ã¢â‚¬Â¢ Tolerance: 0.001 for effect sizes, 0.01 for p-values<br>';
      html += 'Ã¢â‚¬Â¢ Validated: ' + new Date().toISOString().split('T')[0];
      html += '</div>';
...
function registerAdvancedMethodGlobals() {
  if ("undefined" == typeof window) return;
  window.runGOSHAnalysis = runGOSHAnalysis, window.runTSAAnalysis = runTSAAnalysis, window.runPowerAnalysis = runPowerAnalysis, window.autoRenderPowerAnalysis = autoRenderPowerAnalysis, window.runPCurveAnalysis = runPCurveAnalysis, window.runZCurveAnalysis = runZCurveAnalysis, window.runCR2Analysis = runCR2Analysis, window.runRoBMA = runRoBMA, window.runSimplifiedMA = runSimplifiedMA, window.runInfluenceDiagnostics = runInfluenceDiagnostics, window.runCopasModel = runCopasModel, window.runSmallSampleCI = runSmallSampleCI, window.runFullMCMCAnalysis = runFullMCMCAnalysis, window.runGRADEAssessment = runGRADEAssessment, window.runExtendedValidationUI = runExtendedValidationUI, window.renderContourFunnelPlot = renderContourFunnelPlot, window.renderSunsetPlot = renderSunsetPlot, window.ensureAdvancedGapCards = ensureAdvancedGapCards, window.runGLMMAdvanced = runGLMMAdvanced, window.runProfileLikelihoodAdvanced = runProfileLikelihoodAdvanced, safeTimeout(() => {
    "function" == typeof ensureAdvancedGapCards && ensureAdvancedGapCards()
  }, 0)
}
...
function renderRadialPlot(results, containerId = 'radialPlot') {
  try {
    ...
    // 95% CI bounds (slope = pooled Ã‚Â± 1.96)
    const ciUpper = {
      x: [0, maxPrecision],
...
// =============================================================================
// L'ABBÃƒâ€° PLOT
// Reference: L'AbbÃƒÂ© KA, Detsky AS, O'Rourke K. Meta-analysis in clinical research.
...
function renderLabbePlot(results, containerId = 'labbePlot') {
  try {
    if (typeof Plotly === 'undefined') {
      console.error('Plotly not loaded for L\'AbbÃƒÂ© plot');
      return;
    }

    const container = document.getElementById(containerId);
    if (!container) {
      console.error('L\'AbbÃƒÂ© plot container not found:', containerId);
      return;
    }
    ...
    if (!studies.length || studies[0].events_t === undefined) {
      showPlotError(containerId, 'L\'AbbÃƒÂ© plot requires binary outcome data (events and totals)');
      return;
    }
    ...
    if (data.length === 0) {
      showPlotError(containerId, 'No valid studies with binary data for L\'AbbÃƒÂ© plot');
      return;
    }
    ...
    console.log('L\'AbbÃƒÂ© plot rendered successfully');

  } catch (e) {
    console.error('L\'AbbÃƒÂ© plot rendering failed:', e);
    showPlotError(containerId, 'Failed to render L\'AbbÃƒÂ© plot: ' + e.message);
  }
}
...
// Export Critical Gap functions (Mantel-Haenszel, Peto, Cook's Distance)
if (typeof window !== 'undefined') {
  window.mantelHaenszel = mantelHaenszel;
  window.petoMethod = petoMethod;
  window.cookDistance = cookDistance;
  window.convertToABCD = convertToABCD;
  window.testExcessSignificance = testExcessSignificance;
  window.getStandardizedResiduals = getStandardizedResiduals;
  window.renderQQPlot = renderQQPlot;
  window.calculatePearsonCorrelation = calculatePearsonCorrelation;
}

console.log('Critical Gap functions loaded: Mantel-Haenszel, Peto, Cook\'s Distance, TES, QQ Plot');
...
function validateCriticalGapFunctions() {
  ...
  try {
    ...
    if (passOR) {
      results.passed++;
      console.log('Ã¢Å“â€œ Mantel-Haenszel OR: PASS (' + mhOR.estimate.toFixed(4) + ' vs R ' + refOR.estimate.toFixed(4) + ')');
    } else {
      results.failed++;
      console.log('Ã¢Å“â€” Mantel-Haenszel OR: FAIL (' + mhOR.estimate.toFixed(4) + ' vs R ' + refOR.estimate.toFixed(4) + ')');
    }
  } catch (e) {
    results.failed++;
    results.tests.push({ name: 'Mantel-Haenszel OR', passed: false, error: e.message });
    console.log('Ã¢Å“â€” Mantel-Haenszel OR: ERROR - ' + e.message);
  }
  ...
  try {
    ...
    if (passRR) {
      results.passed++;
      console.log('Ã¢Å“â€œ Mantel-Haenszel RR: PASS (' + mhRR.estimate.toFixed(4) + ' vs R ' + refRR.estimate.toFixed(4) + ')');
    } else {
      results.failed++;
      console.log('Ã¢Å“â€” Mantel-Haenszel RR: FAIL');
    }
  } catch (e) {
    results.failed++;
  ...
  try {
    ...
    if (passPeto) {
      results.passed++;
      console.log('Ã¢Å“â€œ Peto OR: PASS (' + peto.estimate.toFixed(4) + ' vs R ' + refPeto.estimate.toFixed(4) + ')');
    } else {
      results.failed++;
      console.log('Ã¢Å“â€” Peto OR: FAIL');
    }
    ...
    console.log((passI2 ? 'Ã¢Å“â€œ' : 'Ã¢Å“â€”') + ' Peto IÃ‚Â²: ' + peto.heterogeneity.I2.toFixed(1) + '% vs R ' + refPeto.I2.toFixed(1) + '%');
  } catch (e) {
    results.failed++;
  ...
  try {
    ...
    if (passMaxStudy) {
      results.passed++;
      console.log("Ã¢Å“â€œ Cook's Distance: Max influential study = " + cook.maxCookStudy + ' (matches R)');
    } else {
      results.failed++;
      console.log("Ã¢Å“â€” Cook's Distance: Max study mismatch");
    }
  } catch (e) {
    results.failed++;
  ...
  try {
    ...
    results.passed++;
    console.log('Ã¢Å“â€œ TES: O=' + tes.observed + ', E=' + tes.expected.toFixed(2) + ', p=' + tes.pvalue.toFixed(4));
  } catch (e) {
    results.failed++;
...
// ============================================================================
// INNOVATIVE FEATURES - Beyond metafor
// Version 2.0 - January 2026
// ============================================================================

// ============================================================================
// CATEGORY 1: ROBUST BAYESIAN META-ANALYSIS (RoBMA)
// Reference: BartoÃ…Â¡ et al. (2023) Research Synthesis Methods
// ============================================================================

/**
 * Robust Bayesian Meta-Analysis with Model Averaging
 * Implements simplified RoBMA framework
 */
function robmaModelAveraging(yi, vi, options = {}) {
  const k = yi.length;
  const nSamples = options.nSamples || 5000;
  const priorEffect = options.priorEffect || { type: 'normal', mean: 0, sd: 1 };
  const priorTau = options.priorTau || { type: 'halfCauchy', scale: 0.5 };

  // Define model space: 2 (effect) x 2 (heterogeneity) x 2 (bias) = 8 models
  const models = [
    { hasEffect: false, hasHet: false, hasBias: false, name: 'Null' },
    { hasEffect: true, hasHet: false, hasBias: false, name: 'FE' },
    { hasEffect: false, hasHet: true, hasBias: false, name: 'Het only' },
    { hasEffect: true, hasHet: true, hasBias: false, name: 'RE' },
    { hasEffect: false, hasHet: false, hasBias: true, name: 'Bias only' },
    { hasEffect: true, hasHet: false, hasBias: true, name: 'FE+Bias' },
    { hasEffect: false, hasHet: true, hasBias: true, name: 'Het+Bias' },
    { hasEffect: true, hasHet: true, hasBias: true, name: 'RE+Bias' }
  ];

  // Compute marginal likelihoods (BIC approximation)
  const sei = vi.map(v => Math.sqrt(v));
  const results = models.map(model => {
    ...
    if (model.hasEffect || model.hasHet) {
      const w = vi.map(v => 1 / v);
      const sumW = w.reduce((a, b) => a + b, 0);
      theta = model.hasEffect ? yi.reduce((s, y, i) => s + w[i] * y, 0) / sumW : 0;
      nParams += model.hasEffect ? 1 : 0;

      if (model.hasHet) {
        const Q = yi.reduce((s, y, i) => s + w[i] * Math.pow(y - theta, 2), 0);
    ...
    for (let i = 0; i < k; i++) {
      ...
      const residual = yi[i] - theta;
      logLik -= 0.5 * (Math.log(2 * Math.PI * totalVar) + residual * residual / totalVar);

      // Selection model adjustment for bias
      if (model.hasBias) {
        const z = yi[i] / sei[i];
        const pval = 2 * (1 - pnorm(Math.abs(z)));
        const selectionProb = pval < 0.05 ? 1 : 0.3; // Step function
        logLik += Math.log(selectionProb);
      }
    }

    // BIC approximation to marginal likelihood
    const bic = -2 * logLik + nParams * Math.log(k);
    const marginalLogLik = -bic / 2;
  ...
  const maxMargLik = Math.max(...results.map(r => r.marginalLogLik));
  const unnormProbs = results.map(r => Math.exp(r.marginalLogLik - maxMargLik));
  const sumProbs = unnormProbs.reduce((a, b) => a + b, 0);
  const posteriorProbs = unnormProbs.map(p => p / sumProbs);

  results.forEach((r, i) => r.posteriorProb = posteriorProbs[i]);

  // Model-averaged estimates
  const avgTheta = results.reduce((s, r) => s + r.posteriorProb * r.theta, 0);
  const avgTau2 = results.reduce((s, r) => s + r.posteriorProb * r.tau2, 0);

  // Compute Bayes Factors (with safeguards against division by zero)
  const probEffect = results.filter(r => r.hasEffect).reduce((s, r) => s + r.posteriorProb, 0);
  const probNoEffect = Math.max(1 - probEffect, 1e-10);
  const BF10_effect = probEffect / probNoEffect;

  const probHet = results.filter(r => r.hasHet).reduce((s, r) => s + r.posteriorProb, 0);
  const probNoHet = Math.max(1 - probHet, 1e-10);
  const BF10_het = probHet / probNoHet;

  const probBias = results.filter(r => r.hasBias).reduce((s, r) => s + r.posteriorProb, 0);
  const probNoBias = Math.max(1 - probBias, 1e-10);
  const BF10_bias = probBias / probNoBias;

  return {
    method: 'Robust Bayesian Meta-Analysis (RoBMA)',
    models: results.sort((a, b) => b.posteriorProb - a.posteriorProb),
    modelAveraged: {
      theta: avgTheta,
      tau2: avgTau2,
    ...
    bayesFactors: {
      effect: { BF10: BF10_effect, interpretation: interpretBF(BF10_effect) },
      heterogeneity: { BF10: BF10_het, interpretation: interpretBF(BF10_het) },
      publicationBias: { BF10: BF10_bias, interpretation: interpretBF(BF10_bias) }
    },
    posteriorProbabilities: {
      effect: probEffect,
      noEffect: probNoEffect,
      heterogeneity: probHet,
      publicationBias: probBias
    },
    bestModel: results[0],
    reference: 'BartoÃ…Â¡ et al. (2023) Research Synthesis Methods'
  };
...
/**
 * Meta-analytic Z-curve for publication bias assessment
 */
function metaAnalyticZCurve(yi, vi) {
  const k = yi.length;
  const sei = vi.map(v => Math.sqrt(v));
  const z = yi.map((y, i) => y / sei[i]);
  const pvals = z.map(zz => 2 * (1 - pnorm(Math.abs(zz))));

  // Count significant results
  const nSig = pvals.filter(p => p < 0.05).length;
  const observedDiscoveryRate = nSig / k;

  // Expected discovery rate under various power scenarios
  const zCritical = 1.96;
  const significantZ = z.filter((zz, i) => pvals[i] < 0.05).map(Math.abs);

  // Estimate mean power from significant z-values
  let expectedDiscoveryRate = 0;
  if (significantZ.length > 0) {
    const meanSigZ = significantZ.reduce((a, b) => a + b, 0) / significantZ.length;
    const estimatedNCP = meanSigZ; // Non-centrality parameter
    expectedDiscoveryRate = 1 - pnorm(zCritical - estimatedNCP);
  }

  // Expected replicability
  const expectedReplicability = significantZ.length > 0
    ? significantZ.reduce((s, zz) => s + (1 - pnorm(zCritical - zz)), 0) / significantZ.length
    : 0;

  // File drawer ratio
  const fileDrawerRatio = expectedDiscoveryRate > 0
    ? (observedDiscoveryRate - expectedDiscoveryRate) / expectedDiscoveryRate
    : 0;

  // Z-curve density estimation (simplified)
  const zBins = [];
  for (let zz = 0; zz <= 6; zz += 0.2) {
    const count = z.filter(zzz => Math.abs(zzz) >= zz && Math.abs(zzz) < zz + 0.2).length;
    zBins.push({ z: zz + 0.1, count, density: count / k });
  }

  return {
    method: 'Meta-analytic Z-curve',
    k,
    nSignificant: nSig,
    observedDiscoveryRate,
    expectedDiscoveryRate,
    expectedReplicability,
    fileDrawerRatio,
    zDistribution: zBins,
    interpretation: fileDrawerRatio > 0.5
      ? 'Substantial evidence of publication bias'
      : fileDrawerRatio > 0.2
        ? 'Moderate evidence of publication bias'
        : 'Little evidence of publication bias',
    reference: 'BartoÃ…Â¡ & Schimmack (2025)'
  };
}

// ============================================================================
// CATEGORY 2: MULTIVERSE & SPECIFICATION CURVE ANALYSIS
// Reference: Voracek et al. (2019), GÃƒÂ¶tz (2024)
// ============================================================================

/**
 * Multiverse Meta-Analysis - Run all 2^k study subsets
 */
function multiverseMetaAnalysis(yi, vi, names = null, maxSubsets = 1000) {
  const k = yi.length;
  const totalSubsets = Math.pow(2, k) - 1;

  // For large k, sample randomly
  const sampleSize = Math.min(totalSubsets, maxSubsets);
  const subsets = [];
  ...
  if (totalSubsets <= maxSubsets) {
    ...
    while (subsets.length < sampleSize) {
      const size = Math.floor(Math.random() * (k - 1)) + 2; // At least 2 studies
      const indices = [];
  ...
  // Run meta-analysis on each subset
  const results = subsets.map(indices => {
    const subYi = indices.map(i => yi[i]);
    const subVi = indices.map(i => vi[i]);
    const subNames = names ? indices.map(i => names[i]) : indices.map(i => `Study ${i + 1}`);

    // DerSimonian-Laird
    const w = subVi.map(v => 1 / v);
    const sumW = w.reduce((a, b) => a + b, 0);
    const theta_fe = subYi.reduce((s, y, i) => s + w[i] * y, 0) / sumW;
    const Q = subYi.reduce((s, y, i) => s + w[i] * Math.pow(y - theta_fe, 2), 0);
    const c = sumW - w.reduce((a, b) => a + b * b, 0) / sumW;
    const tau2 = Math.max(0, (Q - (indices.length - 1)) / c);

    const w_re = subVi.map(v => 1 / (v + tau2));
    const sumW_re = w_re.reduce((a, b) => a + b, 0);
    const theta = subYi.reduce((s, y, i) => s + w_re[i] * y, 0) / sumW_re;
    const se = Math.sqrt(1 / sumW_re);
    const z = theta / se;
    const pval = 2 * (1 - pnorm(Math.abs(z)));

    const I2 = indices.length > 1 ? Math.max(0, (Q - (indices.length - 1)) / Q * 100) : 0;
  ...
  // Statistics across multiverse
  const thetas = results.map(r => r.theta);
  const meanTheta = thetas.reduce((a, b) => a + b, 0) / thetas.length;
  const sdTheta = Math.sqrt(thetas.reduce((s, t) => s + Math.pow(t - meanTheta, 2), 0) / thetas.length);
  const minTheta = Math.min(...thetas);
  const maxTheta = Math.max(...thetas);

  const nSignificant = results.filter(r => r.significant).length;
  const proportionSignificant = nSignificant / results.length;

  // Sign consistency
  const nPositive = thetas.filter(t => t > 0).length;
  const signConsistency = Math.max(nPositive, results.length - nPositive) / results.length;

  // Specification curve data (sorted by effect size)
  const specCurve = results.sort((a, b) => a.theta - b.theta).map((r, i) => ({
    rank: i + 1,
    ...r
  }));

  return {
    method: 'Multiverse Meta-Analysis',
    k,
    totalPossibleSubsets: totalSubsets,
    analyzedSubsets: results.length,
    summary: {
      meanEffect: meanTheta,
      sdEffect: sdTheta,
      minEffect: minTheta,
      maxEffect: maxTheta,
      medianEffect: thetas.sort((a, b) => a - b)[Math.floor(thetas.length / 2)],
      range: maxTheta - minTheta
    },
    robustness: {
      proportionSignificant,
      signConsistency,
      interpretation: signConsistency > 0.95
        ? 'Very robust - consistent direction across specifications'
        : signConsistency > 0.8
          ? 'Robust - mostly consistent direction'
          : signConsistency > 0.6
            ? 'Moderate robustness - some inconsistency'
            : 'Not robust - effect direction depends on specification'
    },
    specificationCurve: specCurve,
    vibrationOfEffects: {
      VoE: sdTheta,
      interpretation: sdTheta < 0.1 ? 'Low vibration' : sdTheta < 0.3 ? 'Moderate vibration' : 'High vibration'
    },
    reference: 'Voracek et al. (2019) Zeitschrift fÃƒÂ¼r Psychologie'
  };
}
 ...
 */
function specificationCurveAnalysis(yi, vi, names = null, options = {}) {
  const multiverse = multiverseMetaAnalysis(yi, vi, names, options.maxSubsets || 500);

  // Identify which studies drive the results
  const studyInfluence = [];
  for (let i = 0; i < yi.length; i++) {
    const withStudy = multiverse.specificationCurve.filter(r => r.indices.includes(i));
    const withoutStudy = multiverse.specificationCurve.filter(r => !r.indices.includes(i));

    const meanWith = withStudy.length > 0
      ? withStudy.reduce((s, r) => s + r.theta, 0) / withStudy.length : 0;
    const meanWithout = withoutStudy.length > 0
      ? withoutStudy.reduce((s, r) => s + r.theta, 0) / withoutStudy.length : 0;

    studyInfluence.push({
      study: names ? names[i] : `Study ${i + 1}`,
      index: i,
      meanEffectWith: meanWith,
      meanEffectWithout: meanWithout,
      influence: meanWith - meanWithout,
      absInfluence: Math.abs(meanWith - meanWithout)
    });
  }

  studyInfluence.sort((a, b) => b.absInfluence - a.absInfluence);

  return {
    method: 'Specification Curve Analysis',
    multiverse,
    studyInfluence,
    mostInfluentialStudy: studyInfluence[0],
    leastInfluentialStudy: studyInfluence[studyInfluence.length - 1],
    reference: 'Simonsohn et al. (2020) Nature Human Behaviour'
  };
}
 ...
 */
function vibrationOfEffects(yi, vi) {
  const multiverse = multiverseMetaAnalysis(yi, vi, null, 200);
  const effects = multiverse.specificationCurve.map(r => r.theta);

  // Jitter ratio (Patel et al.)
  const minE = Math.min(...effects);
  const maxE = Math.max(...effects);
  const jitterRatio = maxE !== 0 ? (maxE - minE) / Math.abs(maxE) : 0;

  // Proteus phenomenon check
  const fullAnalysis = multiverse.specificationCurve.find(r => r.k === yi.length);
  const oppositeSign = effects.filter(e => (e > 0) !== (fullAnalysis.theta > 0)).length;
  const proteusIndex = oppositeSign / effects.length;

  return {
    method: 'Vibration of Effects',
    vibrationSD: multiverse.summary.sdEffect,
    jitterRatio,
    proteusIndex,
    interpretation: {
      vibration: multiverse.vibrationOfEffects.interpretation,
      proteus: proteusIndex > 0.1 ? 'Proteus phenomenon detected - effect reverses in some specifications' : 'No Proteus phenomenon'
    },
    reference: 'Patel et al. (2015) International Journal of Epidemiology'
  };
}

// ============================================================================
// CATEGORY 3: ADVANCED PREDICTION INTERVALS
// Reference: Noma (2023) Research Synthesis Methods
// ============================================================================

/**
 * Improved Prediction Interval (Noma method)
 */
function improvedPredictionInterval(yi, vi, tau2 = null, alpha = 0.05) {
  const k = yi.length;

  // Estimate tau2 if not provided (REML)
  if (tau2 === null) {
  ...
  // Standard prediction interval (HTS)
  const varTheta = 1 / sumW;
  const sePred_hts = Math.sqrt(varTheta + tau2);
  const tCrit = qt(1 - alpha / 2, k - 2);
  const pi_hts = {
    lower: theta - tCrit * sePred_hts,
    upper: theta + tCrit * sePred_hts
  };

  // Improved method (Noma) - accounts for tau2 estimation uncertainty
  const Q = yi.reduce((s, y, i) => s + w[i] * Math.pow(y - theta, 2), 0);
  const c = sumW - w.reduce((a, b) => a + b * b, 0) / sumW;

  // Variance of tau2 estimate
  const varTau2 = 2 / (c * c) * (k - 1 + 2 * (Q - (k - 1)) * c / sumW);

  // Adjusted prediction variance
  const adjFactor = 1 + varTau2 / (2 * Math.pow(tau2 + varTheta, 2));
  const sePred_noma = Math.sqrt((varTheta + tau2) * adjFactor);

  const pi_noma = {
    lower: theta - tCrit * sePred_noma,
    upper: theta + tCrit * sePred_noma
  };
  ...
  for (let b = 0; b < nBoot; b++) {
    const bootIndices = Array.from({ length: k }, () => Math.floor(Math.random() * k));
    const bootYi = bootIndices.map(i => yi[i]);
    const bootVi = bootIndices.map(i => vi[i]);
    const bootW = bootVi.map(v => 1 / (v + tau2));
    const bootSumW = bootW.reduce((a, b) => a + b, 0);
    const bootTheta = bootYi.reduce((s, y, i) => s + bootW[i] * y, 0) / bootSumW;
    // Add prediction error
    const predError = randomNormal(0, Math.sqrt(tau2));
    bootThetas.push(bootTheta + predError);
  }
  bootThetas.sort((a, b) => a - b);
  const pi_boot = {
    lower: bootThetas[Math.floor(nBoot * alpha / 2)],
    upper: bootThetas[Math.floor(nBoot * (1 - alpha / 2))]
  };

  return {
    method: 'Improved Prediction Intervals',
    theta,
    tau2,
    standardPI: pi_hts,
    improvedPI: pi_noma,
    bootstrapPI: pi_boot,
    improvement: {
      widthRatio: (pi_noma.upper - pi_noma.lower) / (pi_hts.upper - pi_hts.lower),
      note: 'Improved PI accounts for tauÃ‚Â² estimation uncertainty'
    },
    reference: 'Noma (2023) Research Synthesis Methods'
  };
}
...
/**
 * Simple random normal generator
 */
 ...
 */
function calibratedPredictionInterval(yi, vi, tau2 = null, targetCoverage = 0.95) {
  const k = yi.length;
  ...
  // Calibration via leave-one-out cross-validation
  const looResiduals = [];
  for (let i = 0; i < k; i++) {
    const looYi = yi.filter((_, j) => j !== i);
    const looVi = vi.filter((_, j) => j !== i);
    const looW = looVi.map(v => 1 / (v + tau2));
    const looSumW = looW.reduce((a, b) => a + b, 0);
    const looTheta = looYi.reduce((s, y, j) => s + looW[j] * y, 0) / looSumW;
    looResiduals.push(yi[i] - looTheta);
  }

  // Empirical variance of prediction errors
  const empVar = looResiduals.reduce((s, r) => s + r * r, 0) / k;
  const calibrationFactor = Math.sqrt(empVar / (varTheta + tau2));

  const sePred = Math.sqrt(varTheta + tau2) * calibrationFactor;
  const zCrit = qnorm(1 - (1 - targetCoverage) / 2);

  return {
    method: 'Calibrated Prediction Interval',
    theta,
    tau2,
    calibrationFactor,
    predictionInterval: {
      lower: theta - zCrit * sePred,
      upper: theta + zCrit * sePred
    },
    targetCoverage,
    note: calibrationFactor > 1.2
      ? 'Standard PI likely has under-coverage; calibration applied'
      : 'Standard PI has adequate coverage'
  };
}

// ============================================================================
// CATEGORY 4: LIVING REVIEW FEATURES
// Reference: Mayo Clinic Proceedings: Digital Health 2024
// ============================================================================

/**
 * Sequential Meta-Analysis with monitoring boundaries
 */
function sequentialMetaAnalysis(yi, vi, names = null, options = {}) {
  const k = yi.length;
  const alpha = options.alpha || 0.05;
  const beta = options.beta || 0.2; // Type II error
  const delta = options.expectedEffect || 0.3; // Expected effect size

  const cumulative = [];
  let cumulativeZ = 0;
  let cumulativeVar = 0;
  let conclusionReached = false;
  let conclusionStudy = null;
  let conclusionType = null;

  // O'Brien-Fleming boundaries
  const obfBoundary = (info, maxInfo) => {
    const infoFrac = info / maxInfo;
    return qnorm(1 - alpha / 2) / Math.sqrt(infoFrac);
  };

  // Calculate information size (required studies for 80% power)
  const avgVar = vi.reduce((a, b) => a + b, 0) / k;
  const requiredInfo = Math.pow(qnorm(1 - alpha / 2) + qnorm(1 - beta), 2) / (delta * delta);
  const informationSize = Math.ceil(requiredInfo * avgVar);

  for (let i = 0; i < k; i++) {
    const currentYi = yi.slice(0, i + 1);
    const currentVi = vi.slice(0, i + 1);

    // Fixed effect cumulative
    const w = currentVi.map(v => 1 / v);
    const sumW = w.reduce((a, b) => a + b, 0);
    const theta = currentYi.reduce((s, y, j) => s + w[j] * y, 0) / sumW;
    const se = Math.sqrt(1 / sumW);
    const z = theta / se;
    const pval = 2 * (1 - pnorm(Math.abs(z)));

    // Information fraction
    const currentInfo = sumW;
    const infoFraction = currentInfo / (informationSize / avgVar);

    // Monitoring boundaries
    const upperBoundary = obfBoundary(infoFraction, 1);
    const lowerBoundary = -upperBoundary;
    const futilityBoundary = delta * Math.sqrt(currentInfo) * 0.5; // Simplified

    const crossedEfficacy = Math.abs(z) > upperBoundary;
    const crossedFutility = Math.abs(z) < futilityBoundary && infoFraction > 0.5;

    if (!conclusionReached && crossedEfficacy) {
      conclusionReached = true;
      conclusionStudy = i + 1;
      conclusionType = 'Efficacy boundary crossed';
    }
    if (!conclusionReached && crossedFutility) {
      conclusionReached = true;
      conclusionStudy = i + 1;
      conclusionType = 'Futility boundary crossed';
    }

    cumulative.push({
      study: i + 1,
      name: names ? names[i] : `Study ${i + 1}`,
      theta,
      se,
      z,
      pval,
      ...
      crossedEfficacy,
      crossedFutility,
      conclusionReached: conclusionReached && conclusionStudy <= i + 1
    });
  }

  return {
    method: 'Sequential Meta-Analysis',
    k,
    informationSize,
    informationFractionAchieved: cumulative[k - 1].infoFraction,
    cumulative,
    conclusion: {
      reached: conclusionReached,
      study: conclusionStudy,
      type: conclusionType,
      finalEffect: cumulative[k - 1].theta,
      finalPval: cumulative[k - 1].pval
    },
    recommendation: !conclusionReached
      ? `Continue searching - ${Math.round((1 - cumulative[k - 1].infoFraction) * 100)}% more information needed`
      : conclusionType.includes('Efficacy')
        ? 'Sufficient evidence for effect - consider stopping'
        : 'Evidence suggests no meaningful effect - consider stopping',
    reference: 'Wetterslev et al. (2008) J Clin Epidemiol'
  };
}

/**
 * Information Size calculation for meta-analysis
 */
function calculateInformationSize(expectedEffect, alpha = 0.05, power = 0.8, avgVariance = null) {
  const za = qnorm(1 - alpha / 2);
  const zb = qnorm(power);

  const requiredD = Math.pow(za + zb, 2) / Math.pow(expectedEffect, 2);

  return {
    method: 'Information Size Calculation',
    expectedEffect,
    alpha,
    power,
    requiredInformationUnits: requiredD,
    estimatedStudies: avgVariance ? Math.ceil(requiredD * avgVariance) : null,
    formula: 'D = (zÃŽÂ±/2 + zÃŽÂ²)Ã‚Â² / ÃŽÂ´Ã‚Â²',
    reference: 'Wetterslev et al. (2008)'
  };
}

/**
 * Living Review Status Assessment
 */
function livingReviewStatus(yi, vi, previousAnalysis = null) {
  const current = {
    k: yi.length,
    w: vi.map(v => 1 / v),
  };
  current.sumW = current.w.reduce((a, b) => a + b, 0);
  current.theta = yi.reduce((s, y, i) => s + current.w[i] * y, 0) / current.sumW;
  current.se = Math.sqrt(1 / current.sumW);
  current.pval = 2 * (1 - pnorm(Math.abs(current.theta / current.se)));
  ...
  if (previousAnalysis) {
    const effectChange = Math.abs(current.theta - previousAnalysis.theta);
    const seChange = current.se / previousAnalysis.se;

    // Detect meaningful changes
    if (effectChange > 1.96 * Math.sqrt(current.se * current.se + previousAnalysis.se * previousAnalysis.se)) {
      changeDetected = true;
      changeType = 'Effect size changed significantly';
      changeDetails = { previous: previousAnalysis.theta, current: current.theta, change: effectChange };
    }

    // Detect conclusion change
    const prevSig = previousAnalysis.pval < 0.05;
    const currSig = current.pval < 0.05;
    if (prevSig !== currSig) {
      changeDetected = true;
      changeType = currSig ? 'Became significant' : 'Lost significance';
    }

    // Detect precision improvement
    if (seChange < 0.8) {
      changeDetails = { ...changeDetails, precisionImprovement: `${Math.round((1 - seChange) * 100)}% reduction in SE` };
    }
  }

  return {
    method: 'Living Review Status',
    currentAnalysis: {
      k: current.k,
      theta: current.theta,
      se: current.se,
      pval: current.pval,
      significant: current.pval < 0.05
    },
    previousAnalysis,
    changeDetected,
    changeType,
    changeDetails,
    recommendation: changeDetected
      ? 'UPDATE REQUIRED: Significant change detected since last analysis'
      : 'No update needed - conclusions stable',
    timestamp: new Date().toISOString()
  };
}

// ============================================================================
// CATEGORY 5: ADVANCED HETEROGENEITY
// ============================================================================

/**
 * R-squared for Meta-Regression
 */
function metaRegressionR2(yi, vi, moderator) {
  const k = yi.length;

  // Random effects without moderator
  const w0 = vi.map(v => 1 / v);
  const sumW0 = w0.reduce((a, b) => a + b, 0);
  const theta0 = yi.reduce((s, y, i) => s + w0[i] * y, 0) / sumW0;
  const Q0 = yi.reduce((s, y, i) => s + w0[i] * Math.pow(y - theta0, 2), 0);
  const c0 = sumW0 - w0.reduce((a, b) => a + b * b, 0) / sumW0;
  const tau2_0 = Math.max(0, (Q0 - (k - 1)) / c0);

  // Meta-regression with moderator
  const x = moderator;
  const xMean = x.reduce((a, b) => a + b, 0) / k;
  const xCentered = x.map(xx => xx - xMean);

  // Weighted least squares
  const sumWX2 = xCentered.reduce((s, xx, i) => s + w0[i] * xx * xx, 0);
  const sumWXY = xCentered.reduce((s, xx, i) => s + w0[i] * xx * yi[i], 0);
  const beta1 = sumWXY / sumWX2;
  const beta0 = theta0 - beta1 * xMean;

  const predicted = x.map(xx => beta0 + beta1 * xx);
  const Q1 = yi.reduce((s, y, i) => s + w0[i] * Math.pow(y - predicted[i], 2), 0);
  const tau2_1 = Math.max(0, (Q1 - (k - 2)) / c0);

  // R-squared (proportion of between-study variance explained)
  const R2 = tau2_0 > 0 ? Math.max(0, 1 - tau2_1 / tau2_0) : 0;

  // Adjusted R-squared
  const adjR2 = 1 - (1 - R2) * (k - 1) / (k - 2);

  return {
    method: 'Meta-Regression RÃ‚Â²',
    tau2_without: tau2_0,
    tau2_with: tau2_1,
    R2,
    adjustedR2: Math.max(0, adjR2),
    regression: {
      intercept: beta0,
      slope: beta1
    },
    interpretation: R2 > 0.75
      ? 'Moderator explains most heterogeneity'
      : R2 > 0.5
        ? 'Moderator explains substantial heterogeneity'
        : R2 > 0.25
          ? 'Moderator explains some heterogeneity'
          : 'Moderator explains little heterogeneity',
    reference: 'LÃƒÂ³pez-LÃƒÂ³pez et al. (2014)'
  };
}

/**
 * Permutation test for IÃ‚Â²
 */
function permutationTestI2(yi, vi, nPerm = 1000) {
  const k = yi.length;
  ...
  const theta = yi.reduce((s, y, i) => s + w[i] * y, 0) / sumW;
  const Q = yi.reduce((s, y, i) => s + w[i] * Math.pow(y - theta, 2), 0);
  const I2_obs = Math.max(0, (Q - (k - 1)) / Q * 100);

  // Permutation distribution
  const permI2 = [];
  for (let p = 0; p < nPerm; p++) {
    // Shuffle yi values
    const shuffled = [...yi].sort(() => Math.random() - 0.5);
    const permTheta = shuffled.reduce((s, y, i) => s + w[i] * y, 0) / sumW;
    const permQ = shuffled.reduce((s, y, i) => s + w[i] * Math.pow(y - permTheta, 2), 0);
    const permI2val = Math.max(0, (permQ - (k - 1)) / permQ * 100);
    permI2.push(permI2val);
  }

  permI2.sort((a, b) => a - b);

  // P-value (proportion of permuted IÃ‚Â² >= observed)
  const pval = permI2.filter(i2 => i2 >= I2_obs).length / nPerm;

  // Confidence interval
  const ci_lower = permI2[Math.floor(nPerm * 0.025)];
  const ci_upper = permI2[Math.floor(nPerm * 0.975)];

  return {
    method: 'Permutation Test for IÃ‚Â²',
    I2_observed: I2_obs,
    pvalue: pval,
    permutationCI: { lower: ci_lower, upper: ci_upper },
    nPermutations: nPerm,
    significant: pval < 0.05,
    interpretation: pval < 0.05
      ? 'Significant heterogeneity detected (permutation test)'
      : 'No significant heterogeneity (permutation test)'
  };
}

/**
 * Outlier-Robust tauÃ‚Â² estimation
 */
function robustTau2(yi, vi, method = 'winsorized') {
  const k = yi.length;
  const w = vi.map(v => 1 / v);
  const sumW = w.reduce((a, b) => a + b, 0);
  const theta = yi.reduce((s, y, i) => s + w[i] * y, 0) / sumW;
  ...
  const residuals = yi.map((y, i) => (y - theta) / Math.sqrt(vi[i] + tau2_dl));

  let tau2_robust;

  if (method === 'winsorized') {
    // Winsorize extreme residuals
    const sortedRes = [...residuals].sort((a, b) => a - b);
    const lower = sortedRes[Math.floor(k * 0.1)];
    const upper = sortedRes[Math.floor(k * 0.9)];

    const winsorized = yi.map((y, i) => {
      const r = residuals[i];
      if (r < lower) return theta + lower * Math.sqrt(vi[i] + tau2_dl);
      if (r > upper) return theta + upper * Math.sqrt(vi[i] + tau2_dl);
      return y;
    });

    const thetaW = winsorized.reduce((s, y, i) => s + w[i] * y, 0) / sumW;
    const QW = winsorized.reduce((s, y, i) => s + w[i] * Math.pow(y - thetaW, 2), 0);
    tau2_robust = Math.max(0, (QW - (k - 1)) / c);
  } else if (method === 'trimmed') {
    // Trim 10% from each end
    const sorted = yi.map((y, i) => ({ y, v: vi[i], w: w[i] }))
      .sort((a, b) => a.y - b.y);
    const trimN = Math.floor(k * 0.1);
    const trimmed = sorted.slice(trimN, k - trimN);

    const trimSumW = trimmed.reduce((s, d) => s + d.w, 0);
    const thetaT = trimmed.reduce((s, d) => s + d.w * d.y, 0) / trimSumW;
    const QT = trimmed.reduce((s, d) => s + d.w * Math.pow(d.y - thetaT, 2), 0);
    const cT = trimSumW - trimmed.reduce((s, d) => s + d.w * d.w, 0) / trimSumW;
    tau2_robust = Math.max(0, (QT - (trimmed.length - 1)) / cT);
  }

  return {
    method: `Outlier-Robust tauÃ‚Â² (${method})`,
    tau2_standard: tau2_dl,
    tau2_robust,
    reduction: tau2_dl > 0 ? (tau2_dl - tau2_robust) / tau2_dl * 100 : 0,
    interpretation: tau2_robust < tau2_dl * 0.8
      ? 'Substantial reduction - outliers inflating heterogeneity'
      : 'Minimal reduction - heterogeneity not driven by outliers'
  };
}

// ============================================================================
// CATEGORY 6: NOVEL PUBLICATION BIAS METHODS
// ============================================================================

/**
 * 3-Parameter Selection Model (Vevea-Hedges)
 */
function threeParameterSelectionModel(yi, vi, severity = 'moderate') {
  const k = yi.length;
  const sei = vi.map(v => Math.sqrt(v));
  const z = yi.map((y, i) => y / sei[i]);
  const pvals = z.map(zz => 2 * (1 - pnorm(Math.abs(zz))));

  // Selection weights by severity
  const weights = {
    mild: { sig: 1, marginal: 0.8, nonsig: 0.6 },
    moderate: { sig: 1, marginal: 0.5, nonsig: 0.2 },
    severe: { sig: 1, marginal: 0.3, nonsig: 0.05 }
  };

  const w = weights[severity] || weights.moderate;

  // Assign selection probabilities
  const selProb = pvals.map(p => {
    if (p < 0.025) return w.sig;      // Two-tailed sig
    if (p < 0.10) return w.marginal;  // Marginally significant
    return w.nonsig;                   // Non-significant
  });

  // Weighted analysis
  const adjW = vi.map((v, i) => selProb[i] / v);
  const sumAdjW = adjW.reduce((a, b) => a + b, 0);
  const thetaAdj = yi.reduce((s, y, i) => s + adjW[i] * y, 0) / sumAdjW;
  const seAdj = Math.sqrt(1 / sumAdjW);

  // Unadjusted
  const wOrig = vi.map(v => 1 / v);
  const sumWOrig = wOrig.reduce((a, b) => a + b, 0);
  const thetaOrig = yi.reduce((s, y, i) => s + wOrig[i] * y, 0) / sumWOrig;

  return {
    method: '3-Parameter Selection Model',
    severity,
    selectionWeights: w,
    unadjusted: {
      theta: thetaOrig,
      se: Math.sqrt(1 / sumWOrig)
    },
    adjusted: {
      theta: thetaAdj,
      se: seAdj,
    ...
    bias: thetaOrig - thetaAdj,
    biasPercent: thetaOrig !== 0 ? (thetaOrig - thetaAdj) / Math.abs(thetaOrig) * 100 : 0,
    reference: 'Vevea & Hedges (1995) Psychometrika'
  };
}
 ...
 * p-Uniform* method
 */
function pUniformStar(yi, vi) {
  const k = yi.length;
  const sei = vi.map(v => Math.sqrt(v));
  const z = yi.map((y, i) => y / sei[i]);
  const pvals = z.map(zz => 1 - pnorm(zz)); // One-tailed p-values

  // Select significant studies (one-tailed p < 0.025)
  const sigIndices = pvals.map((p, i) => p < 0.025 ? i : -1).filter(i => i >= 0);

  if (sigIndices.length < 3) {
    return {
      method: 'p-Uniform*',
      error: 'Insufficient significant studies (need at least 3)',
      k,
      kSignificant: sigIndices.length
    };
  }

  const sigYi = sigIndices.map(i => yi[i]);
  const sigVi = sigIndices.map(i => vi[i]);
  const sigSei = sigIndices.map(i => sei[i]);

  // Fixed-effect estimate from significant studies
  const w = sigVi.map(v => 1 / v);
  const sumW = w.reduce((a, b) => a + b, 0);
  const thetaFE = sigYi.reduce((s, y, i) => s + w[i] * y, 0) / sumW;

  // p-Uniform* estimation via conditional p-values
  // Under the true effect ÃŽÂ¼, p-values should be uniform
  function conditionalPval(theta) {
    return sigIndices.map(i => {
      const ncp = theta / sei[i];
      const zObs = yi[i] / sei[i];
      // P(Z > z_obs | Z > z_crit, ÃŽÂ¼)
      const pCond = (1 - pnorm(zObs - ncp)) / (1 - pnorm(1.96 - ncp));
      return Math.min(Math.max(pCond, 0.001), 0.999);
    });
  }

  // Find theta that makes conditional p-values most uniform (KS test)
  let bestTheta = thetaFE;
  let bestKS = 1;

  for (let theta = thetaFE * 0.5; theta <= thetaFE * 1.5; theta += thetaFE * 0.05) {
    if (theta === 0) continue;
    const condP = conditionalPval(theta).sort((a, b) => a - b);
    const expected = condP.map((_, i) => (i + 0.5) / condP.length);
    const ks = Math.max(...condP.map((p, i) => Math.abs(p - expected[i])));
    if (ks < bestKS) {
      bestKS = ks;
      bestTheta = theta;
    }
  ...
  const bootThetas = [];
  for (let b = 0; b < nBoot; b++) {
    const bootIdx = Array.from({ length: sigIndices.length }, () =>
      sigIndices[Math.floor(Math.random() * sigIndices.length)]);
    const bootW = bootIdx.map(i => 1 / vi[i]);
    const bootSumW = bootW.reduce((a, b) => a + b, 0);
    const bootTheta = bootIdx.reduce((s, idx, j) => s + bootW[j] * yi[idx], 0) / bootSumW;
    bootThetas.push(bootTheta);
  }
  const seBoot = Math.sqrt(bootThetas.reduce((s, t) => s + Math.pow(t - bestTheta, 2), 0) / nBoot);

  return {
    method: 'p-Uniform*',
    k,
    kSignificant: sigIndices.length,
    naiveEstimate: thetaFE,
    adjustedEstimate: bestTheta,
    se: seBoot,
    ci_lower: bestTheta - 1.96 * seBoot,
    ci_upper: bestTheta + 1.96 * seBoot,
    bias: thetaFE - bestTheta,
    ksStatistic: bestKS,
    reference: 'van Aert et al. (2016) Behavior Research Methods'
  };
}
 ...
 * Limit Meta-Analysis
 */
function limitMetaAnalysis(yi, vi) {
  const k = yi.length;
  const sei = vi.map(v => Math.sqrt(v));

  // Weighted regression of effect on precision (1/SE)
  const precision = sei.map(se => 1 / se);
  const w = vi.map(v => 1 / v);
  const sumW = w.reduce((a, b) => a + b, 0);
  ...
  const meanPrec = precision.reduce((s, p, i) => s + w[i] * p, 0) / sumW;

  // Weighted regression
  const sumWXY = yi.reduce((s, y, i) => s + w[i] * (precision[i] - meanPrec) * (y - meanY), 0);
  const sumWX2 = precision.reduce((s, p, i) => s + w[i] * Math.pow(p - meanPrec, 2), 0);

  const beta1 = sumWXY / sumWX2;
  const beta0 = meanY - beta1 * meanPrec; // Intercept = limit as precision -> Ã¢Ë†Å¾

  // Standard error of intercept
  const residuals = yi.map((y, i) => y - (beta0 + beta1 * precision[i]));
  const mse = residuals.reduce((s, r, i) => s + w[i] * r * r, 0) / (k - 2);
  const seIntercept = Math.sqrt(mse * (1 / sumW + meanPrec * meanPrec / sumWX2));

  return {
    method: 'Limit Meta-Analysis',
    limitEstimate: beta0,
    se: seIntercept,
    ci_lower: beta0 - 1.96 * seIntercept,
    ci_upper: beta0 + 1.96 * seIntercept,
    slope: beta1,
    interpretation: beta1 > 0
      ? 'Small-study effects detected (larger effects in smaller studies)'
      : 'No small-study effects',
    reference: 'RÃƒÂ¼cker et al. (2011) Biostatistics'
  };
}
 ...
 */
function waapWLS(yi, vi, power = 0.8) {
  const k = yi.length;
  const sei = vi.map(v => Math.sqrt(v));
  ...
  const w = vi.map(v => 1 / v);
  const sumW = w.reduce((a, b) => a + b, 0);
  const thetaFE = yi.reduce((s, y, i) => s + w[i] * y, 0) / sumW;

  // Calculate power for each study
  const zBeta = qnorm(power);
  const zAlpha = 1.96;

  const studyPower = sei.map(se => {
    const ncp = Math.abs(thetaFE) / se;
    return 1 - pnorm(zAlpha - ncp);
  });

  // Identify adequately powered studies
  const adequateMask = studyPower.map(p => p >= power);
  const nAdequate = adequateMask.filter(Boolean).length;

  if (nAdequate < 2) {
    return {
      method: 'WAAP-WLS',
      error: 'Insufficient adequately powered studies',
      k,
      nAdequate,
  ...
  const waapW = adequateMask.map((adequate, i) => adequate ? w[i] : 0);
  const waapSumW = waapW.reduce((a, b) => a + b, 0);
  const thetaWAAP = yi.reduce((s, y, i) => s + waapW[i] * y, 0) / waapSumW;
  const seWAAP = Math.sqrt(1 / waapSumW);

  // WLS: Weighted by precision squared (downweight small studies more)
  const wlsW = vi.map(v => 1 / (v * v));
  const wlsSumW = wlsW.reduce((a, b) => a + b, 0);
  const thetaWLS = yi.reduce((s, y, i) => s + wlsW[i] * y, 0) / wlsSumW;

  return {
    method: 'WAAP-WLS',
    k,
    nAdequatelyPowered: nAdequate,
    powerThreshold: power,
    standardFE: thetaFE,
    WAAP: {
      theta: thetaWAAP,
      se: seWAAP,
      ci_lower: thetaWAAP - 1.96 * seWAAP,
      ci_upper: thetaWAAP + 1.96 * seWAAP
    },
    WLS: {
      theta: thetaWLS
    },
...
// ============================================================================
// CATEGORY 7: CLINICAL DECISION SUPPORT
// ============================================================================

/**
 * Threshold NNT Calculator
 */
function thresholdNNT(or, baselineRisk, threshold = null) {
  // Convert OR to RR (approximation for rare outcomes)
  const rr = or / (1 - baselineRisk + baselineRisk * or);

  // Absolute risk reduction
  const arr = baselineRisk - baselineRisk * rr;

  // NNT
  const nnt = arr !== 0 ? 1 / Math.abs(arr) : Infinity;

  // NNT at different baseline risks
  const nntByRisk = [0.01, 0.05, 0.1, 0.2, 0.3, 0.5].map(risk => {
    const rrAtRisk = or / (1 - risk + risk * or);
    const arrAtRisk = risk - risk * rrAtRisk;
    return {
      baselineRisk: risk,
      absoluteRiskReduction: arrAtRisk,
      nnt: arrAtRisk !== 0 ? Math.ceil(1 / Math.abs(arrAtRisk)) : Infinity,
      interpretation: arrAtRisk > 0 ? 'NNT (benefit)' : 'NNH (harm)'
    };
  });
  ...
  if (threshold) {
    // Find baseline risk where NNT equals threshold
    for (let risk = 0.001; risk <= 0.999; risk += 0.001) {
      const rrAtRisk = or / (1 - risk + risk * or);
      const arrAtRisk = risk - risk * rrAtRisk;
      const nntAtRisk = Math.abs(1 / arrAtRisk);
      if (nntAtRisk <= threshold) {
        thresholdResult = {
          thresholdNNT: threshold,
          minimumBaselineRisk: risk,
          interpretation: `Treatment worthwhile when baseline risk Ã¢â€°Â¥ ${(risk * 100).toFixed(1)}%`
        };
        break;
      }
    }
  ...
  return {
    method: 'Threshold NNT Calculator',
    oddsRatio: or,
    baselineRisk,
    riskRatio: rr,
    absoluteRiskReduction: arr,
    nnt: Math.ceil(nnt),
    nntByBaselineRisk: nntByRisk,
    thresholdAnalysis: thresholdResult,
    clinicalInterpretation: nnt < 10
      ? 'Very effective - treat ~' + Math.ceil(nnt) + ' patients to benefit one'
      : nnt < 50
 ...
 */
function decisionCurveAnalysis(or, baselineRisk, ciLower, ciUpper) {
  const thresholds = [];

  for (let pt = 0.01; pt <= 0.99; pt += 0.01) {
    // Net benefit calculation
    // NB = True positives/N - False positives/N Ãƒâ€” (pt/(1-pt))

    // Using OR and baseline risk
    const rr = or / (1 - baselineRisk + baselineRisk * or);
    const treatRisk = baselineRisk * rr;
    const noTreatRisk = baselineRisk;

    // Net benefit of treating all
    const nbTreatAll = noTreatRisk - (1 - noTreatRisk) * (pt / (1 - pt));

    // Net benefit of treating based on test (simplified)
    const nbTest = treatRisk - (1 - treatRisk) * (pt / (1 - pt));
    ...
    thresholds.push({
      ...
      netBenefit: {
        treatAll: nbTreatAll,
        treatNone: nbNone,
        difference: nbTreatAll - nbNone
      }
    });
  }

  // Find optimal threshold range
  const beneficial = thresholds.filter(t => t.netBenefit.treatAll > 0);
  ...
  return {
    ...
    baselineRisk,
    curve: thresholds,
    optimalRange: beneficial.length > 0 ? {
      lower: beneficial[0].threshold,
      upper: beneficial[beneficial.length - 1].threshold
    } : null,
    recommendation: beneficial.length > 0
      ? `Treatment beneficial when decision threshold between ${(beneficial[0].threshold * 100).toFixed(0)}% and ${(beneficial[beneficial.length - 1].threshold * 100).toFixed(0)}%`
      : 'Treatment not beneficial at any threshold',
 ...
 */
function valueOfInformation(theta, se, costPerPatient, nPotentialPatients) {
  // Expected Value of Perfect Information (EVPI)
  // EVPI = E[max(0, ÃŽÂ¸)] - max(0, E[ÃŽÂ¸])

  const mu = theta;
  const sigma = se;

  // E[max(0, ÃŽÂ¸)] where ÃŽÂ¸ ~ N(mu, sigmaÃ‚Â²)
  // = mu Ãƒâ€” ÃŽÂ¦(mu/sigma) + sigma Ãƒâ€” Ãâ€ (mu/sigma)
  const phi = (x) => Math.exp(-x * x / 2) / Math.sqrt(2 * Math.PI);
  const expectedMax = mu * pnorm(mu / sigma) + sigma * phi(mu / sigma);

  // max(0, E[ÃŽÂ¸])
  const maxExpected = Math.max(0, mu);

  // EVPI per patient (in effect size units)
  const evpiPerPatient = expectedMax - maxExpected;

  // Convert to monetary value
  const evpiTotal = evpiPerPatient * costPerPatient * nPotentialPatients;

  // Expected Value of Sample Information (EVSI) for additional study
  const nAdditional = 100; // Hypothetical additional study size
  const posteriorVar = 1 / (1 / (sigma * sigma) + nAdditional);
  const posteriorSigma = Math.sqrt(posteriorVar);
  const evsiPerPatient = mu * pnorm(mu / posteriorSigma) + posteriorSigma * phi(mu / posteriorSigma) - maxExpected;

  return {
    method: 'Value of Information Analysis',
    currentEstimate: { theta: mu, se: sigma },
    EVPI: {
      perPatient: evpiPerPatient,
      total: evpiTotal,
      interpretation: 'Maximum value of eliminating all uncertainty'
    },
    EVSI: {
      additionalN: nAdditional,
      perPatient: evsiPerPatient,
      total: evsiPerPatient * costPerPatient * nPotentialPatients,
      interpretation: `Value of conducting study with n=${nAdditional}`
    },
    recommendation: evpiTotal > 1000000
      ? 'High value in reducing uncertainty - further research worthwhile'
      : evpiTotal > 100000
        ? 'Moderate value - consider research if feasible'
        : 'Low value - current evidence may be sufficient',
    reference: 'Claxton et al. (2002) Health Economics'
  };
}
 ...
 */
function multiCriteriaDecision(treatments, criteria) {
  // treatments: array of { name, efficacy, safety, cost, convenience }
  // criteria: { efficacy: weight, safety: weight, cost: weight, convenience: weight }
  ...
  // Normalize scores within each criterion
  const normalized = {};
  for (const criterion in weights) {
    const values = treatments.map(t => t[criterion] || 0);
    const max = Math.max(...values);
    const min = Math.min(...values);
    const range = max - min || 1;

    // For cost, lower is better (invert)
    if (criterion === 'cost') {
      normalized[criterion] = values.map(v => 1 - (v - min) / range);
    } else {
      normalized[criterion] = values.map(v => (v - min) / range);
    }
  }

  // Calculate weighted scores
  const scores = treatments.map((t, i) => {
    let score = 0;
    for (const criterion in normalizedWeights) {
      score += normalizedWeights[criterion] * (normalized[criterion]?.[i] || 0);
    }
    return {
      treatment: t.name,
      weightedScore: score,
      criteriaScores: Object.keys(normalizedWeights).reduce((obj, c) => {
        obj[c] = normalized[c]?.[i] || 0;
        return obj;
      }, {})
    };
  });
  ...
  return {
    ...
    rankings: scores,
    bestOption: scores[0],
    sensitivity: 'Run with different weights to assess robustness'
  };
}

// ============================================================================
// CATEGORY 8: VISUALIZATION INNOVATIONS
// ============================================================================

/**
 * Albatross Plot data
 */
function albatrossPlotData(yi, vi, names = null) {
  const k = yi.length;
  const sei = vi.map(v => Math.sqrt(v));
  const z = yi.map((y, i) => y / sei[i]);
  const pvals = z.map(zz => 2 * (1 - pnorm(Math.abs(zz))));

  // Estimate sample sizes from SE (assuming binary outcome OR)
  // SE(logOR) Ã¢â€°Ë† sqrt(1/a + 1/b + 1/c + 1/d) Ã¢â€°Ë† sqrt(4/n) for balanced
  const estimatedN = vi.map(v => Math.round(4 / v));

  // Effect size contours for different sample sizes
  const contours = [50, 100, 200, 500, 1000].map(n => {
    const points = [];
    for (let effect = -2; effect <= 2; effect += 0.1) {
      const se = 2 / Math.sqrt(n);
      const z = effect / se;
      const p = 2 * (1 - pnorm(Math.abs(z)));
      points.push({ effect, p: -Math.log10(p), n });
    }
    return { n, points };
  });

  return {
    method: 'Albatross Plot',
    studies: yi.map((y, i) => ({
      name: names ? names[i] : `Study ${i + 1}`,
      effect: y,
      pvalue: pvals[i],
      negLogP: -Math.log10(pvals[i]),
      estimatedN: estimatedN[i],
      significant: pvals[i] < 0.05
    })),
    contours,
    interpretation: 'Studies should cluster along contour lines if effect is real',
    reference: 'Harrison et al. (2017) Int J Epidemiol'
  };
}

/**
 * Sunset Plot data (Power-enhanced funnel)
 */
function sunsetPlotData(yi, vi, expectedEffect = null) {
  const k = yi.length;
  const sei = vi.map(v => Math.sqrt(v));

  // If no expected effect, use pooled estimate
  if (expectedEffect === null) {
    const w = vi.map(v => 1 / v);
    const sumW = w.reduce((a, b) => a + b, 0);
    expectedEffect = yi.reduce((s, y, i) => s + w[i] * y, 0) / sumW;
  }

  // Calculate power for each study
  const power = sei.map(se => {
    const ncp = Math.abs(expectedEffect) / se;
    return 1 - pnorm(1.96 - ncp) + pnorm(-1.96 - ncp);
  });

  // Power contours
  const powerLevels = [0.33, 0.66, 0.95];
  const contours = powerLevels.map(targetPower => {
    // Find SE that gives target power for expected effect
    const zBeta = qnorm(targetPower);
    const requiredSE = Math.abs(expectedEffect) / (1.96 + zBeta);
    return {
      power: targetPower,
      se: requiredSE,
      label: `${Math.round(targetPower * 100)}% power`
    };
  });
  ...
  return {
    ...
    studies: yi.map((y, i) => ({
      ...
      se: sei[i],
      power: power[i],
      powerCategory: power[i] >= 0.8 ? 'Adequate' : power[i] >= 0.5 ? 'Low' : 'Very low',
      significant: Math.abs(y / sei[i]) > 1.96
    })),
    powerContours: contours,
    medianPower: [...power].sort((a, b) => a - b)[Math.floor(k / 2)],
    adequatelyPowered: power.filter(p => p >= 0.8).length,
    interpretation: 'Studies in shaded "sunset" region have inadequate power',
    reference: 'Schauer & Hedges (2021)'
  };
}

/**
 * Drapery Plot data (P-value function)
 */
function draperyPlotData(yi, vi) {
  const k = yi.length;
  const sei = vi.map(v => Math.sqrt(v));

  // Range of effect sizes to evaluate
  const pooledSE = Math.sqrt(1 / vi.reduce((s, v) => s + 1 / v, 0));
  const w = vi.map(v => 1 / v);
  const sumW = w.reduce((a, b) => a + b, 0);
  const pooled = yi.reduce((s, y, i) => s + w[i] * y, 0) / sumW;

  const effectRange = [];
  for (let e = pooled - 4 * pooledSE; e <= pooled + 4 * pooledSE; e += pooledSE / 20) {
    effectRange.push(e);
  }

  // P-value functions for each study
  const pvalueFunctions = yi.map((y, i) => ({
    study: `Study ${i + 1}`,
    curve: effectRange.map(e => {
      const z = (y - e) / sei[i];
      const p = 2 * (1 - pnorm(Math.abs(z)));
      return { effect: e, pvalue: p };
    })
  }));

  // Combined p-value function
  const combinedCurve = effectRange.map(e => {
    const z = (pooled - e) / pooledSE;
    const p = 2 * (1 - pnorm(Math.abs(z)));
    return { effect: e, pvalue: p };
  });
  ...
  return {
    ...
    pooledSE,
    studyCurves: pvalueFunctions,
    combinedCurve,
    interpretation: 'Shows p-value for testing each possible effect size',
    reference: 'Infanger & Schmidt-TrucksÃƒÂ¤ss (2019)'
  };
}
 ...
 * Harvest Plot data
 */
function harvestPlotData(yi, vi, n, names = null, qualities = null) {
  // Convert to internal format
  // yi: effect sizes, vi: variances, n: sample sizes
  const k = yi.length;

  const studies = yi.map((effect, i) => ({
    name: names ? names[i] : `Study ${i + 1}`,
    effectSize: effect,
    effectDirection: effect > 0.1 ? 'benefit' : effect < -0.1 ? 'harm' : 'noEffect',
    quality: qualities ? qualities[i] : (vi[i] < 0.05 ? 'high' : vi[i] < 0.15 ? 'medium' : 'low'),
    sampleSize: n[i] || 100
  }));

  const categorized = {
    benefit: { low: [], medium: [], high: [] },
    noEffect: { low: [], medium: [], high: [] },
    harm: { low: [], medium: [], high: [] }
  };

  studies.forEach(s => {
    categorized[s.effectDirection][s.quality].push({
      ...s,
      height: Math.log10(s.sampleSize || 100)
    });
  });
  ...
  return {
    ...
    categorized,
    summary: {
      benefit: Object.values(categorized.benefit).flat().length,
      noEffect: Object.values(categorized.noEffect).flat().length,
      harm: Object.values(categorized.harm).flat().length
    },
    interpretation: 'Visual summary of effect direction, quality, and sample size',
    reference: 'Ogilvie et al. (2008) BMC Med Res Methodol'
  };
}
...
if (typeof window !== 'undefined') {
  // RoBMA
  window.robmaModelAveraging = robmaModelAveraging;
  window.interpretBF = interpretBF;
  window.metaAnalyticZCurve = metaAnalyticZCurve;

  // Multiverse
  window.multiverseMetaAnalysis = multiverseMetaAnalysis;
  window.specificationCurveAnalysis = specificationCurveAnalysis;
  window.vibrationOfEffects = vibrationOfEffects;

  // Prediction Intervals
  window.improvedPredictionInterval = improvedPredictionInterval;
  window.calibratedPredictionInterval = calibratedPredictionInterval;

  // Living Review
  window.sequentialMetaAnalysis = sequentialMetaAnalysis;
  window.calculateInformationSize = calculateInformationSize;
  window.livingReviewStatus = livingReviewStatus;

  // Heterogeneity
  window.metaRegressionR2 = metaRegressionR2;
  window.permutationTestI2 = permutationTestI2;
  window.robustTau2 = robustTau2;

  // Publication Bias
  window.threeParameterSelectionModel = threeParameterSelectionModel;
  window.pUniformStar = pUniformStar;
  window.limitMetaAnalysis = limitMetaAnalysis;
  window.waapWLS = waapWLS;

  // Clinical Decision
  window.thresholdNNT = thresholdNNT;
  window.decisionCurveAnalysis = decisionCurveAnalysis;
  window.valueOfInformation = valueOfInformation;
  window.multiCriteriaDecision = multiCriteriaDecision;

  // Visualization
  window.albatrossPlotData = albatrossPlotData;
  window.sunsetPlotData = sunsetPlotData;
  window.draperyPlotData = draperyPlotData;
  window.harvestPlotData = harvestPlotData;

  console.log('TruthCert Innovative Features v2.0 loaded - 35 new functions');
}

// ============================================================================
// FINAL GAP CLOSURE: GLMM, Network MA, Profile Likelihood
// ============================================================================

/**
 * Generalized Linear Mixed Model Meta-Analysis for Rare Events
 * Approximate counterpart to metafor's rma.glmm() for sparse/zero-cell data
 * Uses binomial-normal likelihood with Gauss-Hermite quadrature
 */
function glmmMetaAnalysis(ai, bi, ci, di, options = {}) {
  const k = ai.length;
  if (0 === k) return {
    error: "No studies provided"
  };
  const measure = options.measure || "OR",
    method = options.method || "ML",
    nQuad = Math.max(7, Math.min(41, options.nQuad || 13)),
    continuity = options.continuity || .5,
    n1 = ai.map((e, t) => e + bi[t]),
    n2 = ci.map((e, t) => e + di[t]),
    hasZeroCells = ai.some((e, t) => 0 === e || 0 === bi[t] || 0 === ci[t] || 0 === di[t]),
    ghNodes = gaussHermiteNodes(nQuad),
    yi = [],
    vi = [];
  for (let e = 0; e < k; e++) {
    const t = 0 === ai[e] || 0 === bi[e] || 0 === ci[e] || 0 === di[e] ? continuity : 0,
      n = ai[e] + t,
      a = bi[e] + t,
      s = ci[e] + t,
      i = di[e] + t;
    yi.push(Math.log(n * i / (a * s))), vi.push(1 / n + 1 / a + 1 / s + 1 / i)
  }
  const tau2Init = Math.max(estimateTau2_DL(yi, vi).tau2, 1e-4),
    muInit = calculatePooledEstimate(yi, vi, tau2Init).theta,
    baselineLogit = ai.map((e, t) => {
      const a = (e + ci[t] + continuity) / (n1[t] + n2[t] + 2 * continuity);
      return Math.log(a / (1 - a))
    });

  function clampProbability(e) {
    return Math.min(1 - 1e-12, Math.max(1e-12, e))
  }
  ...
  function logBinomial(e, t, n) {
    const a = clampProbability(n);
    return lgamma(t + 1) - lgamma(e + 1) - lgamma(t - e + 1) + e * Math.log(a) + (t - e) * Math.log(1 - a)
  }

  function studyLogLikelihood(e, t) {
    if ("OR" === measure) {
      const n = clampProbability(invLogit(baselineLogit[e] + t / 2)),
        a = clampProbability(invLogit(baselineLogit[e] - t / 2));
      return logBinomial(ai[e], n1[e], n) + logBinomial(ci[e], n2[e], a)
    }
    const n = clampProbability(invLogit(baselineLogit[e])),
      a = clampProbability(n * Math.exp(t));
    return logBinomial(ai[e], n1[e], a) + logBinomial(ci[e], n2[e], n)
  }

  function logLikelihood(e, t) {
    if (!isFinite(e) || !isFinite(t) || t < 0) return -1 / 0;
    const n = Math.max(t, 1e-8);
    let a = 0;
    for (let t = 0; t < k; t++) {
      const s = [];
      for (let i = 0; i < ghNodes.nodes.length; i++) {
        const r = e + Math.sqrt(2) * n * ghNodes.nodes[i],
          o = Math.log(Math.max(ghNodes.weights[i], 1e-300)) + studyLogLikelihood(t, r);
        isFinite(o) && s.push(o)
      }
      if (0 === s.length) return -1 / 0;
      const i = Math.max(...s),
        r = s.reduce((e, t) => e + Math.exp(t - i), 0);
      a += i + Math.log(r) - .5 * Math.log(Math.PI)
    }
    return a
  }

  function optimize1DMax(e, t, n, a = 1e-5, s = 80) {
    ...
    let r = t,
      ...
      c = e(l),
      u = e(d);
    for (let t = 0; t < s && Math.abs(o - r) > a; t++)
      c > u ? (o = d, d = l, u = c, l = o - i * (o - r), c = e(l)) : (r = l, l = d, c = u, d = r + i * (o - r), u = e(d));
    const p = (r + o) / 2;
    return {
      x: p,
      fx: e(p)
  ...
  function localOptimize(e, t) {
    let n = e,
      a = Math.max(t, 1e-6),
      s = logLikelihood(n, a),
      i = 0;
    for (; i < 18; i++) {
      const e = optimize1DMax(e => logLikelihood(e, a), n - 2.5, n + 2.5, 1e-4, 90);
      n = e.x;
      const t = Math.log(a),
        r = optimize1DMax(e => logLikelihood(n, Math.exp(e)), Math.max(-9, t - 1.5), Math.min(2, t + 1.5), 1e-4, 90);
      a = Math.max(Math.exp(r.x), 1e-8);
      const o = logLikelihood(n, a);
      if (Math.abs(o - s) < 1e-6) {
        s = o;
        break
      }
      s = o
  ...
  const starts = [
      [muInit, Math.sqrt(tau2Init)],
      [0, .1],
      [muInit - .5, .2],
      [muInit + .5, .2],
      [muInit, .8]
    ],
    candidates = starts.map(e => localOptimize(e[0], e[1])),
    best = candidates.reduce((e, t) => !e || t.ll > e.ll ? t : e, null),
    h = 1e-3,
    ll0 = best.ll,
    llP = logLikelihood(best.mu + h, best.tau),
    llM = logLikelihood(best.mu - h, best.tau),
    d2ll = (llP - 2 * ll0 + llM) / (h * h),
    fallbackInfo = vi.reduce((e, t) => e + 1 / (t + best.tau * best.tau), 0),
    seMu = d2ll < -1e-10 ? Math.sqrt(-1 / d2ll) : Math.sqrt(1 / Math.max(fallbackInfo, 1e-8)),
    tau2 = best.tau * best.tau,
    ciLowerLog = best.mu - 1.96 * seMu,
    ciUpperLog = best.mu + 1.96 * seMu,
    transform = "OR" === measure || "RR" === measure ? e => Math.exp(e) : e => e;
  return {
    method: "Generalized Linear Mixed Model (GLMM)",
    model: "Binomial-Normal (Gauss-Hermite quadrature)",
    estimation: method,
    measure,
    k,
    estimate: {
      log: best.mu,
      exp: Math.exp(best.mu),
      se: seMu
    },
    tau2,
    tau: best.tau,
    ...
    hasZeroCells,
    zeroCellHandling: "Exact binomial likelihood with logistic-normal random effects",
    quadraturePoints: nQuad,
    optimizer: {
      strategy: "coordinate-wise bounded likelihood maximization",
      starts: starts.length,
      converged: !0,
      iterations: best.iterations
    },
    interpretation: hasZeroCells ? "GLMM fit completed and is suitable for sparse/zero-cell data" : "GLMM fit completed with exact binomial likelihood",
    reference: "Stijnen et al. (2010) Statistics in Medicine"
  }
}
const GAUSS_HERMITE_CACHE = {};

function hermitePolyAndPrev(e, t) {
  if (0 === e) return {
    hn: 1,
    hn1: 0
  };
...
function gaussHermiteNodes(n) {
  const e = Math.max(3, Math.floor(n || 13));
  if (GAUSS_HERMITE_CACHE[e]) return GAUSS_HERMITE_CACHE[e];
  const t = new Array(e),
    a = new Array(e),
    s = Math.floor((e + 1) / 2),
    i = 1e-14;
  for (let n = 1; n <= s; n++) {
    let s = Math.sqrt(2 * e + 1) * Math.cos(Math.PI * (4 * n - 1) / (4 * e + 2));
    for (let t = 0; t < 60; t++) {
      const n = hermitePolyAndPrev(e, s),
        a = n.hn / (2 * e * n.hn1);
      if (s -= a, Math.abs(a) < i) break
    }
    const r = hermitePolyAndPrev(e, s),
      o = (e - 1) * Math.log(2) + lgamma(e + 1) + .5 * Math.log(Math.PI) - 2 * Math.log(e) - 2 * Math.log(Math.abs(r.hn1)),
      l = Math.exp(o);
    t[n - 1] = -s, t[e - n] = s, a[n - 1] = l, a[e - n] = l
  }
  const r = a.reduce((e, t) => e + t, 0),
    o = Math.sqrt(Math.PI) / (r || 1);
  for (let e = 0; e < a.length; e++) a[e] *= o;
  const l = {
    nodes: t,
    weights: a
  };
  return GAUSS_HERMITE_CACHE[e] = l, l
}
function betaBinomialMA(ai, bi, ci, di) {
  const k = ai.length;
  const n1 = ai.map((a, i) => a + bi[i]);
  const n2 = ci.map((c, i) => c + di[i]);

  // Method of moments for beta parameters
  const p1_mean = ai.reduce((s, a, i) => s + a / n1[i], 0) / k;
  const p2_mean = ci.reduce((s, c, i) => s + c / n2[i], 0) / k;

  const p1_var = ai.reduce((s, a, i) => s + Math.pow(a / n1[i] - p1_mean, 2), 0) / (k - 1);
  const p2_var = ci.reduce((s, c, i) => s + Math.pow(c / n2[i] - p2_mean, 2), 0) / (k - 1);

  // Estimate overdispersion
  const rho1 = Math.max(0, (p1_var - p1_mean * (1 - p1_mean) / k) / (p1_mean * (1 - p1_mean)));
  const rho2 = Math.max(0, (p2_var - p2_mean * (1 - p2_mean) / k) / (p2_mean * (1 - p2_mean)));

  // Pooled log-OR with overdispersion adjustment
  const logOR = Math.log((p1_mean / (1 - p1_mean + 1e-10)) / (p2_mean / (1 - p2_mean + 1e-10) + 1e-10));
  const seLogOR = Math.sqrt(1/(k * p1_mean * (1 - p1_mean)) * (1 + rho1) +
                            1/(k * p2_mean * (1 - p2_mean)) * (1 + rho2));

  return {
    method: 'Beta-Binomial Meta-Analysis',
    k,
    estimate: {
      logOR,
      OR: Math.exp(logOR),
    ...
    ci: {
      lower: Math.exp(logOR - 1.96 * seLogOR),
      upper: Math.exp(logOR + 1.96 * seLogOR)
    },
    overdispersion: {
      treatment: rho1,
      control: rho2,
      interpretation: rho1 > 0.1 || rho2 > 0.1 ? 'Substantial overdispersion detected' : 'Minimal overdispersion'
    },
    reference: 'Kuss (2015) Statistics in Medicine'
  };
}

/**
 * Network Meta-Analysis Consistency Tests
 * Implements node-splitting and design-by-treatment interaction
 */
function networkMAConsistency(treatments, comparisons) {
  // treatments: array of treatment names
  // comparisons: array of {t1, t2, effect, se, design}

  const k = comparisons.length;
  const nTreatments = treatments.length;

  // Build design matrix
  const designs = [...new Set(comparisons.map(c => c.design))];
  ...
  comparisons.forEach(c => {
    ...
    directEvidence[key].push(c);
  });

  // Node-splitting analysis
  const nodeSplitResults = [];

  Object.keys(directEvidence).forEach(comparison => {
    const [t1, t2] = comparison.split('-');
    const direct = directEvidence[comparison];

    // Pool direct evidence
    const directW = direct.map(d => 1 / (d.se * d.se));
    const directSum = directW.reduce((a, b) => a + b, 0);
    const directEffect = direct.reduce((s, d, i) => s + directW[i] * d.effect, 0) / directSum;
    const directSE = Math.sqrt(1 / directSum);

    // Find indirect paths (simplified - assumes single intermediate)
    let indirectEffect = null;
    let indirectSE = null;

    treatments.forEach(intermediate => {
      if (intermediate !== t1 && intermediate !== t2) {
        const path1Key = [t1, intermediate].sort().join('-');
        const path2Key = [intermediate, t2].sort().join('-');

        if (directEvidence[path1Key] && directEvidence[path2Key]) {
          const e1 = directEvidence[path1Key];
          const e2 = directEvidence[path2Key];

          const w1 = e1.map(d => 1 / (d.se * d.se));
          const w2 = e2.map(d => 1 / (d.se * d.se));
          const effect1 = e1.reduce((s, d, i) => s + w1[i] * d.effect, 0) / w1.reduce((a, b) => a + b, 0);
          const effect2 = e2.reduce((s, d, i) => s + w2[i] * d.effect, 0) / w2.reduce((a, b) => a + b, 0);

          // Direction adjustment
          const sign1 = path1Key.split('-')[0] === t1 ? 1 : -1;
          const sign2 = path2Key.split('-')[0] === intermediate ? 1 : -1;

          indirectEffect = sign1 * effect1 + sign2 * effect2;
          indirectSE = Math.sqrt(1 / w1.reduce((a, b) => a + b, 0) + 1 / w2.reduce((a, b) => a + b, 0));
        }
      }
    });

    if (indirectEffect !== null) {
      const difference = directEffect - indirectEffect;
      const seDiff = Math.sqrt(directSE * directSE + indirectSE * indirectSE);
      const z = difference / seDiff;
      const pValue = 2 * (1 - pnorm(Math.abs(z)));

      nodeSplitResults.push({
        comparison,
        direct: { effect: directEffect, se: directSE },
        indirect: { effect: indirectEffect, se: indirectSE },
        difference,
        seDifference: seDiff,
        z,
        pValue,
  ...
  // Design-by-treatment interaction test (simplified Q statistic)
  let Q_inconsistency = 0;
  let df_inconsistency = 0;

  nodeSplitResults.forEach(ns => {
    Q_inconsistency += Math.pow(ns.z, 2);
    df_inconsistency += 1;
  });

  const pValue_global = df_inconsistency > 0 ? 1 - pchisq(Q_inconsistency, df_inconsistency) : 1;

  // IÃ‚Â² for inconsistency
  const I2_inconsistency = df_inconsistency > 0 ?
    Math.max(0, (Q_inconsistency - df_inconsistency) / Q_inconsistency * 100) : 0;

  return {
    method: 'Network Meta-Analysis Consistency Assessment',
    nTreatments,
    nComparisons: k,
    nDesigns,
    nodeSplitting: {
      results: nodeSplitResults,
      nInconsistent: nodeSplitResults.filter(r => r.inconsistent).length,
      inconsistentComparisons: nodeSplitResults.filter(r => r.inconsistent).map(r => r.comparison)
    },
    globalInconsistency: {
      Q: Q_inconsistency,
      df: df_inconsistency,
      pValue: pValue_global,
      I2: I2_inconsistency
    },
    interpretation: pValue_global < 0.05
      ? 'Significant inconsistency detected - interpret NMA results with caution'
      : I2_inconsistency > 50
        ? 'Moderate inconsistency - consider sensitivity analyses'
        : 'No significant inconsistency - direct and indirect evidence agree',
    reference: 'Dias et al. (2010) Statistics in Medicine; Higgins et al. (2012) BMJ'
  };
}

/**
 * Side-splitting (SIDE) test for network MA
 */
function sideSplittingTest(network) {
  // Wrapper for node-splitting with cleaner interface
  return networkMAConsistency(network.treatments, network.comparisons);
}

/**
 * Net heat plot data for network MA inconsistency visualization
 */
function netHeatPlotData(consistencyResults) {
  const comparisons = consistencyResults.nodeSplitting.results;
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j < n; j++) {
      if (i === j) {
        ...
        // Contribution to inconsistency
        const contrib = Math.abs(comparisons[i].z * comparisons[j].z) /
                       (Math.abs(comparisons[i].z) + Math.abs(comparisons[j].z) + 0.01);
        row.push(contrib);
      }
    }
    matrix.push(row);
  ...
  return {
    method: 'Net Heat Plot Data',
    labels: comparisons.map(c => c.comparison),
    matrix,
    hotspots: comparisons.filter(c => c.inconsistent).map(c => c.comparison),
    reference: 'Krahn et al. (2013) BMC Medical Research Methodology'
  };
}

/**
 * Profile Likelihood for tauÃ‚Â² and other parameters
 * Equivalent to metafor's profile()
 */
function profileLikelihood(yi, vi, parameter = 'tau2', range = null, nPoints = 50) {
  const k = yi.length;

  // Fixed-effect estimate
  const w_fe = vi.map(v => 1 / v);
  const sumW_fe = w_fe.reduce((a, b) => a + b, 0);
  const theta_fe = yi.reduce((s, y, i) => s + w_fe[i] * y, 0) / sumW_fe;

  // REML estimate of tauÃ‚Â²
  const Q = yi.reduce((s, y, i) => s + w_fe[i] * Math.pow(y - theta_fe, 2), 0);
  const c = sumW_fe - w_fe.reduce((s, w) => s + w * w, 0) / sumW_fe;
  const tau2_reml = Math.max(0, (Q - (k - 1)) / c);

  // Define range for profiling
  let profileRange;
  if (range) {
    profileRange = range;
  } else if (parameter === 'tau2') {
    profileRange = [0, Math.max(tau2_reml * 4, 1)];
  } else if (parameter === 'mu') {
    const se_theta = Math.sqrt(1 / sumW_fe + tau2_reml);
    profileRange = [theta_fe - 4 * se_theta, theta_fe + 4 * se_theta];
  }

  // Profile likelihood function
  function restrictedLogLikelihood(tau2, mu = null) {
    const w = vi.map(v => 1 / (v + tau2));
    const sumW = w.reduce((a, b) => a + b, 0);
    const theta = mu !== null ? mu : yi.reduce((s, y, i) => s + w[i] * y, 0) / sumW;
  ...
  // Generate profile
  const profile = [];
  const step = (profileRange[1] - profileRange[0]) / (nPoints - 1);

  let maxLL = -Infinity;
  let maxParam = null;

  for (let i = 0; i < nPoints; i++) {
    const paramValue = profileRange[0] + i * step;
    let ll;

    if (parameter === 'tau2') {
      ll = restrictedLogLikelihood(paramValue);
    } else if (parameter === 'mu') {
      ll = restrictedLogLikelihood(tau2_reml, paramValue);
    }
  ...
  // Calculate confidence interval (likelihood ratio test)
  const criticalValue = 3.841 / 2; // chi-squared(1) / 2 for 95% CI
  const threshold = maxLL - criticalValue;

  let ciLower = profileRange[0];
  let ciUpper = profileRange[1];

  for (let i = 0; i < profile.length; i++) {
    if (profile[i].logLikelihood >= threshold) {
      ciLower = profile[i].value;
      break;
    }
  }
  ...
  return {
    ...
    nPoints,
    plotData: {
      x: profile.map(p => p.value),
      y: profile.map(p => p.logLikelihood),
      threshold,
      estimate: maxParam
    },
    interpretation: `Profile likelihood CI for ${parameter}: [${ciLower.toFixed(4)}, ${ciUpper.toFixed(4)}]`,
    reference: 'Hardy & Thompson (1996) Statistics in Medicine'
  };
}
 ...
 */
function likelihoodRatioTest(yi, vi) {
  const k = yi.length;

  // Null model (fixed effect, tauÃ‚Â² = 0)
  const w0 = vi.map(v => 1 / v);
  const sumW0 = w0.reduce((a, b) => a + b, 0);
  const theta0 = yi.reduce((s, y, i) => s + w0[i] * y, 0) / sumW0;
  ...
  let ll0 = 0;
  for (let i = 0; i < k; i++) {
    ll0 -= 0.5 * Math.log(2 * Math.PI * vi[i]);
    ll0 -= 0.5 * Math.pow(yi[i] - theta0, 2) / vi[i];
  }

  // Alternative model (random effects)
  const Q = yi.reduce((s, y, i) => s + w0[i] * Math.pow(y - theta0, 2), 0);
  const c = sumW0 - w0.reduce((s, w) => s + w * w, 0) / sumW0;
  const tau2 = Math.max(0, (Q - (k - 1)) / c);

  const w1 = vi.map(v => 1 / (v + tau2));
  const sumW1 = w1.reduce((a, b) => a + b, 0);
  const theta1 = yi.reduce((s, y, i) => s + w1[i] * y, 0) / sumW1;

  let ll1 = 0;
  for (let i = 0; i < k; i++) {
    const totalVar = vi[i] + tau2;
    ll1 -= 0.5 * Math.log(2 * Math.PI * totalVar);
    ll1 -= 0.5 * Math.pow(yi[i] - theta1, 2) / totalVar;
  }

  // Likelihood ratio statistic
  const LR = 2 * (ll1 - ll0);

  // P-value (mixture of chi-squared, since tauÃ‚Â² >= 0)
  const pValue = 0.5 * (1 - pchisq(LR, 0)) + 0.5 * (1 - pchisq(LR, 1));

  return {
    method: 'Likelihood Ratio Test for Heterogeneity',
    nullHypothesis: 'tauÃ‚Â² = 0 (homogeneity)',
    LR_statistic: LR,
    df: '0:1 mixture',
    pValue,
    tau2_ML: tau2,
    ...
    interpretation: pValue < 0.05
      ? 'Reject homogeneity - significant heterogeneity present'
...
// ============================================================================
// EXPORT GAP CLOSURE FUNCTIONS
// ============================================================================

if (typeof window !== 'undefined') {
  // GLMM
  window.glmmMetaAnalysis = glmmMetaAnalysis;
  window.betaBinomialMA = betaBinomialMA;
  window.gaussHermiteNodes = gaussHermiteNodes;

  // Network MA Consistency
  window.networkMAConsistency = networkMAConsistency;
  window.sideSplittingTest = sideSplittingTest;
  window.netHeatPlotData = netHeatPlotData;

  // Profile Likelihood
  window.profileLikelihood = profileLikelihood;
  window.likelihoodRatioTest = likelihoodRatioTest;

  console.log('TruthCert Gap Closure v1.0 loaded - GLMM, Network MA, Profile Likelihood');
}
...
function metaAnalysisDL_boot(yi, vi) {
  ...
  const se = Math.sqrt(1 / sumW_re);
  const I2 = calculateI2Metafor(tau2, vi, k);
...
function saveProject(filename = null) {
    // Capture current table state; fallback to analyzed results for legacy states
  const studyData = getStudyTableSnapshot().length > 0
    ? getStudyTableSnapshot()
    : (AppState.results && AppState.results.studies
       ? AppState.results.studies.map((s, i) => ({
           study: AppState.results.names ? AppState.results.names[i] : ('Study ' + (i+1)),
           yi: s.yi,
           vi: s.vi,
           sei: Math.sqrt(s.vi)
         }))
...
function loadProject(jsonOrFile) {
  return new Promise((resolve, reject) => {
    if (typeof jsonOrFile === 'string') {
      ...
      reject(new Error('Unsupported project input. Provide JSON text or a file.'));
    }
  });
}
...
function applyProjectState(state) {
  if (state.settings) {
    ...
    syncTau2MethodSelectors();
    if (state.settings.tau2Method) {
      const tau2MethodSelect = document.getElementById('tau2MethodSelect');
      const tau2MethodSelectPanel = document.getElementById('tau2MethodSelectPanel');
      tau2MethodSelect && (tau2MethodSelect.value = state.settings.tau2Method);
      tau2MethodSelectPanel && (tau2MethodSelectPanel.value = state.settings.tau2Method);
    }
  }
  if (state.data && state.data.length > 0) {
    document.getElementById('studyTableBody').innerHTML = '';
    state.data.forEach(study => addStudyRow(study, !1));
  }
  if (state.results) {
  ...
  canUseUndoManager() && (UndoManager.clear(), UndoManager.saveState('Load project'));
...
let _metaSprintBridgeListenersInstalled = false;

function _safeParseJSONText(text) {
  try {
...
function _normalizeMetaSprintBridgeEnvelope(input) {
...
function _isMetaSprintBridgeStateValid(state) {
...
function _runTruthCertWhenReady(attempt = 0) {
  if (typeof runTruthCertAnalysis !== "function") return;
  if (AppState && AppState.results && AppState.results.pooled) {
    runTruthCertAnalysis();
    if (typeof exportTruthCertMethodsResultsForMetaSprint === 'function') {
      exportTruthCertMethodsResultsForMetaSprint({ silent: true });
    }
    goToTab("verdict");
    return;
  }
  if (attempt < 30) {
    safeTimeout(() => _runTruthCertWhenReady(attempt + 1), 250);
  }
}

function buildTruthCertMethodsResultsWriteback() {
  const results = AppState && AppState.results ? AppState.results : null;
  const settings = AppState && AppState.settings ? AppState.settings : {};
  const truth = AppState && AppState.truthcert ? AppState.truthcert : null;
  if (!results || !results.pooled || !Number.isFinite(results.pooled.theta)) {
    return {
      ok: false,
      reason: 'Run analysis first.'
    };
  }
  ...
  if (Number.isFinite(piLo) && Number.isFinite(piHi)) {
    resultsMarkdown += 'Prediction interval: ' + piLo.toFixed(4) + ' to ' + piHi.toFixed(4) + '. ';
  }
  if (truth && truth.verdict && truth.verdict.verdict) {
    resultsMarkdown += 'TruthCert evidence verdict: ' + String(truth.verdict.verdict) + '.';
  }
  resultsMarkdown += '\n';
...
function exportTruthCertMethodsResultsForMetaSprint(options = {}) {
  const built = buildTruthCertMethodsResultsWriteback();
  if (!built.ok) {
    if (!options.silent) showToast('Cannot export Methods+Results: ' + built.reason, 'warning');
    return built;
  }
  try {
    localStorage.setItem(TRUTHCERT_TO_METASPRINT_WRITEBACK_KEY, JSON.stringify(built.payload));
    if (!options.silent) showToast('TruthCert Methods+Results sent to MetaSprint bridge.', 'success');
    return {
      ok: true,
      payload: built.payload
    };
  } catch (err) {
    if (!options.silent) showToast('Failed to write MetaSprint bridge payload: ' + (err.message || 'unknown error'), 'error');
    return {
      ok: false,
...
function importMetaSprintBridgePayload(input, options = {}) {
  const envelope = _normalizeMetaSprintBridgeEnvelope(input);
  if (!envelope) {
    return {
      ok: false,
  ...
  if (!_isMetaSprintBridgeStateValid(state)) {
    showToast("MetaSprint bridge payload invalid or too small (need >=2 studies)", "error");
    return {
      ok: false,
  ...
  showToast("Imported " + importedN + " studies from MetaSprint", "success");

  try {
    localStorage.removeItem(METASPRINT_BRIDGE_STORAGE_KEY);
  } catch (_) {}

  const shouldAutoRun = options.autoRun !== void 0 ? !!options.autoRun : (envelope.autoRun !== false);
  const shouldAutoTruthCert = options.autoTruthCert !== void 0 ? !!options.autoTruthCert : (envelope.autoTruthCert !== false);

  goToTab("data");
  if (shouldAutoRun && typeof runAnalysis === "function") {
    safeTimeout(() => {
      try {
        runAnalysis();
        if (shouldAutoTruthCert) safeTimeout(() => _runTruthCertWhenReady(0), 600);
      } catch (err) {
        showToast("Auto-run failed after import: " + (err.message || "unknown error"), "warning");
      }
    }, 120);
  }
...
function consumePendingMetaSprintBridge() {
  let payload = null;
  try {
    payload = localStorage.getItem(METASPRINT_BRIDGE_STORAGE_KEY);
  } catch (_) {
    payload = null;
  }
  if (!payload) return {
  ...
  const parsed = _safeParseJSONText(payload);
  if (!parsed) {
    try {
      localStorage.removeItem(METASPRINT_BRIDGE_STORAGE_KEY);
    } catch (_) {}
  ...
  return importMetaSprintBridgePayload(parsed, {});
}

function installMetaSprintBridgeListeners() {
  if (_metaSprintBridgeListenersInstalled || typeof window === "undefined") return;
  _metaSprintBridgeListenersInstalled = true;

  window.addEventListener("message", event => {
    const data = event && event.data ? event.data : null;
    if (!data || typeof data !== "object") return;
    if (data.type !== "metasprint-truthcert-import") return;
    importMetaSprintBridgePayload(data, {});
  });

  window.addEventListener("load", () => {
    safeTimeout(() => consumePendingMetaSprintBridge(), 250);
  });
}
...
const UndoManager = {
  ...
  redo() {
    ...
    if (dataTypeSelect && AppState.settings.dataType) {
      dataTypeSelect.value = AppState.settings.dataType;
      updateEffectMeasures();
      updateTableHeaders();
      syncTau2MethodSelectors();
      const effectMeasureSelect = document.getElementById('effectMeasureSelect');
      effectMeasureSelect && AppState.settings.effectMeasure && (effectMeasureSelect.value = AppState.settings.effectMeasure);
      const tau2MethodSelect = document.getElementById('tau2MethodSelect');
      tau2MethodSelect && AppState.settings.tau2Method && (tau2MethodSelect.value = AppState.settings.tau2Method);
      const tau2MethodSelectPanel = document.getElementById('tau2MethodSelectPanel');
      tau2MethodSelectPanel && AppState.settings.tau2Method && (tau2MethodSelectPanel.value = AppState.settings.tau2Method);
      const hksjCheckbox = document.getElementById('hksjCheckbox');
      hksjCheckbox && (hksjCheckbox.checked = !!AppState.settings.hksj);
      const bayesianCheckbox = document.getElementById('bayesianCheckbox');
      bayesianCheckbox && (bayesianCheckbox.checked = !!AppState.settings.bayesian)
    }
    const tableBody = document.getElementById('studyTableBody');
    if (tableBody) {
      tableBody.innerHTML = '';
      const snapshot = Array.isArray(state.data) ? state.data : [];
      if (snapshot.length > 0) for (const row of snapshot) addStudyRow(row, !1);
      else for (let e = 0; e < 5; e++) addStudyRow(null, !1);
      renumberRows(), updateDataSummary()
    }
  },
...
function calculateICERInterventions(costs, effects, options = {}) {
...
if (typeof window !== 'undefined') {
  ...
  window.importMetaSprintBridgePayload = importMetaSprintBridgePayload;
  window.consumePendingMetaSprintBridge = consumePendingMetaSprintBridge;
  window.installMetaSprintBridgeListeners = installMetaSprintBridgeListeners;
  window.buildTruthCertMethodsResultsWriteback = buildTruthCertMethodsResultsWriteback;
  window.exportTruthCertMethodsResultsForMetaSprint = exportTruthCertMethodsResultsForMetaSprint;
  installMetaSprintBridgeListeners();
